...
//                 float scale;
    ...
    // write model version
    std::string model_name = std::string(caffemodel);
    std::string model_version = "";
    size_t pos1 = model_name.rfind('.');
    if (pos1 != std::string::npos) {
        size_t pos2 = model_name.rfind('/');
        if (pos2 != std::string::npos) {
            if (pos2 < pos1) {
                model_version = model_name.substr(pos2, pos1);
            }
        } else {
            model_version = model_name.substr(0, pos1);
        }

    }
    fprintf(pp, "\n\n\n\nVER: %s", model_version.c_str());
