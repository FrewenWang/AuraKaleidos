 ...
 * in a single API call. The API can be used with QnnSignal. When the function is invoked, the
 *        deserialization/initialization of each context will occur in the background. As soon as a
 *        graph reaches an executable state (i.e., initialization is completed), the client will
 *        receive a notification via the specified notification function. Once notification is
 ...
 *            NULL value can be supplied for the device handle, which is equivalent to calling
 *            QnnDevice_create() with a NULL config.
 *
