...
def predict_images(net):
    ...
    images_path = "/root/paddlejob/data_train/val_resize/image"
    pathSave = "/root/paddlejob/data_train/val_resize/image_drawrslt"
    ...
    if cap.isOpened():
        ...
        while (ret):
            ...
            if ret:
                ...
                # print('resized image shpredict_images_maxminape:', image_pre.shape)
                image = np.expand_dims(image_pre, axis=0)
...
if __name__ == '__main__':
    ...
    # 0.25倍expand1-2，初始lr=0.005
    pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/原始模型/20211101105050/20211101193546_epoch7.pdparams"
    pathModel = "/root/paddlejob/workspace/log/modelsave/epoch19.pdparams"
    ...
    #统计最大最小人脸 to do 将最大最小人脸数据上传到afs
    # predict_images_maxmin(net)

    #运行测试图片 to do 将测试集数据上传到afs
    predict_images(net)
