...
def parse_instance(content, outdir):
    ...
    for name, groups in cytoolz.groupby('file_name', merged_info_list).items():
        ...
        for group in groups:
            filenames.append \
                (os.path.join(outdir, re.sub(" ", "_", group['category_id']), os.path.splitext(name)[0] + ".xml"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--anno_file", help="annotation file for object instance/keypoint",
                        default=r'D:\CoCoData\annotations_14_17\instances_train2017.json')
    parser.add_argument("--type", type=str, default='instance', help="object instance or keypoint",
                        choices=['instance', 'keypoint'])
    parser.add_argument("--output", help="output directory for voc annotation xml file",
                        default=r'/Users/frewen/03.ProgramSpace/20.AI/01.WorkSpace/AliceAILearning/AliceYOLO/yolo-v8/datasets/coco')
