...
def dist2bbox(distance, anchor_points, box_format='xywh', dim=-1):
    '''Transform distance(ltrb) to box(xywh or xyxy).'''
    lt, rb = torch.split(distance, 2, dim)
    x1y1 = anchor_points - lt
    x2y2 = anchor_points + rb
    if box_format == 'xyxy':
        bbox = torch.cat([x1y1, x2y2], dim)
...
def check_git_status(repo='ultralytics/yolov5'):
    ...
    if n > 0:
        ...
        s += f"⚠️ YOLO-all is out of date by {n} commit{'s' * (n > 1)}. Use `{pull}` or `git clone {url}` to update."
    else:
...
def check_file(file, suffix=''):
    ...
    if Path(file).is_file() or not file:  # exists
        ...
        files = []
        for d in 'datasets/data', 'models', 'utils':  # search directories
...
def print_mutation(results, hyp, save_dir, bucket, prefix=colorstr('evolve: ')):
    ...
    with open(evolve_yaml, 'w') as f:
        ...
        f.write('# YOLO-all Hyperparameter Evolution Results\n' + f'# Best generation: {i}\n' +
...
def make_anchors(feats, strides, grid_cell_offset=0.5, torch_1_10=check_version(torch.__version__, '1.10.0')):
