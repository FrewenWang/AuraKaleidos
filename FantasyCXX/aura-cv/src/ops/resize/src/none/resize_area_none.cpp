...
static Status ResizeAreaFastNoneImpl(Context *ctx, const Mat &src, Mat &dst, const MI_S32 *area_offset, const MI_S32 *x_offset,
                                     MI_S32 scale_x, MI_S32 scale_y)
{
    using FastSumType = typename ResizeAreaTraits<Tp>::FastSumType;
    if (MI_NULL == area_offset || MI_NULL == x_offset)
    {
...
static Status ResizeAreaNoneHelper(Context *ctx, const Mat &src, Mat &dst)
{
    ...
    {
        ret = ResizeBnNoneImpl<Tp>(ctx, src, dst, MI_TRUE);
        if (ret != Status::OK)
        {
    ...
    {
        MI_S32 area = int_scale_x * int_scale_y;
        MI_S32 src_step = istride / sizeof(Tp);
        MI_S32 *buffer = static_cast<MI_S32*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, (channel * owidth + area) * sizeof(MI_S32), 0));
        if (MI_NULL == buffer)
        {
        ...
        MI_S32 *area_offset = buffer;
        MI_S32 *x_offset = area_offset + area;

        for (MI_S32 sy = 0, k = 0; sy < int_scale_y; sy++)
        {
        ...
        {
            MI_S32 j = dx * channel;
            MI_S32 sx = int_scale_x * j;
            for (MI_S32 k = 0; k < channel; k++)
            {
                x_offset[j + k] = sx + k;
            }
        }
    ...
    else
    {
        ...
        MI_S32 x_tab_size = GetAreaOffset(iwidth, owidth, channel, scale_x, x_tab);
        ...
        ThreadBuffer thread_buffer(ctx, channel * owidth * 2 * sizeof(MI_F32));
...
Status ResizeAreaNone(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            ret = ResizeAreaNoneHelper<MI_U8>(ctx, src, dst);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNoneImpl failed, type: MI_U8");
            }
            break;
        }
        ...
        case ElemType::S8:
        {
            ret = ResizeAreaNoneHelper<MI_S8>(ctx, src, dst);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNoneImpl failed, type: MI_S8");
            }
            break;
        }
        ...
        case ElemType::U16:
        {
            ret = ResizeAreaNoneHelper<MI_U16>(ctx, src, dst);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNoneImpl failed, type: MI_U16");
            }
            break;
        }
        ...
        case ElemType::S16:
        {
            ret = ResizeAreaNoneHelper<MI_S16>(ctx, src, dst);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNoneImpl failed, type: MI_S16");
            }
            break;
        }
        ...
        case ElemType::F32:
        {
            ret = ResizeAreaNoneHelper<MI_F32>(ctx, src, dst);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNoneImpl failed, type: MI_F32");
            }
            break;
        }
