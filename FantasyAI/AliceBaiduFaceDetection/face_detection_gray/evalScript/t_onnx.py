...
def predict_video_onnx(ort_sess):
    ...
    while 1:
        ...
        # image = cv2.imread(pathImg)
        ret, image = cap.read()
        # image = cv2.imread("/media/baidu/YUBIN/测试/55241e0e78f39111ddec43017.jpg")
        if image is None:
            continue
        img_w = image.shape[1]
        img_h = image.shape[0]
        ...
        cv2.imwrite("/home/baidu/Desktop/output-方向盘下移/rslt.jpg", image_disp)
        cv2.imshow('preds', image_disp)
        cv2.waitKey(1)

if __name__ == "__main__":
    onnx_file = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/全量数据/量化模型/v3_a100/FaceDetection0217V4Main.onnx"
