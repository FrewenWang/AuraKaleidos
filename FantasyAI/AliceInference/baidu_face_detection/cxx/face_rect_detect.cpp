...
int main(int, char**) {
    ...
    constexpr int64_t height = 160;
    constexpr int64_t numClasses = 1000;
    constexpr int64_t numInputElements = numChannles * height * width;

    const string imgFile = "/home/frewen/03.ProgramSpace/20.AIStudy/03.Source/onnxruntime_resnet/assets/dog.png";
    ...
    auto outputNodeName = session.GetOutputNameAllocated(0, ort_alloc);
    ...
    try{
        for (size_t i = 0; i < 10; i++)
        {
            ...
            session.Run(Ort::RunOptions{nullptr}, inputNames.data(), &inputTensor, 1, outputNames.data(),  3);
