...
void WarpAffineNormalize::warp_affine_normalize(const vision::VTensor& src, vision::VTensor& dst,
...
void WarpAffineNormalize::warp_affine_normalize_opencv(const vision::VTensor& src, vision::VTensor& dst,
													   const vision::VTensor& M, va_cv::VSize dsize, int flags,
...
//    PERF_TICK(PerfUtil::global(), "warp_affine_normalize_opencv - convert");
    cv::Mat mat_warp_f;
    if (src.dType != FP32) {
        if (src.c == 1) {
            mat_warp.convertTo(mat_warp_f, CV_32FC1);
...
//    PERF_TOCK(PerfUtil::global(), "warp_affine_normalize_opencv - convert");

//    PERF_TICK(PerfUtil::global(), "warp_affine_normalize_opencv - norm");
    cv::Mat mat_dst;
    if (mean.empty() && stddev.empty()) {
        cv::Mat mat_mean;
        cv::Mat mat_stddev;
...
//    PERF_TOCK(PerfUtil::global(), "warp_affine_normalize_opencv - norm");
    dst = vision::TensorConverter::convert_from<cv::Mat>(mat_dst, true);
#endif
}

void WarpAffineNormalize::warp_affine_normalize_naive(const vision::VTensor& src, vision::VTensor& dst,
													  const vision::VTensor& M, va_cv::VSize dsize, int flags,
													  int borderMode, const va_cv::VScalar& borderValue,
													  const vision::VTensor& mean, const vision::VTensor& stddev) {
    // todo:
...
void WarpAffineNormalize::warp_affine_normalize_sse(const vision::VTensor& src, vision::VTensor& dst,
													const vision::VTensor& M, va_cv::VSize dsize, int flags,
													int borderMode, const va_cv::VScalar& borderValue,
													const vision::VTensor& mean, const vision::VTensor& stddev) {
    // todo:
...
void WarpAffineNormalize::warp_affine_normalize_neon(const vision::VTensor& src, vision::VTensor& dst,
													 const vision::VTensor& M, va_cv::VSize dsize, int flags,
													 int borderMode, const va_cv::VScalar& borderValue,
													 const vision::VTensor& mean, const vision::VTensor& stddev) {
    // todo:
