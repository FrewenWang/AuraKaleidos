...
AllocatorDMABufHeap::AllocatorDMABufHeap()
                                         : Allocator(AURA_MEM_DMA_BUF_HEAP, "DMABufHeap"),
                                           m_is_valid(DT_FALSE),
                                           m_dl_handle(DT_NULL),
                                           m_buffer_allocator(DT_NULL),
                                           m_create_func(DT_NULL),
                                           m_free_func(DT_NULL),
                                           m_alloc_func(DT_NULL),
                                           m_map2ion_func(DT_NULL)
{
    m_dl_handle = dlopen("libdmabufheap.so", RTLD_LAZY | RTLD_LOCAL);
    dlerror();     /* Clear any existing error */

    if (m_dl_handle != DT_NULL)
    {
...
DT_BOOL AllocatorDMABufHeap::IsValid() const
{
    return m_is_valid;
}

Buffer AllocatorDMABufHeap::Allocate(DT_S64 size, DT_S32 align)
{
    if (IsValid())
    {
        align = (align <= 0) ? DMA_BUF_HEAP_ADDR_ALIGN : align;
        DT_S32 handle = m_alloc_func(m_buffer_allocator, "system", size, 1, align);
        DT_VOID *host_addr = mmap(DT_NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, handle, 0);
...
DT_VOID AllocatorDMABufHeap::Free(Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
...
Status AllocatorDMABufHeap::Map(const Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
        DT_S32 ret = m_sync_start_func(m_buffer_allocator, buffer.m_property, kSyncReadWrite, DT_NULL, DT_NULL);
...
Status AllocatorDMABufHeap::Unmap(const Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
        DT_S32 ret = m_sync_end_func(m_buffer_allocator, buffer.m_property, kSyncReadWrite, DT_NULL, DT_NULL);
        return (0 == ret) ? Status::OK : Status::ERROR;
    }
    return Status::ERROR;
}
...
AllocatorIonMTK::AllocatorIonMTK()
                                 : Allocator(AURA_MEM_DMA_BUF_HEAP, "Mtk Ion"),
                                   m_is_valid(DT_FALSE), m_dl_handle(DT_NULL),
                                   m_mtk_dl_handle(DT_NULL), m_ion_fd(-1),
                                   m_mt_ion_open_func(DT_NULL), m_ion_alloc_mm_func(DT_NULL),
                                   m_ion_mmap_func(DT_NULL), m_ion_import_func(DT_NULL),
                                   m_ion_munmap_func(DT_NULL), m_ion_share_close_func(DT_NULL),
                                   m_ion_custom_ioctl_func(DT_NULL), m_ion_close_func(DT_NULL),
                                   m_ion_shared_func(DT_NULL), m_ion_free_func(DT_NULL)
{
    DT_BOOL loaded_mt_ion  = DT_FALSE;
    DT_BOOL loaded_libion  = DT_FALSE;

    // load symbol from libion_mtk.so
    m_mtk_dl_handle = dlopen("libion_mtk.so", RTLD_LAZY | RTLD_LOCAL);
    dlerror();     /* Clear any existing error */

    if (m_mtk_dl_handle != DT_NULL)
    {
...
AllocatorIonMTK::~AllocatorIonMTK()
{
    ...
    if (m_mtk_dl_handle != DT_NULL)
    {
        ...
        m_mtk_dl_handle = DT_NULL;
    }
}

DT_BOOL AllocatorIonMTK::IsValid() const
{
    return m_is_valid;
}

Buffer AllocatorIonMTK::Allocate(DT_S64 size, DT_S32 align)
{
    if (IsValid())
    {
        align = (align <= 0) ? DMA_BUF_HEAP_ADDR_ALIGN : align;

        DT_S32 buffer_handle = 0;
        ...
        DT_VOID *host_addr = m_ion_mmap_func(m_ion_fd, DT_NULL, size,
...
DT_VOID AllocatorIonMTK::Free(Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
        DT_BOOL free_success = DT_TRUE;
...
Status AllocatorIonMTK::Map(const Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
        ...
        DT_S32 ret = m_ion_import_func(m_ion_fd, buffer.m_property, &ion_user_handle);
...
AllocatorIonQualcomm::AllocatorIonQualcomm()
                                           : Allocator(AURA_MEM_DMA_BUF_HEAP, "Qcom Ion"),
                                             m_is_valid(DT_FALSE), m_dl_handle(DT_NULL),
                                             m_ion_fd(-1), m_ion_open_func(DT_NULL),
                                             m_ion_close_func(DT_NULL), m_ion_alloc_func(DT_NULL)
{
    m_dl_handle = dlopen("libion.so", RTLD_LAZY | RTLD_LOCAL);
    dlerror();     /* Clear any existing error */
...
DT_BOOL AllocatorIonQualcomm::IsValid() const
{
    return m_is_valid;
}

Buffer AllocatorIonQualcomm::Allocate(DT_S64 size, DT_S32 align)
{
    if (IsValid())
    {
        DT_S32 handle = 0;
...
DT_VOID AllocatorIonQualcomm::Free(Buffer &buffer)
{
    if (AURA_MEM_DMA_BUF_HEAP == buffer.m_type && buffer.m_origin != DT_NULL)
    {
        munmap(buffer.m_origin, buffer.m_capacity);
