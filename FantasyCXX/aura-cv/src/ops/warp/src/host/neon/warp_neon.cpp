...
template <typename Tp, typename std::enable_if<(sizeof(DT_S16) == sizeof(Tp))>::type * = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID VectorStore(float32x4x2_t v2qf32_map_xy, int16x4x2_t v2ds16_map_xy, Tp *map_row)
{
    v2ds16_map_xy.val[0] = neon::vqmovn(neon::vcvt<DT_S32>(neon::vrndn(v2qf32_map_xy.val[0])));
    v2ds16_map_xy.val[1] = neon::vqmovn(neon::vcvt<DT_S32>(neon::vrndn(v2qf32_map_xy.val[1])));
    neon::vstore(map_row, v2ds16_map_xy);
}

template <typename Tp, typename std::enable_if<(sizeof(DT_F32) == sizeof(Tp))>::type * = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID VectorStore(float32x4x2_t v2qf32_map_xy, int16x4x2_t v2ds16_map_xy, Tp *map_row)
{
...
template <typename Tp, WarpType WARP_TYPE, typename std::enable_if<WarpType::AFFINE == WARP_TYPE>::type * = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID WarpBlockCore(DT_F32 *map_x_row, DT_F32 *map_y_row, Mat &map, DT_S32 bh, DT_S32 bw)
{
    float32x4_t   vqf32_x_y, vqf32_y_y;
    float32x4x2_t v2qf32_map_xy;
    int16x4x2_t   v2ds16_map_xy;

    DT_S32 bw_align = bw & (-4);

    for (DT_S32 y1 = 0; y1 < bh; y1++)
    {
        ...
        // middle
        for (DT_S32 x1 = 0; x1 < bw_align; x1 += 4)
        {
        ...
        // right border
        for (DT_S32 x1 = bw_align; x1 < bw; x1++)
        {
...
template <typename Tp, WarpType WARP_TYPE, typename std::enable_if<WarpType::PERSPECTIVE == WARP_TYPE>::type * = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID WarpBlockCore(DT_F32 *map_x_row, DT_F32 *map_y_row, Mat &map, DT_S32 bh, DT_S32 bw)
{
    ...
    for (DT_S32 y1 = 0; y1 < bh; y1++)
    {
        ...
        for (DT_S32 x1 = bw_align; x1 < bw; x1++)
        {
            DT_F32 w = map_x_row[x1 * 3 + 2] + map_y_row[y1 * 3 + 2];
            w        = NearlyEqual(w, (DT_F32)0.f) ? 0 : 1. / w;
                            ...
                            DT_S32 start_row, DT_S32 end_row)
{
    ...
    DT_S32 bh0   = Min(BLOCK_SZ / 2, end_row - start_row);
    DT_S32 bw0   = Min(BLOCK_SZ * BLOCK_SZ / bh0, width);
    bh0          = Min(BLOCK_SZ * BLOCK_SZ / bw0, end_row - start_row);

    DT_S32  xy_channel = map_x.GetSizes().m_channel;
    ...
    for (DT_S32 y = start_row; y < end_row; y += bh0)
    {
        for (DT_S32 x = 0; x < width; x += bw0)
        {
            DT_S32 bh = Min(bh0, end_row - y);
            DT_S32 bw = Min(bw0, width - x);
...
Status WarpNeon::Initialize()
{
    ...
    DT_S32 channel = (WarpType::AFFINE == m_warp_type) ? 2 : 3;
...
Status WarpNeon::Run()
{
    ...
    DT_S32 elem_size  = (m_interp_type == InterpType::NEAREST) ? sizeof(DT_S16) : sizeof(DT_F32);
    ...
    Status ret     = Status::ERROR;
    DT_S32 pattern = AURA_MAKE_PATTERN(elem_size, m_warp_type);

    switch (pattern)
    {
        case AURA_MAKE_PATTERN(sizeof(DT_S16), WarpType::AFFINE):
        {
            ret = wp->ParallelFor(0, height, WarpNeonBlock<DT_S16, WarpType::AFFINE>, m_ctx, std::cref(*src), std::ref(*dst), std::ref(m_map_x), std::ref(m_map_y),
                                  m_interp_type, m_border_type, std::cref(m_border_value), std::ref(share_remap), std::ref(thread_buffer));
            break;
        }

        case AURA_MAKE_PATTERN(sizeof(DT_F32), WarpType::AFFINE):
        {
            ret = wp->ParallelFor(0, height, WarpNeonBlock<DT_F32, WarpType::AFFINE>, m_ctx, std::cref(*src), std::ref(*dst), std::ref(m_map_x), std::ref(m_map_y),
                                  m_interp_type, m_border_type, std::cref(m_border_value), std::ref(share_remap), std::ref(thread_buffer));
            break;
        }

        case AURA_MAKE_PATTERN(sizeof(DT_S16), WarpType::PERSPECTIVE):
        {
            ret = wp->ParallelFor(0, height, WarpNeonBlock<DT_S16, WarpType::PERSPECTIVE>, m_ctx, std::cref(*src), std::ref(*dst), std::ref(m_map_x), std::ref(m_map_y),
                                  m_interp_type, m_border_type, std::cref(m_border_value), std::ref(share_remap), std::ref(thread_buffer));
            break;
        }

        case AURA_MAKE_PATTERN(sizeof(DT_F32), WarpType::PERSPECTIVE):
        {
            ret = wp->ParallelFor(0, height, WarpNeonBlock<DT_F32, WarpType::PERSPECTIVE>, m_ctx, std::cref(*src), std::ref(*dst), std::ref(m_map_x), std::ref(m_map_y),
                                  m_interp_type, m_border_type, std::cref(m_border_value), std::ref(share_remap), std::ref(thread_buffer));
            break;
        }
