...
struct CustomType
{
    ...
    Sequence<DT_U8>                seq;
    KeyPoint_<DT_F32>              keypoint;
    Point2_<DT_S16>                point2;
    Point3_<DT_U32>                point3;
    Rect_<DT_S32>                  rect;
    Scalar_<DT_F32>                scalar;
    Sizes2_<DT_F64>                sizes2;
    Sizes3_<DT_U64>                sizes3;
    std::string                    str;
    std::vector<KeyPoint_<DT_F32>> keypoint_array;
};

template <typename Tp, typename std::enable_if<std::is_same<Tp, CustomType>::value>::type* = DT_NULL>
...
static DT_BOOL operator!=(const KeyPoint_<DT_F32> &a, const KeyPoint_<DT_F32> &b)
{
...
NEW_TESTCASE(runtime_hexagon_hexagon_rpc_param_test)
{
    ...
    DT_U8 data_in[]{1, 2, 3, 4, 5, 6, 7, 8, 9}, data_out[10];

    param_in.base           = 1;
    param_in.level          = HexagonPowerLevel::LOW;
    param_in.seq            = {data_in, 10};
    param_in.keypoint       = KeyPoint_<DT_F32>(1, 1, 0.5f);
    param_in.point2         = Point2_<DT_S16>(1, 2);
    param_in.point3         = Point3_<DT_U32>(1, 2, 3);
    param_in.rect           = Rect_<DT_S32>(1, 2, 3, 4);
    param_in.scalar         = Scalar_<DT_F32>(1, 2, 3, 4);
    param_in.sizes2         = Sizes2_<DT_F64>(1, 2);
    param_in.sizes3         = Sizes3_<DT_U64>(1, 2, 3);
    ...
    param.Set(param_in);
    param.Get(param_out, DT_TRUE);

    Status ret = Status::OK;

    ret |= AURA_CHECK_EQ(ctx, param_in.base,      param_out.base,      "check DT_S32 failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.level,     param_out.level,     "check HexagonPowerLevel failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.seq.len,   param_out.seq.len,   "check Sequence<DT_U8> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.point2,    param_out.point2,    "check Point2_<DT_S16> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.point3,    param_out.point3,    "check Point3_<DT_U32> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.rect,      param_out.rect,      "check Rect_<DT_S32> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.scalar,    param_out.scalar,    "check Scalar_<DT_F32> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.sizes2,    param_out.sizes2,    "check Sizes2_<DT_F64> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.sizes3,    param_out.sizes3,    "check Sizes3_<DT_U64> failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.str,       param_out.str,       "check std::string failed\n");
    ret |= AURA_CHECK_EQ(ctx, param_in.keypoint_array.size(), param_out.keypoint_array.size(), "check Sequence<KeyPoint_<DT_F32>> failed\n");
    for (DT_S32 i = 0; i < param_in.seq.len; i++)
    {
        ret |= AURA_CHECK_EQ(ctx, param_in.seq.data[i], param_out.seq.data[i], "check std::Sequence<DT_U8> failed\n");
    }
    if (param_in.keypoint != param_out.keypoint)
    {
        AURA_LOGE(ctx, AURA_TAG, "check KeyPoint_<DT_F32> failed\n");
        ret = Status::ERROR;
    }
    for (size_t i = 0; i < param_in.keypoint_array.size(); i++)
    {
        ...
        {
            AURA_LOGE(ctx, AURA_TAG, "std::vector<KeyPoint_<DT_F32>*>\n");
            ret = Status::ERROR;
        }
    }
