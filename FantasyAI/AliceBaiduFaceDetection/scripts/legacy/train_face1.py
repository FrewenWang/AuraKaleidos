...
if __name__ == "__main__":
    ...
    if use_cloud:
        ...
        images_root = '/root/paddlejob/data_tar'
        ...
        train_label_file = ['/root/paddlejob/data_tar/filelist/train.txt']
        eval_label_file = ['/root/paddlejob/data_tar/filelist/val.txt']
        eval_label_file_prune = ['/root/paddlejob/data_tar/filelist/val200.txt']
        model_save_path = './afs_aicv/leisheng526/vehicle/models/' + model_name_prefix
        # pretrain_weight = './afs/leisheng526/vehicle/pretrain_models/model_final-tiny'
        pretrain_weight = './afs_aicv/filelist/20211102214616_epoch4_1000'
        resume_weight = ''
    else:
        images_root = '/media/baidu/3.6TB_SSD/facedetect/缩小尺寸'
        train_label_file = ['/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/filelist/train.txt']
        eval_label_file_prune = ['/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/filelist/val200.txt']
        eval_label_file = ['/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/filelist/val.txt']

        model_save_path = '/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/剪枝模型/v2/' + model_name_prefix
    ...
    eval_dataset = FaceDataSet(image_dir=images_root, anno_path=eval_label_file,
                               tags_map=zip(tag_maps, tag_name), use_cloud=use_cloud)
    # path_backImg = "/media/baidu/3.6TB_SSD/val_data/back_from_ImgNet"
    # mix_back_imgnet1 = os.listdir(path_backImg)
    mix_back_imgnet2 = []
    # for filet in mix_back_imgnet1:
    #     mix_back_imgnet2.append(os.path.join(path_backImg, filet))
    pathInfos = open(train_label_file[0]).readlines()
    # pathNeg = "/media/baidu/ssd2/ppyolo/opendata/coco_train2014_noHand"
    pathNeg = "/root/paddlejob/data_tar/back_from_ImgNet"
    allNegs1 = os.listdir(pathNeg)
    allNegFiles = []
    for filet in allNegs1:
        allNegFiles.append(os.path.join(pathNeg, filet))
    sample_transforms = [
        {'Decode': {}},
        {'Mixup_MergeNewFace': {"pathInfos": pathInfos, "pathRoot": images_root, "prob": 0.2, "pathNegs": allNegFiles}},
        # {'Mixup_Imgnet':{"back_imgs":mix_back_imgnet2, "prob":0.2}},
        {"RandomRotate": {"prob": 0.5}},
        {"RandomBlurNoise": {"prob": 0.5}},
        {'RandomDistort': {}},
        {'RandomExpand': {'fill_value': [123.675, 116.28, 103.53], 'ratio': 2}},
        # {'RandomCrop': {}},
    ...
    eval_loader_prune(eval_dataset_prune, worker_num=8)
    ...
    lr = paddle.optimizer.lr.MultiStepDecay(learning_rate=0.01, milestones=[10, 15, 20], gamma=0.1)
    ...
    if usePruner:
        ...
        pruner.sensitive(
            sen_file="/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/剪枝文件/L1Norm_sen_yb_sense60.pickle")  # "/root/paddlejob/data_tar/filelist/L1Norm_sen_yb_sense3.pickle"
