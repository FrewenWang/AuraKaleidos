...
int main(int argc, char** argv)
{
    ...
    matrix_c_desc.image_row_pitch = wrapper.get_image_row_pitch(matrix_c_format, matrix_c_desc);

    cl_mem_dmabuf_host_ptr matrix_c_buf = wrapper.make_buffer_for_nonplanar_image(matrix_c_format,
                                                                                       matrix_c_desc);
    ...
    err = clEnqueueUnmapMemObject(command_queue, matrix_c_mem, image_ptr, 0, nullptr, &unmap_event);
    if (err != CL_SUCCESS)
    {
        std::cerr << "Error " << err << " unmapping matrix C image" << "\n";
        return err;
    }
