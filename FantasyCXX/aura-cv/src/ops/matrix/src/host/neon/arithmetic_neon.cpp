...
template <typename Tp, ArithmNeonOpType Type, typename VqType = typename neon::QVector<Tp>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VADD == Type)>::type* = MI_NULL>
...
template <typename Tp, ArithmNeonOpType Type, typename VdType = typename neon::DVector<Tp>::VType,
          ...
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VADDL == Type)>::type* = MI_NULL>
...
template <typename Tp, ArithmNeonOpType Type, typename VqType = typename neon::QVector<Tp>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VQADD == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VqType vq_src0, VqType vq_src1)
{
...
template <typename Tp, ArithmNeonOpType Type, typename VqType = typename neon::QVector<Tp>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VSUB == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VqType vq_src0, VqType vq_src1)
{
...
template <typename Tp, ArithmNeonOpType Type, typename VdType = typename neon::DVector<Tp>::VType,
          typename VqType = typename neon::WVectorBits<VdType>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VSUBL == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VdType vq_src0, VdType vq_src1)
{
...
template <typename Tp, ArithmNeonOpType Type, typename VqType = typename neon::QVector<Tp>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VQSUB == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VqType vq_src0, VqType vq_src1)
{
...
template <typename Tp, ArithmNeonOpType Type, typename VqType = typename neon::QVector<Tp>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VMUL == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VqType vq_src0, VqType vq_src1)
{
...
template <typename Tp, ArithmNeonOpType Type, typename VdType = typename neon::DVector<Tp>::VType,
          typename VqType = typename neon::WVectorBits<VdType>::VType,
          typename std::enable_if<(ArithmNeonOpType::NEON_OP_VMULL == Type)>::type* = MI_NULL>
AURA_ALWAYS_INLINE VqType NeonOpFunc(VdType vq_src0, VdType vq_src1)
{
...
static Status ArithmNeonImpl(const Mat &src0, const Mat &src1, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
...
template <typename Tp, ArithmNeonOpType Type>
static Status ArithmPromoteNeonImpl(const Mat &src0, const Mat &src1, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
...
static Status ArithmUnsignedSubPromoteNeonImpl(const Mat &src0, const Mat &src1, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    ...
    constexpr MI_S32 ELEM_COUNTS = 16 / sizeof(St);
...
static Status ArithmIntegerMulNeonImpl(const Mat &src0, const Mat &src1, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    using VqType    = typename neon::QVector<Tp>::VType;
    using VdType    = typename neon::DVector<Tp>::VType;
    using VqSumType = typename neon::WVectorBits<VdType>::VType;
...
template <typename Tp>
static Status ArithmFloatDivNeonImpl(const Mat &src0, const Mat &src1, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    using VqType = typename neon::QVector<Tp>::VType;
    constexpr MI_S32 ELEM_COUNTS = 16 / sizeof(Tp);
...
static Status ArithmAddNeonHelper(Context *ctx, const Mat &src0, const Mat &src1, Mat &dst)
{
    ...
    MI_S32 pattern = AURA_MAKE_PATTERN(src0.GetElemType(), dst.GetElemType());

    switch (pattern)
    {
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::U8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U8, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S8, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U16, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S16, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U32, ElemType::U32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U32, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S32, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S32, ArithmNeonOpType::NEON_OP_VQADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::U16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_U8, ArithmNeonOpType::NEON_OP_VADDL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S8, ArithmNeonOpType::NEON_OP_VADDL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_U16, ArithmNeonOpType::NEON_OP_VADDL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S16, ArithmNeonOpType::NEON_OP_VADDL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        ...
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F16, ArithmNeonOpType::NEON_OP_VADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        ...
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F32, ArithmNeonOpType::NEON_OP_VADD>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
...
static Status ArithmSubNeonHelper(Context *ctx, const Mat &src0, const Mat &src1, Mat &dst)
{
    ...
    switch (pattern)
    {
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::U8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U8, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S8, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U16, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S16, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U32, ElemType::U32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_U32, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S32, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_S32, ArithmNeonOpType::NEON_OP_VQSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmUnsignedSubPromoteNeonImpl<MI_U8, MI_S16>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S8, ArithmNeonOpType::NEON_OP_VSUBL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmUnsignedSubPromoteNeonImpl<MI_U16, MI_S32>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S16, ArithmNeonOpType::NEON_OP_VSUBL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
#if defined(AURA_ENABLE_NEON_FP16)
        case AURA_MAKE_PATTERN(ElemType::F16, ElemType::F16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F16, ArithmNeonOpType::NEON_OP_VSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
#endif // AURA_ENABLE_NEON_FP16
        case AURA_MAKE_PATTERN(ElemType::F32, ElemType::F32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F32, ArithmNeonOpType::NEON_OP_VSUB>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
...
static Status ArithmMulNeonHelper(Context *ctx, const Mat &src0, const Mat &src1, Mat &dst)
{
    ...
    switch (pattern)
    {
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::U8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_U8>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S8):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_S8>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_U16>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_S16>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U32, ElemType::U32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_U32>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S32, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmIntegerMulNeonImpl<MI_S32>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U8, ElemType::U16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_U8, ArithmNeonOpType::NEON_OP_VMULL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S8, ElemType::S16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S8, ArithmNeonOpType::NEON_OP_VMULL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_U16, ArithmNeonOpType::NEON_OP_VMULL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmPromoteNeonImpl<MI_S16, ArithmNeonOpType::NEON_OP_VMULL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
#if defined(AURA_ENABLE_NEON_FP16)
        case AURA_MAKE_PATTERN(ElemType::F16, ElemType::F16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F16, ArithmNeonOpType::NEON_OP_VMUL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
#endif // AURA_ENABLE_NEON_FP16
        case AURA_MAKE_PATTERN(ElemType::F32, ElemType::F32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmNeonImpl<MI_F32, ArithmNeonOpType::NEON_OP_VMUL>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
...
static Status ArithmDivNeonHelper(Context *ctx, const Mat &src0, const Mat &src1, Mat &dst)
{
    ...
    switch (pattern)
    {
        ...
        case AURA_MAKE_PATTERN(ElemType::F16, ElemType::F16):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmFloatDivNeonImpl<MI_F16>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
#endif // AURA_ENABLE_NEON_FP16
        case AURA_MAKE_PATTERN(ElemType::F32, ElemType::F32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ArithmFloatDivNeonImpl<MI_F32>,
                                  std::cref(src0), std::cref(src1), std::ref(dst));
            break;
        }
...
template <typename Tp>
static Status ScalarDivideMatNeonImpl(MI_F32 scalar, const Mat &src, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    using VqType = typename neon::QVector<Tp>::VType;
    constexpr MI_S32 ELEM_COUNTS = 16 / sizeof(Tp);
...
Status ScalarDivideNeon::SetArgs(MI_F32 scalar, const Array *src, Array *dst)
{
...
Status ScalarDivideNeon::Run()
{
    ...
    switch (pattern)
    {
        case AURA_MAKE_PATTERN(ElemType::F32, ElemType::F32):
        {
            ret = wp->ParallelFor(static_cast<MI_S32>(0), height, ScalarDivideMatNeonImpl<MI_F32>,
