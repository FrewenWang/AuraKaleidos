...
AURA_ALWAYS_INLINE DT_VOID ButterflyBlockNeon(DT_F32 *x0_ptr, DT_F32 *x1_ptr, const DT_F32 *w_ptr)
{
                               ...
                               const std::complex<DT_F32> *dft_exp_table)
{
    ...
    DT_S32 size = start_level;

    if (2 == size)
    {
        for (DT_S32 i = 0; i < n; i += 2)
        {
            DT_F32 *x0_ptr = reinterpret_cast<DT_F32*>(src + i);
            DT_F32 *x1_ptr = reinterpret_cast<DT_F32*>(src + i + 1);
    ...
    if (4 == size)
    {
        DT_S32 table_step = n / 4;
        DT_F32 w[4] = {dft_exp_table[0].real(), dft_exp_table[0].imag(),
                       dft_exp_table[table_step].real(), dft_exp_table[table_step].imag()};

        for (DT_S32 i = 0; i < n; i += 4) // i for group start index
        {
            DT_F32 *x0_ptr = reinterpret_cast<DT_F32*>(src + i);
            DT_F32 *x1_ptr = reinterpret_cast<DT_F32*>(src + i + 2);
            ButterflyBlockNeon<float32x2_t, float32x2x2_t>(x0_ptr, x1_ptr, w);
        }
        size *= 2;
    }

    std::complex<DT_F32> w[4] = {{0.0f, 0.0f}};
    for (; size < n; size *= 2)
    {
        DT_S32 half_size  = size / 2;
        DT_S32 table_step = n / size;

        for (DT_S32 i = 0; i < n; i += size) // i for group start index
        {
            for (DT_S32 j = 0; j < half_size; j += 4)
            {
                DT_F32 *x0_ptr = reinterpret_cast<DT_F32*>(src + i + j);
                DT_F32 *x1_ptr = reinterpret_cast<DT_F32*>(src + i + half_size + j);
                DT_F32 *w_ptr  = reinterpret_cast<DT_F32*>(w);
                DT_S32 exp_idx = j * table_step;

                for (DT_S32 k = 0; k < 4; ++k)
                {
    ...
    // layer n
    {
        DT_S32 half_size  = n / 2;
        for (DT_S32 i = 0; i < half_size; i += 4)
        {
            DT_F32 *x0_ptr = reinterpret_cast<DT_F32*>(src + i);
            DT_F32 *x1_ptr = reinterpret_cast<DT_F32*>(src + half_size + i);
            const DT_F32 *w_ptr = reinterpret_cast<const DT_F32*>(&dft_exp_table[i]);
...
static DT_VOID FFTRadix2Neon(std::complex<DT_F32> *src, DT_S32 n,
                             const std::complex<DT_F32> *exp_table,
                             const DT_U16 *idx_table, DT_BOOL with_scale)
{
    if (!IsPowOf2(n))
    {
        return;
...
Status DftRadix2RowNeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    ...
    DT_S32 buffer_sz = half_w * sizeof(std::complex<DT_F32>);
    ThreadBuffer thread_buffer(ctx, buffer_sz);

    DT_S32 table_sz = half_w * sizeof(DT_U16) + ((half_w + width) / 2) * sizeof(std::complex<DT_F32>);
    DT_VOID *temp_buffer = AURA_ALLOC(ctx, table_sz);
    if (DT_NULL == temp_buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
        return Status::ERROR;
    }

    DT_U16 *idx_table = reinterpret_cast<DT_U16*>(temp_buffer);
    std::complex<DT_F32> *dft_row_exp_table  = reinterpret_cast<std::complex<DT_F32>*>(idx_table + half_w);
    std::complex<DT_F32> *row_real_exp_table = dft_row_exp_table + half_w / 2;
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        ...
        std::complex<DT_F32> *buffer_ptr = thread_buffer.GetThreadData<std::complex<DT_F32>>();
        ...
        for (DT_S32 y = start; y < end; ++y)
        {
            ...
            std::complex<DT_F32> *dst_complex = dst.Ptr<std::complex<DT_F32>>(y);

            for (DT_S32 x = 0; x < half_w; x++)
            {
                DT_S32 idx = idx_table[x];
                buffer_ptr[x].real(SaturateCast<DT_F32>(src_row[2 * idx]));
                buffer_ptr[x].imag(SaturateCast<DT_F32>(src_row[2 * idx + 1]));
            }

            ButterflyTransformNeon(buffer_ptr, 2, half_w, DT_FALSE, dft_row_exp_table);

            for (DT_S32 x = 1; x < half_w; x++)
            {
                std::complex<DT_F32> yk = buffer_ptr[x];
                std::complex<DT_F32> yk_conj = std::conj(buffer_ptr[half_w - x]);
                ...
                std::complex<DT_F32> result = fk + row_real_exp_table[x] * gk;
                dst_complex[x] = result;
                dst_complex[width - x] = std::conj(result);
            }
            {
                std::complex<DT_F32> y0 = buffer_ptr[0];
...
template <>
Status DftRadix2RowNeonImpl<DT_F32, 1>(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
    Sizes3 sz = src.GetSizes();
    DT_S32 width  = sz.m_width;
    ...
    if (1 == width)
    {
        DftRadix2RowProcCol1None<DT_F32, 1>(src, dst);
        return Status::OK;
    }
    if (2 == width)
    {
        DftRadix2RowProcCol2None<DT_F32, 1>(src, dst);
        return Status::OK;
    }
    ...
    DT_S32 table_sz = width * sizeof(DT_U16) + width / 2 * sizeof(std::complex<DT_F32>);
    DT_VOID *temp_buffer = AURA_ALLOC(ctx, table_sz);
    if (DT_NULL == temp_buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
    ...
    DT_U16 *idx_table = reinterpret_cast<DT_U16*>(temp_buffer);
    std::complex<DT_F32> *dft_row_exp_table = reinterpret_cast<std::complex<DT_F32>*>(idx_table + width);
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        for (DT_S32 y = start; y < end; ++y)
        {
            const std::complex<DT_F32> *src_complex = src.Ptr<std::complex<DT_F32>>(y);
            std::complex<DT_F32> *dst_complex = dst.Ptr<std::complex<DT_F32>>(y);
...
static Status DftRadix2RowNeon(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            ret = DftRadix2RowNeonImpl<DT_U8, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::S8:
        {
            ret = DftRadix2RowNeonImpl<DT_S8, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::U16:
        {
            ret = DftRadix2RowNeonImpl<DT_U16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::S16:
        {
            ret = DftRadix2RowNeonImpl<DT_S16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
#if defined(AURA_ENABLE_NEON_FP16)
        case ElemType::F16:
        {
            ret = DftRadix2RowNeonImpl<MI_F16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
#endif // AURA_ENABLE_NEON_FP16
        case ElemType::F32:
        {
            ret = DftRadix2RowNeonImpl<DT_F32, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        default:
        {
...
template <typename Tp, DT_BOOL IS_DST_C1>
AURA_INLINE DT_VOID DftRadix2Col8Neon(Mat &src, Mat &dst, DT_U16 *idx_table, std::complex<DT_F32> *exp_table,
                                      std::complex<DT_F32> *buffer_data, DT_BOOL with_scale, DT_S32 x)
{
    DT_S32 height = src.GetSizes().m_height;
    DT_S32 row_pitch = src.GetRowPitch();

    struct ComplexPacket
    {
        std::complex<DT_F32> data[8];
    };

    // Init buffer
    std::complex<DT_F32> *buffer_row0 = buffer_data + 0 * height;
    std::complex<DT_F32> *buffer_row1 = buffer_data + 1 * height;
    std::complex<DT_F32> *buffer_row2 = buffer_data + 2 * height;
    std::complex<DT_F32> *buffer_row3 = buffer_data + 3 * height;
    std::complex<DT_F32> *buffer_row4 = buffer_data + 4 * height;
    std::complex<DT_F32> *buffer_row5 = buffer_data + 5 * height;
    std::complex<DT_F32> *buffer_row6 = buffer_data + 6 * height;
    std::complex<DT_F32> *buffer_row7 = buffer_data + 7 * height;

    // move data into temp buffer
    DT_U8 *src_row = reinterpret_cast<DT_U8*>(src.GetData()) + x * sizeof(std::complex<DT_F32>);
...
template <typename Tp, DT_BOOL IS_DST_C1>
AURA_INLINE DT_VOID DftRadix2Col1Neon(Mat &src, Mat &dst, DT_U16 *idx_table, std::complex<DT_F32> *exp_table,
                                      std::complex<DT_F32> *buffer_data, DT_BOOL with_scale, DT_S32 x)
{
    DT_S32 height = src.GetSizes().m_height;
    DT_S32 row_pitch = src.GetRowPitch();

    DT_U8 *src_row = reinterpret_cast<DT_U8*>(src.GetData());
    for (DT_S32 i = 0; i < height; ++i)
    {
        std::complex<DT_F32> *dst_complex = reinterpret_cast<std::complex<DT_F32>*>(src_row);
        DT_S32 idx  = idx_table[i];
...
template <typename Tp, DT_BOOL IS_DST_C1>
Status DftRadix2ColCommNeon(Context *ctx, Mat &src, Mat &dst, ThreadBuffer &thread_buffer, DT_U16 *idx_table,
                            std::complex<DT_F32> *exp_table, DT_BOOL with_scale, DT_S32 start, DT_S32 end)
{
    std::complex<DT_F32> *buffer_ptr = thread_buffer.GetThreadData<std::complex<DT_F32>>();
...
template <typename Tp, DT_U8 IS_INVERSE, DT_BOOL IS_DST_C1>
static DT_VOID DftRadix2ColProcRow2Neon(Mat &src, Mat &dst)
{
    std::complex<DT_F32> *src_c  = src.Ptr<std::complex<DT_F32>>(0);
    std::complex<DT_F32> *src_n0 = src.Ptr<std::complex<DT_F32>>(1);
    ...
    const  DT_F32 coef  = IS_INVERSE ? 0.5f : 1.0f;
    ...
    if (IS_DST_C1)
    {
        ...
        {
            float32x4x2_t v2qf32_c_l  = neon::vload2q((DT_F32 *)src_c + x * 2);
            float32x4x2_t v2qf32_c_r  = neon::vload2q((DT_F32 *)src_c + x * 2 + 8);
            float32x4x2_t v2qf32_n0_l = neon::vload2q((DT_F32 *)src_n0 + x * 2);
            float32x4x2_t v2qf32_n0_r = neon::vload2q((DT_F32 *)src_n0 + x * 2 + 8);
        ...
        for (; x < width; x++)
        {
            std::complex<DT_F32> val_c  = src_c[x];
            std::complex<DT_F32> val_n0 = src_n0[x];
    ...
    else
    {
        ...
        {
            float32x4_t vqf32_row_c_l  = neon::vload1q((DT_F32 *)src_c + x * 2);
            float32x4_t vqf32_row_c_r  = neon::vload1q((DT_F32 *)src_c + x * 2 + 4);
            float32x4_t vqf32_row_n0_l = neon::vload1q((DT_F32 *)src_n0 + x * 2);
            float32x4_t vqf32_row_n0_r = neon::vload1q((DT_F32 *)src_n0 + x * 2 + 4);
        ...
        for (; x < width; x++)
        {
            ...
            std::complex<DT_F32> result_c  = (val_c + val_n0) * coef;
            std::complex<DT_F32> result_n0 = (val_c - val_n0) * coef;
...
template <typename Tp, DT_U8 IS_INVERSE, DT_BOOL IS_DST_C1>
static Status DftRadix2ColNeonImpl(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale)
{
    Sizes3 sz = src.GetSizes();
    DT_S32 width  = sz.m_width;
    DT_S32 height = sz.m_height;
    ...
    DT_S32 w_eff = (width >= 8) ? 8 : 1;
    DT_S32 buffer_sz = height * w_eff * sizeof(std::complex<DT_F32>);
    ThreadBuffer thread_buffer(ctx, buffer_sz);

    DT_S32 table_sz = height * sizeof(DT_U16) + height / 2 * sizeof(std::complex<DT_F32>);
    DT_VOID *temp_buffer = AURA_ALLOC(ctx, table_sz);
    if (DT_NULL == temp_buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
    ...
    DT_U16 *idx_table = reinterpret_cast<DT_U16*>(temp_buffer);
    std::complex<DT_F32> *exp_table = reinterpret_cast<std::complex<DT_F32>*>(idx_table + height);
    ...
    if (!IS_INVERSE)
    {
        ...
        if (0 == width % 2)
        {
            if (0 == height % 2)
            {
                src.At<DT_F32>(0, 0, 1) = 0.0f;
                src.At<DT_F32>(0, width / 2, 1) = 0.0f;
                src.At<DT_F32>(height / 2, 0, 1) = 0.0f;
                src.At<DT_F32>(height / 2, width / 2, 1) = 0.0f;
            }
            else
            {
                src.At<DT_F32>(0, 0, 1) = 0.0f;
...
static Status IDftRadix2ColNeonHelper(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale = DT_FALSE)
{
    Status ret = Status::ERROR;

    DT_BOOL is_dst_c1 = (1 == dst.GetSizes().m_channel);
    if (is_dst_c1)
    {
        ret = DftRadix2ColNeonImpl<Tp, 1, DT_TRUE>(ctx, src, dst, with_scale);
    }
    else
    {
        ret = DftRadix2ColNeonImpl<Tp, 1, DT_FALSE>(ctx, src, dst, with_scale);
    }
...
static Status IDftRadix2ColNeonHelper(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale = DT_FALSE)
{
    ...
    switch (elem_type)
    {
        case ElemType::U8:
        {
            ret = IDftRadix2ColNeonHelper<DT_U8>(ctx, src, dst, with_scale);
            break;
        }
        case ElemType::S8:
        {
            ret = IDftRadix2ColNeonHelper<DT_S8>(ctx, src, dst, with_scale);
            break;
        }
        case ElemType::U16:
        {
            ret = IDftRadix2ColNeonHelper<DT_U16>(ctx, src, dst, with_scale);
            break;
        }
        case ElemType::S16:
        {
            ret = IDftRadix2ColNeonHelper<DT_S16>(ctx, src, dst, with_scale);
            break;
        }
        case ElemType::U32:
        {
            ret = IDftRadix2ColNeonHelper<DT_U32>(ctx, src, dst, with_scale);
            break;
        }
        case ElemType::S32:
        {
            ret = IDftRadix2ColNeonHelper<DT_S32>(ctx, src, dst, with_scale);
            break;
        }
# if defined(AURA_ENABLE_NEON_FP16)
        case ElemType::F16:
...
# endif // AURA_ENABLE_NEON_FP16
        case ElemType::F32:
        {
            ret = IDftRadix2ColNeonHelper<DT_F32>(ctx, src, dst, with_scale);
            break;
        }
        default:
        {
...
template <typename Tp, DT_U8 IS_INVERSE>
static Status DftBluesteinRowNeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
    WorkerPool *wp = ctx->GetWorkerPool();
    if (DT_NULL == wp)
    ...
    DT_S32 width_padding = 1;
    while (width_padding / 2 <= width)
    {
        width_padding *= 2;
    }

    DT_S32 buffer_sz = width_padding * sizeof(std::complex<DT_F32>);
    ThreadBuffer thread_buffer(ctx, buffer_sz);

    DT_S32 table_sz = width_padding * sizeof(DT_U16) + (2 * width_padding + width) * sizeof(std::complex<DT_F32>);
    DT_VOID *temp_buffer = AURA_ALLOC(ctx, table_sz);
    if (DT_NULL == temp_buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
    ...
    DT_U16 *idx_table                    = reinterpret_cast<DT_U16*>(temp_buffer);
    std::complex<DT_F32> *b_hor          = reinterpret_cast<std::complex<DT_F32>*>(idx_table + width_padding);
    std::complex<DT_F32> *table_hor      = b_hor + width_padding;
    std::complex<DT_F32> *dft_exp_table  = table_hor + width;
    std::complex<DT_F32> *idft_exp_table = dft_exp_table + width_padding / 2;
    ...
    FFTRadix2Neon(b_hor, width_padding, dft_exp_table, idx_table, DT_FALSE);

    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        std::complex<DT_F32> *a_hor = thread_buffer.GetThreadData<std::complex<DT_F32>>();
        ...
        for (DT_S32 y = start; y < end; y++)
        {
            ...
            if (IS_INVERSE)
            {
                ...
                for (DT_S32 x = 0; x < width; x++)
                {
                    a_hor[x].real(SaturateCast<DT_F32>(src_row[2 * x + 0]));
                    a_hor[x].imag(SaturateCast<DT_F32>(src_row[2 * x + 1]));
                    a_hor[x] *= table_hor[x];
                }
            }
            else
            {
                ...
                for (DT_S32 x = 0; x < width; x++)
                {
                    a_hor[x].real(SaturateCast<DT_F32>(src_row[x]));
                    a_hor[x].imag(0.0f);
                    a_hor[x] *= table_hor[x];
                }
            }

            for (DT_S32 x = width; x < width_padding; ++x)
            {
                a_hor[x] = std::complex<DT_F32>(0.0f, 0.0f);
            }

            FFTRadix2Neon(a_hor, width_padding, dft_exp_table, idx_table, DT_FALSE);
            for (DT_S32 x = 0; x < width_padding; ++x)
            {
                a_hor[x] *= b_hor[x];
            }
            FFTRadix2Neon(a_hor, width_padding, idft_exp_table, idx_table, DT_TRUE);

            std::complex<DT_F32> *dst_row = dst.Ptr<std::complex<DT_F32>>(y);
            DT_F32 norm_coef = with_scale ? (1.0f / width) : 1.0f;
            for (DT_S32 x = 0; x < width; x++)
            {
...
static Status DftBluesteinRowNeon(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            ret = DftBluesteinRowNeonImpl<DT_U8, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::S8:
        {
            ret = DftBluesteinRowNeonImpl<DT_S8, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::U16:
        {
            ret = DftBluesteinRowNeonImpl<DT_U16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        case ElemType::S16:
        {
            ret = DftBluesteinRowNeonImpl<DT_S16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
#if defined(AURA_ENABLE_NEON_FP16)
        case ElemType::F16:
        {
            ret = DftBluesteinRowNeonImpl<MI_F16, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
#endif // AURA_ENABLE_NEON_FP16
        case ElemType::F32:
        {
            ret = DftBluesteinRowNeonImpl<DT_F32, 0>(ctx, src, dst, DT_FALSE, target);
            break;
        }
        default:
        {
...
template <typename Tp, DT_BOOL IS_DST_C1>
AURA_INLINE DT_VOID FftConvolveColNeon(Mat &src, Mat &dst, DT_S32 h_padding, DT_U16 *idx_table,
                                       std::complex<DT_F32> *a_ver, std::complex<DT_F32> *b_ver,
                                       std::complex<DT_F32> *exp_table, std::complex<DT_F32> *dft_exp_table,
                                       std::complex<DT_F32> *idft_exp_table, DT_BOOL with_scale, DT_S32 x)
{
    DT_S32 height = src.GetSizes().m_height;

    for (DT_S32 y = 0; y < height; y++)
    {
        std::complex<DT_F32> *cur_data = src.Ptr<std::complex<DT_F32>>(y);
        a_ver[y] = cur_data[x] * exp_table[y];
    }

    for (DT_S32 y = height; y < h_padding; y++)
    {
        a_ver[y] = std::complex<DT_F32>(0.0f, 0.0f);
    }

    FFTRadix2Neon(a_ver, h_padding, dft_exp_table, idx_table, DT_FALSE);
    for (DT_S32 i = 0; i < h_padding; ++i)
    {
        a_ver[i] *= b_ver[i];
    }
    FFTRadix2Neon(a_ver, h_padding, idft_exp_table, idx_table, DT_TRUE);

    DT_F32 norm_coef = with_scale ? (1.0f / height) : 1.0f;
    ...
    else
    {
        for (DT_S32 y = 0; y < height; y++)
        {
            ...
            std::complex<DT_F32> result   = a_ver[y] * exp_table[y] * norm_coef;
...
template <typename Tp, DT_U8 IS_INVERSE, DT_BOOL IS_DST_C1>
static Status DftBluesteinColNeonImpl(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
    WorkerPool *wp = ctx->GetWorkerPool();
    if (DT_NULL == wp)
    ...
    DT_S32 height_padding = 1;
    while (height_padding / 2 <= height)
    {
        height_padding *= 2;
    }

    DT_S32 buffer_sz = height_padding * sizeof(std::complex<DT_F32>);
    ThreadBuffer thread_buffer(ctx, buffer_sz);

    DT_S32 table_sz = height_padding * sizeof(DT_U16) + (2 * height_padding + height) * sizeof(std::complex<DT_F32>);
    DT_VOID *temp_buffer = AURA_ALLOC(ctx, table_sz);
    if (DT_NULL == temp_buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
    ...
    DT_U16 *idx_table                    = reinterpret_cast<DT_U16*>(temp_buffer);
    std::complex<DT_F32> *b_ver          = reinterpret_cast<std::complex<DT_F32>*>(idx_table + height_padding);
    std::complex<DT_F32> *exp_table      = b_ver + height_padding;
    std::complex<DT_F32> *dft_exp_table  = exp_table + height;
    std::complex<DT_F32> *idft_exp_table = dft_exp_table + height_padding / 2;
    ...
    FFTRadix2Neon(b_ver, height_padding, dft_exp_table, idx_table, DT_FALSE);

    auto col_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        std::complex<DT_F32> *a_ver = thread_buffer.GetThreadData<std::complex<DT_F32>>();
...
template <typename Tp>
static Status IDftBluesteinColNeonHelper(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    Status ret = Status::ERROR;
    ...
    if (is_dst_c1)
    {
        ret = DftBluesteinColNeonImpl<Tp, 1, DT_TRUE>(ctx, src, dst, with_scale, target);
    }
    else
    {
        ret = DftBluesteinColNeonImpl<Tp, 1, DT_FALSE>(ctx, src, dst, with_scale, target);
    }
...
static Status IDftBluesteinColNeonHelper(Context *ctx, Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    ...
    switch (elem_type)
    {
        case ElemType::U8:
        {
            ret = IDftBluesteinColNeonHelper<DT_U8>(ctx, src, dst, with_scale, target);
            break;
        }
        case ElemType::S8:
        {
            ret = IDftBluesteinColNeonHelper<DT_S8>(ctx, src, dst, with_scale, target);
            break;
        }
        case ElemType::U16:
        {
            ret = IDftBluesteinColNeonHelper<DT_U16>(ctx, src, dst, with_scale, target);
            break;
        }
        case ElemType::S16:
        {
            ret = IDftBluesteinColNeonHelper<DT_S16>(ctx, src, dst, with_scale, target);
            break;
        }
        case ElemType::U32:
        {
            ret = IDftBluesteinColNeonHelper<DT_U32>(ctx, src, dst, with_scale, target);
            break;
        }
        case ElemType::S32:
        {
            ret = IDftBluesteinColNeonHelper<DT_S32>(ctx, src, dst, with_scale, target);
            break;
        }
# if defined(AURA_ENABLE_NEON_FP16)
        case ElemType::F16:
...
# endif // AURA_ENABLE_NEON_FP16
        case ElemType::F32:
        {
            ret = IDftBluesteinColNeonHelper<DT_F32>(ctx, src, dst, with_scale, target);
            break;
        }
        default:
        {
...
Status DftNeon::Run()
{
    ...
    if (IsPowOf2(height))
    {
        ret = DftRadix2ColNeonImpl<DT_F32, 0, DT_FALSE>(m_ctx, *dst, *dst, DT_FALSE);
        if (ret != Status::OK)
        {
    ...
    {
        ret = DftBluesteinColNeonImpl<DT_F32, 0, DT_FALSE>(m_ctx, *dst, *dst, DT_FALSE, m_target);
        if (ret != Status::OK)
        {
...
Status InverseDftNeon::SetArgs(const Array *src, Array *dst, DT_BOOL with_scale)
{
...
Status InverseDftNeon::Run()
{
    ...
    {
        if (DftRadix2RowNeonImpl<DT_F32, 1>(m_ctx, *src, *mid, m_with_scale, m_target) != Status::OK)
        {
    ...
    {
        if (DftBluesteinRowNeonImpl<DT_F32, 1>(m_ctx, *src, *mid, m_with_scale, m_target) != Status::OK)
        {
