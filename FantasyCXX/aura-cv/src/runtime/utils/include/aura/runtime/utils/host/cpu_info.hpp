...
public:
    ...
    std::vector<DT_S32> GetCpuIdxs(CpuAffinity affinity)
    {
    ...
    DT_BOOL IsAtomicsSupported()
    {
        return m_atomics_valid;
    }

private:
    CpuInfo() : m_atomics_valid(DT_FALSE)
    {
        InitCpuIdxs();
    ...
    DT_VOID InitCpuIdxs()
    {
        DT_S32 total_cores_count = Max(static_cast<DT_S32>(std::thread::hardware_concurrency()), 1);

#if defined(AURA_BUILD_ANDROID)
        Status status = Status::OK;

        DT_S64 avg_freq_khz = 0;
        std::vector<DT_S64> cpu_freq_khz;

        DT_S32 cpu_cores_count = 0;
        DT_S64 total_freq_khz = 0;
        for (DT_S32 index = 0; index < total_cores_count; index++)
        {
            DT_S32 freq_values = 0;

            DT_CHAR nama_buffer[256] = {0};
            ...
            cpu_cores_count++;
            DT_S32 ret = fscanf(fp, "%d", &freq_values);
...
private:
    std::vector<DT_S32> m_all_cpu_idxs;
    std::vector<DT_S32> m_big_cpu_idxs;
    std::vector<DT_S32> m_little_cpu_idxs;
    DT_BOOL m_atomics_valid;
};
