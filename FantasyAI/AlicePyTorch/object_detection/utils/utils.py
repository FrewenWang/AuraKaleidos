...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # 4x3x13x13
    nB = pred_boxes.size(0)  # BatchSize 4   批量数据，目前使用的是4
    nA = pred_boxes.size(1)  # 每个格子对应了多少个anchor 目前YOLOv3表示有多少个先验框，目前是3个
    nC = pred_cls.size(-1)  # 类别的数量
    nG = pred_boxes.size(2)  # gridsize
    print("build_targets:({0}x{1}x{2}x{3}x{4})".format(nB, nA, nG, nG, nC))

    # Output tensors
    obj_mask = ByteTensor(nB, nA, nG, nG).fill_(0)  # obj，anchor包含物体, 即为1，默认为0 考虑前景
    noobj_mask = ByteTensor(nB, nA, nG, nG).fill_(1)  # no_obj, anchor不包含物体, 则为1，默认为1 考虑背景
    ...
    gxy = target_boxes[:, :2]
    gwh = target_boxes[:, 2:]
    # Get anchors with best iou 遍历所有的候选框，计算真实值的候选框，求得最大的IOU的值
    ...
    gx, gy = gxy.t()
    gw, gh = gwh.t()
    gi, gj = gxy.long().t()  # 位置信息，向下取整了。对推理出来的数据进行向下取整数
    # Set masks
