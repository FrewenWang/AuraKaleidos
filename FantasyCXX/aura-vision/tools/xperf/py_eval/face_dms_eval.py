...
class DmsEval(BaseEval):
    def __init__(self):
        super(DmsEval, self).__init__()
        self.feature_name = 'dms'
        self.local_data_path = self.local_data_path + '/' + self.feature_name
        self.save_file_name = 'dms_sample_images'
        self.class_names = ["normal", "smoke", "silence", "drink", "open"]
        self.actual = []
        self.predicted = []
        self.test_dict = {}
        self.wrong_detect = []
    ...
    def prepare_eval_data(self):
        ...
        cd /home/iov/vision_space/data/VisionImages/test_images/dms
    ...
    def eval(self, predict_json):
        f1 = open(os.path.join(self.local_data_path, 'dms_detect_wrong_images.csv'), 'w', encoding='utf-8');
        ...
        for image_name, image_label in self.test_dict.items():
            ...
            if face_id < 1:
                self.no_face += 1
                continue
            detect_label = int(image_result_json["dangerous_state"])
            if detect_label < 1:
                continue
    ...
    def write_wrong_record(self):
        with open(os.path.join(self.local_data_path, 'dms_wrong_detect.csv'), 'w', encoding='utf-8') as f:
            csv_writer = csv.writer(f)
            csv_writer.writerow(["image", "image label", "predict label"])
            os.system('rm -r ' + os.path.join(self.local_data_path, 'dms_detect_wrong_images'))
            os.system('mkdir ' + os.path.join(self.local_data_path, 'dms_detect_wrong_images'))
            for record in self.wrong_detect:
                ...
                shutil.copyfile(os.path.join(self.local_data_path, self.save_file_name, record[0]),
                                os.path.join(self.local_data_path, 'dms_detect_wrong_images', record[0]))
