...
def predict_image_onnx(ort_sess):
    ...
    image = cv2.imread("../res/946686572495.jpg")
    img_w = image.shape[1]
    img_h = image.shape[0]
    ...
    img = np.zeros(shape=[1, 1, 160, 288], dtype=np.float32)
    img[0][0] = image_pre

    #img.tofile("input_1x1x160x288.raw")
    #data = np.fromfile("input_1x1x160x288.raw", dtype=np.float32)
    #img = np.reshape(data, (1, 1, 160, 288))
    ...
    # r0.tofile("rslt_fea1_1x18x5x9.raw")
    # r1.tofile("rslt_fea2_1x18x10x18.raw")
    # r2.tofile("rslt_fea3_1x18x20x36.raw")
    # r0 = np.fromfile("rslt_fea1_1x18x5x9.raw", dtype=np.float32)
    # r1 = np.fromfile("rslt_fea2_1x18x10x18.raw", dtype=np.float32)
    # r2 = np.fromfile("rslt_fea3_1x18x20x36.raw", dtype=np.float32)
    # r0 = np.reshape(r0, [1, 18, 5, 9])
    # r1 = np.reshape(r1, [1, 18, 10, 18])
    # r2 = np.reshape(r2, [1, 18, 20, 36])
    # head_output = [r0, r1, r2]
    ...
    for i, pred in enumerate(boxes):
        if pred[1] >= 0.45:
            ...
            #a,e,c,d =  map(int, [265,414,739,1056])
            #cv2.rectangle(image_disp, (a, e), (c, d), (0, 0, 255), 2, 8, 0)
            fscore = pred[1]
...
if __name__ == "__main__":
    onnx_file = "../../models/onnx_models/Headshoulder0921Main_quant.onnx"
