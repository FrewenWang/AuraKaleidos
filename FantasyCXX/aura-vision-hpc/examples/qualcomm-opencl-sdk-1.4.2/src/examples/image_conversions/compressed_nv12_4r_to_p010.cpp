//--------------------------------------------------------------------------------------
// File: compressed_nv12_4r_to_p010.cpp
// Desc: This program converts NV12 to compressed NV12_4R to P010
//
// Author:      QUALCOMM
//
...
static const char *PROGRAM_SOURCE = R"(
    __kernel void
    compressed_nv12_4r_to_p010(__read_only  image2d_t input_compressed_nv12,
...
int main(int argc, char** argv)
{
    if (argc != 3)
    {
        std::cerr << "Usage: " << argv[0] << " <img data file> <out img data file> \n"
                  ...
                  << "Demonstrates conversions from NV12 to compressed NV12 4R to P010\n"
    ...
    cl_kernel           compressed_nv12_4r_to_p010_kernel = wrapper.make_kernel("compressed_nv12_4r_to_p010", program);
    ...
    cl_image_format compressed_nv12_4r_format;
    compressed_nv12_4r_format.image_channel_order     = CL_QCOM_COMPRESSED_NV12_4R;
    compressed_nv12_4r_format.image_channel_data_type = CL_UNORM_INT8;

    cl_image_desc compressed_nv12_4r_desc;
    std::memset(&compressed_nv12_4r_desc, 0, sizeof(compressed_nv12_4r_desc));
    compressed_nv12_4r_desc.image_type   = CL_MEM_OBJECT_IMAGE2D;
    compressed_nv12_4r_desc.image_width  = src_nv12_image_info.y_width;
    compressed_nv12_4r_desc.image_height = src_nv12_image_info.y_height;

    cl_mem_dmabuf_host_ptr compressed_nv12_mem = wrapper.make_buffer_for_compressed_image(compressed_nv12_4r_format, compressed_nv12_4r_desc);
    cl_mem compressed_nv12_4r_image = clCreateImage(
        context,
        CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR | CL_MEM_EXT_HOST_PTR_QCOM,
        &compressed_nv12_4r_format,
        &compressed_nv12_4r_desc,
        &compressed_nv12_mem,
        &err
    );
    if (err != CL_SUCCESS)
    ...
    compressed_y_plane_format.image_channel_order     = CL_QCOM_COMPRESSED_NV12_4R_Y;
    ...
    compressed_y_plane_desc.image_width  = compressed_nv12_4r_desc.image_width;
    compressed_y_plane_desc.image_height = compressed_nv12_4r_desc.image_height;
    compressed_y_plane_desc.mem_object   = compressed_nv12_4r_image;
    ...
    compressed_uv_plane_format.image_channel_order     = CL_QCOM_COMPRESSED_NV12_4R_UV;
    ...
    compressed_uv_plane_desc.mem_object   = compressed_nv12_4r_image;
    ...
    err = clSetKernelArg(compressed_nv12_4r_to_p010_kernel, 0, sizeof(compressed_nv12_4r_image), &compressed_nv12_4r_image);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 0 for compressed_nv12_4r_to_p010_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_4r_to_p010_kernel, 1, sizeof(out_y_plane), &out_y_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 1 for compressed_nv12_4r_to_p010_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_4r_to_p010_kernel, 2, sizeof(out_uv_plane), &out_uv_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 2 for compressed_nv12_4r_to_p010_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_4r_to_p010_kernel, 3, sizeof(sampler), &sampler);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 3 for compressed_nv12_4r_to_p010_kernel." << "\n";
        std::exit(err);
    }
    ...
    err = clEnqueueNDRangeKernel(
        command_queue,
        compressed_nv12_4r_to_p010_kernel,
        2,
        nullptr,
        work_size,
        nullptr,
    ...
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clEnqueueNDRangeKernel for compressed_nv12_4r_to_p010_kernel." << "\n";
        std::exit(err);
    }
    ...
    clReleaseMemObject(compressed_nv12_4r_image);
