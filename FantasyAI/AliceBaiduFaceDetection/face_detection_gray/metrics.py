...
def get_categories(label_list):
    ...
    Args:
        metric_type (str): metric type, currently support 'voc'.
        label_list (str): label name list
    """
...
class VOCMetric(Metric):
    def __init__(self,
                 ...
                 classwise=False):
        self.catid2name = get_categories(label_list)
    ...
    def update(self, inputs, outputs):
        ...
        # scale = inputs['scale_factor'].numpy()
        # scale = np.array([scale[0][0], scale[0][1], scale[0][0], scale[0][1]])
        bboxes = outputs[0][:, 2:].numpy() #/ scale
        scores = outputs[0][:, 1].numpy()
        labels = outputs[0][:, 0].numpy()
        bbox_lengths = outputs[1].numpy()
        ...
        for i in range(gt_boxes.shape[0]):
            ...
            gt_box, gt_label = prune_zero_padding(gt_box, gt_label)
