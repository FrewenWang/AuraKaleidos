#ifndef AURA_RUNTIME_UTILS_LOGGER_HPP__
#define AURA_RUNTIME_UTILS_LOGGER_HPP__
...
#define AURA_PRINT(tag, level, format, ...)                                                            \
    do {                                                                                               \
            aura::Print(tag, level, format, ##__VA_ARGS__);                                            \
    } while (0)
...
#define AURA_PRINTE(tag, format, ...)                                                                  \
        AURA_PRINT(tag, aura::LogLevel::ERROR, "%s[%s:%d] " format,                                    \
                   __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__)

/**
 * @brief Log an informational message without context.
 */
#define AURA_PRINTI(tag, format, ...)                                                                  \
        AURA_PRINT(tag, aura::LogLevel::INFO, format, ##__VA_ARGS__)

/**
 * @brief Log a debug message without context.
 */
#define AURA_PRINTD(tag, format, ...)                                                                  \
        AURA_PRINT(tag, aura::LogLevel::DEBUG, format, ##__VA_ARGS__)
...
#define AURA_LOGE(ctx, tag, format, ...)                                                               \
        AURA_LOG(ctx, tag, aura::LogLevel::ERROR, "%s[%s:%d] " format,                                 \
                 __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__)

/**
 * @brief Logs an informational message.
 */
#define AURA_LOGI(ctx, tag, format, ...)                                                               \
        AURA_LOG(ctx, tag, aura::LogLevel::INFO, format, ##__VA_ARGS__)

/**
 * @brief Logs a debug message.
 */
#define AURA_LOGD(ctx, tag, format, ...)                                                               \
        AURA_LOG(ctx, tag, aura::LogLevel::DEBUG, format, ##__VA_ARGS__)
...
/**
 * @brief Print a formatted message with the specified tag, log level, and format.
 *        Message will be printed using logcat on Android, Farf on Hexagon, and stdout on other platforms.
 *
 * @param tag    The tag associated with the log message.
 ...
 * @param ...    Additional arguments for formatting the log message.
 *
 * @note  Android log print can only support 1023 characters maximum per line.
 *        For long message, consider using '\n' to split into multiple lines or use multiple log prints.
 *
 * @code
 ...
 *        This function is only available on Hexagon.
 *
 * @param level  The log level for the message.
 * @param format The format string for the log message.
 * @param ...    Additional arguments for formatting the log message.
...
public:
     ...
     */
    AURA_DISABLE_COPY_AND_ASSIGN(Logger);
...
#endif // AURA_RUNTIME_UTILS_LOGGER_HPP__