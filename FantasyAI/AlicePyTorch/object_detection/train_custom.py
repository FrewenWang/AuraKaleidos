...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluation_interval", type=int, default=50, help="interval evaluations on validation set")
    ...
    # Get data configuration. 获取数据相关的配置
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_i, (_, images, targets) in enumerate(dataloader):
            ...
            # 进行模型的构建，和进行模型的前向传播，
            # 前向传播的我们传入数据，获取推理结果，然后和标签数据进行对比，求解loss函数
        ...
        if epoch % opt.checkpoint_interval == 0:
            torch.save(model.state_dict(), f"checkpoints/custom/yolov3_ckpt_%d.pth" % epoch)
