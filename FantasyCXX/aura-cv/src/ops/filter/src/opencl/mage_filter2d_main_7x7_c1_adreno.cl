                              ...
                              struct Scalar border_value)
{
    ...
    if (gy >= ksh && (gy + 1) < (height - ksh))
    {
        ...
        v8f32_src[7] = VLOAD(src + v8s32_offset_src.s7, 8);
    }
    else
    {
# if BORDER_CONSTANT
        ...
        short8 v8s16_flag_body = (short8)(v3s16_y_idx_p, (short)gy, (short)(gy + 1), v3s16_y_idx_n);
        ...
        v8f32_src[7] = VLOAD(src + v8s32_offset_src.s7, 8) * (Tp)(v8s16_flag_body.s7) + v8tp_val_border.s7;
#elif BORDER_REFLECT_101
        int8 v8_s32_offset_y = convert_int8(abs(gy + (int8)(-3, -2, -1, 0, 1, 2, 3, 4)));
    ...
    VSTORE(v2tp_result, dst + v2s32_offset_dst.s0, 2);

    v16f32_sum = (float16)0;
    __attribute__((opencl_unroll_hint(7)))
    for (int i = 1; i < 8; ++i)
    {
        const int ker_id = 7 * (i - 1);
        v16f32_sum += FILTER2D_CONVERT((V16Tp)(v8f32_src[i].s01, v8f32_src[i].s12, v8f32_src[i].s23, v8f32_src[i].s34,
                                       v8f32_src[i].s45, v8f32_src[i].s56, v8f32_src[i].s67, (V2Tp)0), float16) *
                                       (float16)((float2)filter[ker_id], (float2)filter[ker_id + 1],
                                       (float2)filter[ker_id + 2], (float2)filter[ker_id + 3], (float2)filter[ker_id + 4],
    ...
    v2f32_result = (float2)(SUM_REDUCE(v16f32_sum.even, 8), SUM_REDUCE(v16f32_sum.odd, 8));
    v2tp_result = CONVERT_SAT(v2f32_result, V2Tp);
    VSTORE(v2tp_result, dst + v2s32_offset_dst.s1, 2);
}