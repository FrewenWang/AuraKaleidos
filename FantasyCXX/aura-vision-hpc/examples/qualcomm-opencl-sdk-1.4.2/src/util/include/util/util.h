...
struct complex_matrix_t
{
    int width, height;
    std::vector<cl_float2> elements;
};
...
/**
 * \brief Saves raw bytes to the given filename
 *
 * @param filename
 * @param image
 * @param size
 */
void save_raw_image_data(const std::string &filename, std::vector<unsigned char> image, size_t size);
...
/**
 * \brief Loads a matrix of complex numbers from the given file according to the format
 *        described in README.md
 * @param filename
 */
...
/**
 * \brief get image channel order as a string
 * @param format
 * @return name of image channel order as string
 */
std::string get_image_order(cl_image_format format);

/**
 * \brief get image channel data type as a string
 * @param format
 * @return name of image channel data type as string
 */
