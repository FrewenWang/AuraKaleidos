...
def predict_image_onnx(ort_sess):
    ...
    image = cv2.imread("../res/316407.yuv.jpg")
    img_w = image.shape[1]
    img_h = image.shape[0]
    ...
    image = np.zeros(shape=[1, 1, 160, 320], dtype=np.float32)
    image[0][0] = image_pre


    #image.tofile("demo_1x1x160x320.raw")
    #image = np.fromfile("demo_1x1x160x320.raw", dtype=np.float32)
    #image = np.reshape(image, [1, 1, 160, 320])
    ort_inputs = {ort_sess.get_inputs()[0].name: image}
    head_output = ort_sess.run(None, ort_inputs)

    #head_output[0].tofile("fea1_1x21x5x10.raw")
    #head_output[1].tofile("fea2_1x21x10x20.raw")
    #head_output[2].tofile("fea3_1x21x20x40.raw")

    #rslt1 = np.fromfile("fea1_1x21x5x10.raw", dtype=np.float32)
    #rslt2 = np.fromfile("fea2_1x21x10x20.raw", dtype=np.float32)
    #rslt3 = np.fromfile("fea3_1x21x20x40.raw", dtype=np.float32)
    #rslt1 = np.reshape(rslt1, [1, 21, 5, 10])
    #rslt2 = np.reshape(rslt2, [1, 21, 10, 20])
    #rslt3 = np.reshape(rslt3, [1, 21, 20, 40])
    ...
    grid_ss = [1., 1., 1.]
    num_bias = 3
    conf_thr_table = [0.55, 0.35, 0.2]  # conf_thr_table = [0.005, 0.005, 0.005]
    det_w = 320
    det_h = 160
    box_rslt = []
    ...
    for i, pred in enumerate(boxes_rslt):
        ...
        #手的阈值
        if pred[0] == 0 and pred[1] > 0.55:
            boxes_dst.append([pred[0], pred[1], l,t,r,b])
        #玩手机的阈值
        elif pred[0] == 1 and pred[1] > 0.35:
            boxes_dst.append([pred[0], pred[1], l,t,r,b])
        l, t, r, b = map(int, [l, t, r, b])
        if pred[0] == 0:
            cv2.rectangle(image_disp, (l, t), (r, b), (0, 255, 255), 2, 8, 0)  # 黄色是手
            cv2.putText(image_disp, str("gesture"), (l, t), 1, 1, (0, 0, 255),
                        1, 8, 0)
        else:
            cv2.rectangle(image_disp, (l, t), (r, b), (0, 0, 255), 2, 8, 0)  # 红色是看手机
            cv2.putText(image_disp, str("play_phone"), (l, t), 1, 1, (0, 0, 255),
                        1, 8, 0)
        fscore = pred[1]
...
if __name__ == "__main__":
    onnx_file = "../../models/onnx_models/HandPhoneDetection0705Main_quant.onnx"
