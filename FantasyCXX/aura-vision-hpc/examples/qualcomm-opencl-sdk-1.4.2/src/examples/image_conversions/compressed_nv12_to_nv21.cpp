//--------------------------------------------------------------------------------------
// File: compressed_nv12_to_nv21.cpp
// Desc: This program converts compressed NV12 to NV21
//
// Author:      QUALCOMM
//
...
static const char *PROGRAM_SOURCE = R"(
    __kernel void
    compressed_nv12_to_nv21(__read_only  image2d_t input_compressed_nv12,
                            __write_only image2d_t out_nv21_y,
                            __write_only image2d_t out_nv21_uv,
                                         sampler_t sampler)
    {
        int2   coord;
        float4 in;
        ...
        // Swizzle the UV values compared to the NV12 kernel.
        uv.xy = read_imagef(input_compressed_nv12, sampler, coord).zy;
        write_imagef(out_nv21_y, coord, (float4)(ys.s3));
        coord += (int2)(1, 0);
        write_imagef(out_nv21_y, coord, (float4)(ys.s2));
        coord += (int2)(0, 1);
        write_imagef(out_nv21_y, coord, (float4)(ys.s1));
        coord -= (int2)(1, 0);
        write_imagef(out_nv21_y, coord, (float4)(ys.s0));
        write_imagef(out_nv21_uv, coord / 2, uv);
    }
...
int main(int argc, char** argv)
{
    if (argc != 5)
    {
        std::cerr << "Usage: " << argv[0] << " <img data file> <width> <height> <out img data file> \n"
                  << "Input image file data should be in format CL_QCOM_NV12 / CL_UNORM_INT8\n"
                  << "Demonstrates conversions from compressed NV12 to NV21\n"
    ...
    const std::string out_image_filename(argv[4]);

    cl_wrapper          wrapper;
    cl_program          program                        = wrapper.make_program(&PROGRAM_SOURCE, 1);
    cl_kernel           compressed_nv12_to_nv21_kernel = wrapper.make_kernel("compressed_nv12_to_nv21", program);
    ...
    src_nv12_image_info.y_width = width;
    src_nv12_image_info.y_height = height;
    ...
    cl_image_format out_nv21_format;
    out_nv21_format.image_channel_order     = CL_QCOM_NV12;
    out_nv21_format.image_channel_data_type = CL_UNORM_INT8;

    cl_image_desc out_nv21_desc;
    std::memset(&out_nv21_desc, 0, sizeof(out_nv21_desc));
    out_nv21_desc.image_type             = CL_MEM_OBJECT_IMAGE2D;
    out_nv21_desc.image_width            = src_nv12_image_info.y_width;
    out_nv21_desc.image_height           = src_nv12_image_info.y_height;
    cl_mem_dmabuf_host_ptr out_nv21_mem = wrapper.make_buffer_for_yuv_image(out_nv21_format, out_nv21_desc);
    cl_mem out_nv21_image = clCreateImage(
            context,
            CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR | CL_MEM_EXT_HOST_PTR_QCOM,
            &out_nv21_format,
            &out_nv21_desc,
    ...
    out_y_plane_desc.image_width  = out_nv21_desc.image_width;
    out_y_plane_desc.image_height = out_nv21_desc.image_height;
    out_y_plane_desc.mem_object   = out_nv21_image;
    ...
    // The image dimensions for the uv-plane derived image must be the same as the parent image, even though the
    // actual dimensions of the uv-plane are halved in each dimension.
    out_uv_plane_desc.image_width  = out_nv21_desc.image_width;
    out_uv_plane_desc.image_height = out_nv21_desc.image_height;
    out_uv_plane_desc.mem_object   = out_nv21_image;
    ...
    err = clSetKernelArg(compressed_nv12_to_nv21_kernel, 0, sizeof(compressed_nv12_image), &compressed_nv12_image);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 0 for compressed_nv12_to_nv21_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_to_nv21_kernel, 1, sizeof(out_y_plane), &out_y_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 1 for compressed_nv12_to_nv21_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_to_nv21_kernel, 2, sizeof(out_uv_plane), &out_uv_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 2 for compressed_nv12_to_nv21_kernel." << "\n";
        std::exit(err);
    }

    err = clSetKernelArg(compressed_nv12_to_nv21_kernel, 3, sizeof(sampler), &sampler);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 3 for compressed_nv12_to_nv21_kernel." << "\n";
        std::exit(err);
    }

    work_size[0] = out_nv21_desc.image_width / 2;
    work_size[1] = out_nv21_desc.image_height / 2;
    err = clEnqueueNDRangeKernel(
            command_queue,
            compressed_nv12_to_nv21_kernel,
            2,
            nullptr,
            work_size,
            nullptr,
    ...
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clEnqueueNDRangeKernel for compressed_nv12_to_nv21_kernel." << "\n";
        std::exit(err);
    }
    ...
    nv12_image_t out_nv21_image_info;
    out_nv21_image_info.y_width  = out_nv21_desc.image_width;
    out_nv21_image_info.y_height = out_nv21_desc.image_height;
    out_nv21_image_info.y_plane.resize(out_nv21_image_info.y_width * out_nv21_image_info.y_height);
    out_nv21_image_info.uv_plane.resize(out_nv21_image_info.y_width * out_nv21_image_info.y_height / 2);

    buf_sync.flags = DMA_BUF_SYNC_START | DMA_BUF_SYNC_READ;
    if (ioctl(out_nv21_mem.dmabuf_filedesc, DMA_BUF_IOCTL_SYNC, &buf_sync))
    {
    ...
    const size_t out_y_region[] = {out_nv21_desc.image_width, out_nv21_desc.image_height, 1};
    row_pitch                   = 0;
    image_ptr = reinterpret_cast<unsigned char *>(clEnqueueMapImage(
            command_queue,
            out_y_plane,
    ...
    for (uint32_t i = 0; i < out_nv21_desc.image_height; ++i)
    {
        std::memcpy(
                out_nv21_image_info.y_plane.data() + i * out_nv21_desc.image_width,
                image_ptr                          + i * row_pitch,
                out_nv21_desc.image_width
        );
    }
    ...
    const size_t out_uv_region[] = {out_nv21_desc.image_width / 2, out_nv21_desc.image_height / 2, 1};
    row_pitch                    = 0;
    image_ptr = reinterpret_cast<unsigned char *>(clEnqueueMapImage(
            command_queue,
            out_uv_plane,
    ...
    // Copies image data from the dmabuf buffer to the host
    for (uint32_t i = 0; i < out_nv21_desc.image_height / 2; ++i)
    {
        std::memcpy(
                out_nv21_image_info.uv_plane.data() + i * out_nv21_desc.image_width,
                image_ptr                           + i * row_pitch,
                out_nv21_desc.image_width
        );
    }
    ...
    save_nv12_image_data(out_image_filename, out_nv21_image_info);
    ...
    clReleaseMemObject(out_nv21_image);
