...
#     :return:
#     """
#     path = "/media/baidu/ssd2/ppyolo/data_mp4/images_test"
#     allFiles = os.listdir(path)
#     for filet in allFiles:
#         pathImg  =  os.path.join(path, filet)
...
#
#         im_shape = np.array([[160, 288]], dtype=np.float32)
#
#         scale_y = 160. / image_disp.shape[0]
#         scale_x = 288. / image_disp.shape[1]
...
#                 centy = (t + b) / 2.
#                 h_pre = b-t
#                 l = max(0,  centx - h_pre/2.)
#                 r = min(centx + h_pre/2., image_disp.shape[1])
#
#                 l,t,r,b = map(int, [l,t,r,b])
#                 cv2.rectangle(image_disp, (l,t), (r,b), (0,255,255), 2, 8, 0)
#                 fscore = pred[1]
#                 cv2.putText(image_disp, str(fscore), (l,t), 1, 1, (0,0,255), 1, 8, 0)
#
...
def predict_images_onnx(ort_sess, image):
    """
    :param net:
    :param path:
    :return:
    ...
    #
    image = (image.astype(np.float32))
    image.tofile("rect_1_NHWC.raw")
    image = np.transpose(image, [0, 3, 1, 2])
    image.tofile("rect_1_NCHW.raw")
    #print('detect', image)
    onnx_facept = "./FaceDetect/FaceDetection0114V4Main.onnx"
...
def predict_video_onnx(ort_sess):
    """
    :param net:
    :param path:
    :return:
    ...
    while 1:
        ...
        # image = cv2.imread(pathImg)
        ret, image = cap.read()
        # image = cv2.imread("/media/baidu/3.6TB_SSD/val_data/0005_1752_DV_lowlight-reverse_rgb_number_one_bmp-001.jpg")
        if image is None:
            continue
        img_w = image.shape[1]
        img_h = image.shape[0]
        ...
        for i, pred in enumerate(boxes):
            if pred[1] >= 0.2:
                ...
                fscore = pred[1]
                fscore=round(fscore, 3)
                cv2.putText(image_disp, str(fscore)+"|"+str(r-l)+"|"+str(b-t), (l,t), 1, 3, (0,0,255), 3, 8, 0)

        cv2.imshow('preds', image_disp)
        cv2.waitKey(1)

# if __name__ == "__main__":
#     onnx_file = "./FaceDetect/FaceDetection0114V4Main.onnx"
#
#     # predict by ONNX Runtime
#     ort_sess = onnxruntime.InferenceSession(onnx_file)
#
#
#     #predict_images_onnx(ort_sess, image_path)
#
#     predict_video_onnx(ort_sess)
