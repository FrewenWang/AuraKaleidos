...
// Desc: This example is included to demonstrate some functions which work only for
//       compressed TP10 format (as opposed to uncompressed TP10 images).
...
int main(int argc, char** argv)
{
    ...
    cl_mem_dmabuf_host_ptr compressed_tp10_mem = wrapper.make_buffer_for_compressed_image(compressed_tp10_format,
    ...
    for (size_t i = 0; i < kernel_execution_params.size(); ++i)
    {
        ...
        {
            std::cerr << "On iteration " << i << ":\n";
            std::cerr << "\tError " << err << " with clFlush for command_queue.\n";
            return err;
        }
