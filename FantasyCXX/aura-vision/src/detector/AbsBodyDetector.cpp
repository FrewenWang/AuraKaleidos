...
int AbsBodyDetector::doDetect(VisionRequest *request, VisionResult *result) {
    int ret = V_TO_INT(Error::OK);
    for (int i = 0; i < static_cast<int>(mRtConfig->bodyNeedDetectCount); ++i) {
        auto *info = result->getBodyResult()->pBodyInfos[i];
