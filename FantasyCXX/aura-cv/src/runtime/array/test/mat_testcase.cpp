...
struct MatMembers
{
    MatMembers(DT_BOOL valid, ElemType elem_type, DT_S32 mem_type, DT_S64 total_bytes, ArrayType array_type, DT_S32 row_step, DT_S32 ref_count)
...
static DT_VOID FillMat(Mat &src)
{
    DT_S32 cnt = 0;
    ...
    for (DT_S32 y = 0; y < size.m_height; ++y)
    {
        ...
        {
            for (DT_S32 c = 0; c < size.m_channel; ++c)
            {
...
NEW_TESTCASE(runtime_array_mat_constructor_test)
{
    ...
    // Mat();
    Mat mat;
    ret |= AURA_CHECK_EQ(ctx, mat.IsValid(), DT_FALSE, "check Mat::IsValid() failed\n");

    // Mat(Context *ctx, ElemType elem_type, const Sizes3 &sizes, DT_S32 mem_type = AURA_MEM_DEFAULT, const Sizes &strides = Sizes(), DT_BOOL init = DT_TRUE);
    mat = Mat(ctx, ElemType::U16, Sizes3(9, 6, 3), AURA_MEM_INVALID);
    ret |= AURA_CHECK_EQ(ctx, mat.IsValid(), DT_FALSE, "check Mat::IsValid() failed\n");
    mat = Mat(ctx, ElemType::U32, Sizes3(9, 6, 3), AURA_MEM_HEAP, Sizes(6, 100));
    ret |= CHECK_MAT(ctx, mat, MatMembers(DT_TRUE, ElemType::U32, AURA_MEM_HEAP, 100 * 9, ArrayType::MAT, 8, 1));
    ...
    ret |= AURA_CHECK_EQ(ctx, mat.IsValid(), DT_TRUE, "check Mat::IsValid() failed\n");
    mat = Mat(ctx, ElemType::U8, Sizes3(9, 6, 3), buffer);
    ret |= CHECK_MAT(ctx, mat, MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_HEAP, 3 * 6 * 9, ArrayType::MAT, 6, 0));
    AURA_FREE(ctx, ptr_heap);

    ptr_heap = AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, 16, 128);
    buffer = ctx->GetMemPool()->GetBuffer(ptr_heap);
    ret |= AURA_CHECK_EQ(ctx, buffer.IsValid(), DT_TRUE, "check Buffer::IsValid() failed\n");
    mat = Mat(ctx, ElemType::U8, Sizes3(9, 6, 3), buffer);
    ret |= AURA_CHECK_EQ(ctx, mat.IsValid(), DT_FALSE, "check Mat::IsValid() failed\n");
    AURA_FREE(ctx, ptr_heap);
    ...
    ret |= CHECK_MAT(ctx, mat_roi, MatMembers(DT_TRUE, ElemType::F64, AURA_MEM_HEAP, 128, ArrayType::MAT, 4, 2));
    Rect roi(1, 1, 2, 2);
    ret |= CHECK_CMP_MAT_MEM(ctx, DT_F64, mat, mat_roi, roi);
    ret |= AURA_CHECK_EQ(ctx, mat_roi.IsContinuous(), DT_FALSE, "check Mat::IsContinuous() failed\n");

    mat = Mat(ctx, ElemType::F32, Sizes3(4, 3, 2));
    ret |= AURA_CHECK_EQ(ctx, mat.IsContinuous(), DT_TRUE, "check Mat::IsContinuous() failed\n");

    // static Mat* Create(Context *ctx, ElemType elem_type, const Sizes3 &sizes, DT_S32 mem_type = AURA_MEM_DEFAULT, const Sizes &strides = Sizes());
    Mat *mat_ptr = Create<Mat>(ctx, ElemType::U32, Sizes3(9, 6, 3), AURA_MEM_HEAP, Sizes(6, 100));
    ret |= CHECK_MAT(ctx, *mat_ptr, MatMembers(DT_TRUE, ElemType::U32, AURA_MEM_HEAP, 100 * 9, ArrayType::MAT, 8, 1));
...
NEW_TESTCASE(runtime_array_mat_init_test)
{
    ...
    Mat mat0 = mat;
    ret |= AURA_CHECK_EQ(ctx, mat0.IsValid(), DT_TRUE, "check Mat::IsValid() failed\n");
    ret |= AURA_CHECK_EQ(ctx, mat0.GetRefCount(), (DT_S32)2, "check Mat::GetRefCount() failed\n");
    mat = mat0.Clone();
    ret |= AURA_CHECK_EQ(ctx, mat0.IsValid(), DT_TRUE, "check Mat::IsValid() failed\n");
    ret |= AURA_CHECK_EQ(ctx, mat0.GetRefCount(), (DT_S32)1, "check Mat::GetRefCount() failed\n");

    mat.Release();
    mat.Release();
    ret |= AURA_CHECK_EQ(ctx, mat.IsValid(), DT_FALSE, "check Mat::IsValid() failed\n");
    ret |= CHECK_MAT(ctx, mat0, MatMembers(DT_TRUE, ElemType::U32, AURA_MEM_HEAP, 9 * 6 * 3 * 4, ArrayType::MAT, 6, 1));
...
NEW_TESTCASE(runtime_array_mat_clone_test)
{
    ...
    ret |= AURA_CHECK_EQ(ctx, mat_clone.IsEqual(mat), DT_TRUE, "check Mat::IsEqual() failed\n");
    ret |= AURA_CHECK_EQ(ctx, mat_clone.IsSizesEqual(mat), DT_TRUE, "check Mat::IsSizesEqual() failed\n");
    ret |= AURA_CHECK_EQ(ctx, mat_clone.IsChannelEqual(mat), DT_TRUE, "check Mat::IsChannelEqual() failed\n");
    ret |= AURA_CHECK_IEQ(ctx, mat.GetData(), mat_clone.GetData(), "check Mat::GetData() failed\n");

    Rect roi = Rect();
    ret |= CHECK_CMP_MAT_MEM(ctx, DT_U32, mat, mat_clone, roi);

    mat_clone = mat.Clone(Rect(1, 2, 3, 4), Sizes(6, 8));
    ret |= CHECK_MAT(ctx, mat_clone, MatMembers(DT_TRUE, ElemType::U32, AURA_MEM_HEAP, 216, ArrayType::MAT, 3, 1));
    roi = Rect(1, 2, 3, 4);
    ret |= CHECK_CMP_MAT_MEM(ctx, DT_U32, mat, mat_clone, roi);
...
NEW_TESTCASE(runtime_array_mat_dump_test)
{
    ...
    if (fin)
    {
        DT_U8 *src_ptr = (DT_U8 *)AURA_ALLOC(ctx, mat.GetTotalBytes());

        fin.read((DT_CHAR *)src_ptr, mat.GetTotalBytes());
        fin.close();

        DT_U8 *ref_ptr = mat.GetBuffer().GetData<DT_U8*>();
        for (DT_S64 i = 0; i < mat.GetTotalBytes(); ++i)
        {
    ...
    ret |= CHECK_CMP_MAT_MEM(ctx, DT_U8, mat, mat_load, roi);
...

    {
        ...
        for (DT_S32 i = 10; i < 20; i++)
        {
            DT_U8 *src = mat_load.Ptr<DT_U8>(i);
            DT_U8 *dst = mat_row.Ptr<DT_U8>(i - 10);
            for (DT_S32 j = 0; j < mat_row.GetSizes().m_width * mat_row.GetSizes().m_channel; j++)
            {
...

    {
        ...
        for (DT_S32 i = 0; i < mat_load.GetSizes().m_height; i++)
        {
            DT_U8 *src = mat_load.Ptr<DT_U8>(i);
            DT_U8 *dst = mat_col.Ptr<DT_U8>(i);
            for (DT_S32 j = 10 * mat_col.GetSizes().m_channel; j < 20 * mat_col.GetSizes().m_channel; j++)
            {
...

    {
        ...
        for (DT_S32 i = 10; i < 20; i++)
        {
            const DT_U8 *src = mat_load_new.Ptr<DT_U8>(i);
            DT_U8 *dst = mat_row.Ptr<DT_U8>(i - 10);
            for (DT_S32 j = 0; j < mat_row.GetSizes().m_width * mat_row.GetSizes().m_channel; j++)
            {
                ret |= AURA_CHECK_EQ(ctx, src[j], dst[j], "check mat memory failed\n");
...
NEW_TESTCASE(runtime_array_mat_print_test)
{
    ...
    DT_S32 elem_types[] = {static_cast<DT_S32>(ElemType::U8),  static_cast<DT_S32>(ElemType::S8),
                           static_cast<DT_S32>(ElemType::U16), static_cast<DT_S32>(ElemType::S16),
                           static_cast<DT_S32>(ElemType::U32), static_cast<DT_S32>(ElemType::S32),
                           static_cast<DT_S32>(ElemType::F32), static_cast<DT_S32>(ElemType::F64)};


    for (DT_S32 i = 0; i < static_cast<DT_S32>(sizeof(elem_types) / sizeof(DT_S32)); i++)
    {
...
NEW_TESTCASE(runtime_array_mat_poly_test)
{
    ...
    ret |= AURA_CHECK_EQ(ctx, array.GetRefCount(), (DT_S32)1, "check Mat::GetRefCount() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.GetRowPitch(), (DT_S32)100, "check Mat::GetRowPitch() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.GetRowStep(), (DT_S32)8, "check Mat::GetRowStep() failed\n");
    ...
    ret |= AURA_CHECK_EQ(ctx, array.GetTotalBytes(), static_cast<DT_S64>(900), "check Mat::GetTotalBytes() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.IsEqual(array0), DT_TRUE, "check Mat::IsEqual() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.IsSizesEqual(array0), DT_TRUE, "check Mat::IsSizesEqual() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.IsChannelEqual(array0), DT_TRUE, "check Mat::IsChannelEqual() failed\n");
    ret |= AURA_CHECK_EQ(ctx, array.IsValid(), DT_TRUE, "check Mat::IsValid() failed\n");
    array.Dump("dump.bin");
    array.Show();
    array.Release();
...
NEW_TESTCASE(runtime_mat_write_read_bmp_test)
{
    ...
    for (DT_S32 i = 0; i < 3; i++)
    {
        ...
        ret |= CHECK_CMP_MAT_MEM(ctx, DT_U8, mat, mat_read, roi);
    }
...
NEW_TESTCASE(runtime_mat_write_read_yuv_test)
{
    ...
    for (size_t i = 0; i < array.size(); i++)
    {
        ...
        for (DT_S32 j = 0; j < len; j++)
        {
            ...
            ret |= CHECK_CMP_MAT_MEM(ctx, DT_U8, mats[j], mats_read[j], roi);
        }
    }
...
NEW_TESTCASE(runtime_mat_yuv_test)
{
    ...
    ret |= CHECK_MAT(ctx, nv12_mats[0], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 128 * 200, ArrayType::MAT, 200, 3));
    ret |= CHECK_MAT(ctx, nv12_mats[1], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 200, ArrayType::MAT, 100, 3));
    AURA_LOGI(ctx, AURA_TAG, "GetYuvIaura NV12 test ok\n");

    std::vector<Mat> yu12_mats = GetYuvIaura(ctx, mat_yuv, IauraFormat::YU12);
    ret |= CHECK_MAT(ctx, yu12_mats[0], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 128 * 200, ArrayType::MAT, 200, 6));
    ret |= CHECK_MAT(ctx, yu12_mats[1], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 100, ArrayType::MAT, 100, 6));
    ret |= CHECK_MAT(ctx, yu12_mats[2], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 100, ArrayType::MAT, 100, 6));
    AURA_LOGI(ctx, AURA_TAG, "GetYuvIaura YU12 test ok\n");

    std::vector<Mat> i422_mats = GetYuvIaura(ctx, mat_yuv, IauraFormat::I422);
    ret |= CHECK_MAT(ctx, i422_mats[0], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 96 * 200, ArrayType::MAT, 200, 9));
    ret |= CHECK_MAT(ctx, i422_mats[1], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 96 * 100, ArrayType::MAT, 100, 9));
    ret |= CHECK_MAT(ctx, i422_mats[2], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 96 * 100, ArrayType::MAT, 100, 9));
    AURA_LOGI(ctx, AURA_TAG, "GetYuvIaura I422 test ok\n");

    std::vector<Mat> i444_mats = GetYuvIaura(ctx, mat_yuv, IauraFormat::I444, DT_TRUE);
    ret |= CHECK_MAT(ctx, i444_mats[0], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 200, ArrayType::MAT, 200, 1));
    ret |= CHECK_MAT(ctx, i444_mats[1], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 200, ArrayType::MAT, 200, 1));
    ret |= CHECK_MAT(ctx, i444_mats[2], MatMembers(DT_TRUE, ElemType::U8, AURA_MEM_DMA_BUF_HEAP, 64 * 200, ArrayType::MAT, 200, 1));
