...
static Status GetAreaOffsetAlign8X(Context *ctx, MI_S32 src_size, MI_S32 dst_size, MI_F32 scale, MI_S32 *x_idx, MI_F32 *x_coef)
{
    if (MI_NULL == x_idx || MI_NULL == x_coef)
    {
    ...
    MI_S32 k = 0;
    for (MI_S32 dx = 0; dx < dst_size; dx++)
    {
        MI_S32 end_id = k + 8; // one pixel in x direction map into 8 pixels

        const MI_F32 fsx1 = dx * scale;
        const MI_F32 fsx2 = fsx1 + scale;
        const MI_F32 cell_width = Min(scale, src_size - fsx1);
        const MI_F32 inv_cell_width = 1.0 / cell_width;
...
static Status GetAreaOffsetAlign8Y(Context *ctx, MI_S32 src_size, MI_S32 dst_size, MI_F32 scale, MI_S32 *y_idx, MI_F32 *y_coef, MI_S32 *y_tab)
{
    if (MI_NULL == y_idx || MI_NULL == y_coef || MI_NULL == y_tab)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
        return Status::ERROR;
    }
    ...
    for (MI_S32 dx = 0; dx < dst_size; dx++)
    {
        ...
        const MI_F32 fsx2 = fsx1 + scale;
        const MI_F32 cell_width = Min(scale, src_size - fsx1);
        const MI_F32 inv_cell_width = 1.0f / cell_width;
...
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_U8, SType>::value || std::is_same<MI_S8, SType>::value, AURA_VOID>::type
ResizeAreaCommNeonCore0(MI_F32 *buffer, MI_S32 idx, float32x4_t &vqf32_alpha_l, float32x4_t &vqf32_alpha_h, VType &vd8_src,
                        float32x4_t &vqf32_beta)
{
    auto vq16_src = neon::vmovl(vd8_src);
    float32x4_t vqf32_sum_result = neon::vload1q(buffer + idx);

    float32x4_t vqf32_src_l = neon::vcvt<MI_F32>(neon::vmovl(neon::vgetlow(vq16_src)));
    float32x4_t vqf32_src_h = neon::vcvt<MI_F32>(neon::vmovl(neon::vgethigh(vq16_src)));
...
template <typename SType, typename VType>
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_U16, SType>::value || std::is_same<MI_S16, SType>::value, AURA_VOID>::type
ResizeAreaCommNeonCore0(MI_F32 *buffer, MI_S32 idx, float32x4_t &vqf32_alpha_l, float32x4_t &vqf32_alpha_h,
...
// VType = float32x4_t
template <typename SType, typename VType>
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_F32, SType>::value, AURA_VOID>::type
ResizeAreaCommNeonCore0(MI_F32 *buffer, MI_S32 idx, float32x4_t &vqf32_alpha_l, float32x4_t &vqf32_alpha_h, VType &vqf32_x0_src,
...
// VType = float16x8_t
template <typename SType, typename VType>
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_F16, SType>::value, AURA_VOID>::type
ResizeAreaCommNeonCore0(MI_F32 *buffer, MI_S32 idx, float32x4_t &vqf32_alpha_l, float32x4_t &vqf32_alpha_h,
                        VType &vqf16_src, float32x4_t &vqf32_beta)
{
    float32x4_t vqf32_src_l = neon::vcvt<MI_F32>(neon::vgetlow(vqf16_src));
    float32x4_t vqf32_src_h = neon::vcvt<MI_F32>(neon::vgethigh(vqf16_src));
...
AURA_ALWAYS_INLINE AURA_VOID ResizeAreaCommNeonCore1(MI_F32 *buffer, MI_S32 idx, Tp *dst_row)
{
    ...
    MI_F32 result = neon::vgetlane<0>(vdf32_temp_data);
    dst_row[0] = SaturateCast<Tp>(result);
}

// Tp = MI_U8, MI_S8, MI_U16, MI_S16, MI_F16
template <typename Tp, MI_S32 C>
static typename std::enable_if<std::is_same<MI_U8, Tp>::value || std::is_same<MI_S8, Tp>::value || std::is_same<MI_U16, Tp>::value ||
std::is_same<MI_S16, Tp>::value || std::is_same<MI_F16, Tp>::value, Status>::type
ResizeAreaCommNeonImpl(Context *ctx, const Mat &src, Mat &dst, ThreadBuffer &thread_buffer, MI_S32 *x_idx, MI_F32 *x_coef,
                       MI_S32 *y_idx, MI_F32 *y_coef, MI_S32 *y_table, MI_S32 start_row, MI_S32 end_row)
{
    if (MI_NULL == x_idx || MI_NULL == x_coef || MI_NULL == y_idx || MI_NULL == y_coef || MI_NULL == y_table)
    {
        AURA_ADD_ERROR_STRING(ctx, "null ptr");
        return Status::ERROR;
    }
    ...
    MI_S32 max_src_height = iheight - 1;
    MI_S32 max_src_width = iwidth - 1;
    MI_S32 stop_x = iwidth - 8;

    float32x4_t vqf32_zeros = neon::vmovq(0.f);
    MI_S32 start_row_x2 = start_row << 1;
    MI_S32 end_row_x2   = end_row << 1;

    for (MI_S32 y = start_row_x2; y < end_row_x2; y += 2)
    {
        MI_S32 start_y_idx = y_table[y];
        MI_S32 end_y_idx = y_table[y + 1];

        // clear buffer
        for (MI_S32 x = 0; x < owidth * C; x++)
        {
            neon::vstore(buffer_data + (x << 2), vqf32_zeros);
        }

        for (MI_S32 j = start_y_idx; j < end_y_idx; j++)
        {
            MI_S32 sy = y_idx[j];
            ...
            if (sy == max_src_height)
            {
                ...
                {
                    MI_S32 tab_idx    = x << 3;
                    MI_S32 buffer_idx = x << 2;

                    MI_S32 start_x = x_idx[tab_idx];
                    if (start_x <= stop_x)
                    {
                        MVType mv_src;
                    ...
                    {
                        MI_S32 cof_idx = tab_idx;
                        MI_F32 y_beta = y_coef[j];

                        for (MI_S32 j = start_x; j <= max_src_width; j++)
                        {
                            for (MI_S32 ch = 0; ch < C; ch++)
                            {
...
static Status ResizeAreaCommNeonHelper(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    {
        ret = ResizeBnCommNeon(ctx, src, dst, MI_TRUE, target);
    }
    else if (scale_x > 7.f)
    {
        ...
        MI_S32 *tab_offset = static_cast<MI_S32*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, (oheight + 1) * sizeof(MI_S32), 0));
        if ((MI_NULL == x_tab) || (MI_NULL == y_tab) || (MI_NULL == tab_offset))
        {
            AURA_FREE(ctx, x_tab);
            AURA_FREE(ctx, y_tab);
        ...
        MI_S32 x_tab_size = GetAreaOffset(iwidth, owidth, C, scale_x, x_tab);
        MI_S32 y_tab_size = GetAreaOffset(iheight, oheight, 1, scale_y, y_tab);
        MI_S32 dy = 0;
        for (MI_S32 k = 0; k < y_tab_size; k++)
        {
        ...
        ThreadBuffer thread_buffer(ctx, C * owidth * 2 * sizeof(MI_F32));
    ...
    {
        const MI_S32 owidth_x8 = owidth << 3;

        MI_S32 table_size = sizeof(MI_S32) * ((owidth * 16) + (iheight * 4) + (oheight * 2));
        MI_S32 *table_data = static_cast<MI_S32*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, table_size, 0));
        if (MI_NULL == table_data)
        {
            AURA_ADD_ERROR_STRING(ctx, "AURA_ALLOC_PARAM failed");
            return Status::ERROR;
        }

        MI_S32 *x_idx   = table_data;
        MI_F32 *x_coef  = reinterpret_cast<MI_F32*>(x_idx + owidth_x8);
        MI_S32 *y_idx   = reinterpret_cast<MI_S32*>(x_coef + owidth_x8);
        MI_F32 *y_coef  = reinterpret_cast<MI_F32*>(y_idx + iheight * 2);
        MI_S32 *y_table = reinterpret_cast<MI_S32*>(y_coef + iheight * 2);
        ...
        ThreadBuffer thread_buffer(ctx, sizeof(MI_F32) * owidth * 4 * C);
        ...
        if (ret != Status::OK)
        {
            AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonImpl<Tp, MI_F32, C> failed");
        }
...
Status ResizeAreaCommNeon(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    switch (pattern)
    {
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U8, 1>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U8, C1");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S8, 1>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S8, C1");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U16, 1>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U16, C1");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S16, 1>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S16, C1");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_F32, 1>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_F32, C1");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U8, 2>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U8, C2");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S8, 2>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S8, C2");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U16, 2>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U16, C2");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S16, 2>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S16, C2");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_F32, 2>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_F32, C2");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U8, 3>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U8, C3");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S8, 3>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S8, C3");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_U16, 3>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_U16, C3");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_S16, 3>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_S16, C3");
            }
            break;
        }
        ...
        {
            ret = ResizeAreaCommNeonHelper<MI_F32, 3>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeAreaCommNeonHelper failed, type:MI_F32, C3");
            }
            break;
        }
