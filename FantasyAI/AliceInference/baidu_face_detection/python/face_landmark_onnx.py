...
class FaceDetectOnnx:
    ...
    def getfacebox(self, image):
        ...
        print("box_rslt:", l, t, r, b)
        # cv.imshow('preds', image_disp)
        # cv.waitKey(1)
        return boxes_rslt


def main_facept_video():
    facedetect = FaceDetectOnnx(pathModel="../onnx_model/FaceDetection0412V4Main_quant.onnx")
    net_inputH = 128
    net_inputW = 128
    ...
    # ret, img = cap.read()
    # img = cv.imread("")
    img = cv.imread("demo_landmark.jpg")
    # imgW = image.shape[1]
    # imgH = image.shape[0]
    ...
    for boxitem in boxRslt:
        ...
        if id % 2 == 0:
            ...
            cv.putText(img_draw, "facecls:" + str(round(facecls, 3)), (100, 170), 1, 2, (0, 0, 255), 2, 8,
                       0)  # 建议分值取>=0.1
            cv.putText(img_draw, "eye:" + str(round(eye_status, 3)), (100, 210), 1, 2, (0, 0, 255), 2, 8,
                       0)  # 睁闭眼判别阈值，建议取
