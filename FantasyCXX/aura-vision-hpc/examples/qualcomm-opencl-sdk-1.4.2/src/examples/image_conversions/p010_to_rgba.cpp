//--------------------------------------------------------------------------------------
// File: p010_to_rgba.cpp
// Desc: This program converts a P010 image to RGBA8888
//
// Author:      QUALCOMM
//
...
static const char *PROGRAM_SOURCE = R"(
    __kernel void
    p010_to_rgba(__read_only  image2d_t input_p010,
                              ...
                              sampler_t sampler)
    {
        ...
        yuv    = read_imagef(input_p010,  sampler, coord);
...
int main(int argc, char** argv)
{
    if (argc < 3)
    {
        std::cerr << "Usage: " << argv[0] << " <input image> <output image> \n"
                     ...
                     "Demonstrates conversions from P010 to RGBA8888.\n";
        return EXIT_FAILURE;
    }
    ...
    cl_kernel           p010_to_rgba        = wrapper.make_kernel("p010_to_rgba", program);
    ...
    out_rgba_desc.image_width            = src_p010_image_info.y_width;
    out_rgba_desc.image_height           = src_p010_image_info.y_height;
    ...
    // P010 to RGBA
    err = clSetKernelArg(p010_to_rgba, 0, sizeof(src_p010_image), &src_p010_image);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 0 for p010_to_rgba kernel." << "\n";
        return err;
    }

    err = clSetKernelArg(p010_to_rgba, 1, sizeof(out_rgba_image), &out_rgba_image);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 2 for p010_to_rgba kernel." << "\n";
        return err;
    }

    err = clSetKernelArg(p010_to_rgba, 2, sizeof(sampler), &sampler);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 3 for p010_to_rgba kernel." << "\n";
        return err;
    }
    ...
    err = clEnqueueNDRangeKernel(command_queue, p010_to_rgba, 2, nullptr, work_size, nullptr, 0, nullptr, nullptr);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clEnqueueNDRangeKernel for p010_to_rgba kernel." << "\n";
        return err;
    }
