...
kernel void SumMain(global St *src, int istep, 
                    global SumType *dst,
                    ...
                    int width,   int height)
{
    ...
    for (int tid = local_id; tid < blk_size; tid += local_size)
    {
        ...
        global St *src_data = src + mad24((idy + offset_y), istep, idw);

        if (idw + ELEM_COUNTS <= width)
        {
            VSt vst_src = VLOAD(src_data, ELEM_COUNTS);
            VSum vsum   = CONVERT(vst_src, VSum);
        ...
        else
        {
            ...
            {
                sum += CONVERT(src_data[i], SumType);
            }
            local_sum[local_id] += sum;
        }
    }
