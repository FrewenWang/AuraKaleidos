...
static constexpr MI_S8 tbl_buffer0[8] = {0, 1, 2, 3,  5,  6,  7,  8};
static constexpr MI_S8 tbl_buffer1[8] = {2, 3, 4, 5,  7,  8,  9, 10};
static constexpr MI_S8 tbl_buffer2[8] = {4, 5, 6, 7,  9, 10, 11, 12};
static constexpr MI_S8 tbl_buffer3[8] = {6, 7, 8, 9, 11, 12, 13, 14};

static constexpr MI_U8 operand_buffer[8]        = {0x03, 0x0C, 0x30, 0xC0, 0x03, 0x0C, 0x30, 0xC0};
static constexpr MI_S8 unpack_shift_buffer[8]   = {0, -2, -4, -6, 0, -2, -4, -6};
static constexpr MI_S8 pack_shift_buffer[8]     = {0,  2,  4,  6, 0,  2,  4,  6};

static Status MipiPackNeonImpl(const Mat &src, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    ...
    uint8x8_t vdu8_mask;
    neon::vdup(vdu8_mask, static_cast<MI_U8>(3));
    ...
    for (MI_S32 y = start_row; y < end_row; y++)
    {
        ...
        for (; x < width_align; x += 8)
        {
            ...
            MI_U8 pixel5  = neon::vgetlane<0>(vdu8_sum);
            MI_U8 pixel10 = neon::vgetlane<1>(vdu8_sum);
...
static Status MipiUnpackU8NeonImpl(const Mat &src, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
...
static Status MipiUnpackU16NeonImpl(const Mat &src, Mat &dst, MI_S32 start_row, MI_S32 end_row)
{
    MI_S32 width       = dst.GetSizes().m_width;
    MI_S32 width_align = width & (-32);
    ...
    {
        const MI_U8 *src_row = src.Ptr<MI_U8>(y);
        MI_U16      *dst_row = dst.Ptr<MI_U16>(y);
        ...
        for (; x < width; x += 4)
        {
            MI_U16 data0 = *(src_row + 0);
            MI_U16 data1 = *(src_row + 1);
            MI_U16 data2 = *(src_row + 2);
            MI_U16 data3 = *(src_row + 3);
            MI_U16 data4 = *(src_row + 4);
