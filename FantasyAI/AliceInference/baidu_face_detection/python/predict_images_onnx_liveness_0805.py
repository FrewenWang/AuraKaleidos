...
class DMSDanger(object): #GetValidDatasets
    ...
    def predict_camera(self, LiveNet, ort_sess_faceDetect, ort_sess_faceLandmark, use_onnx_model=False):
        """
        :param net:
        :param path:
        :return:
        ...
        while video_capture.isOpened():
            ...
            if ret:
                ...
                #print('faceRectsFinal', faceRectsFinal)
                ...
                img_net = img_crop.transpose(2, 0, 1)
                
                img_net = np.expand_dims(img_net, axis=0)
                img_net = img_net.astype(np.float32)

                img_net1 = {LiveNet.get_inputs()[0].name: img_net}
                pred_class = LiveNet.run(None, img_net1)
                prob = np.exp(pred_class) / np.exp(pred_class).sum()  # 归一化
                prob = prob[0][0]
                thePredict = prob.argsort()[-1]

                #  live:0  attack:1 
                if prob[0] > 0.8:  # 活体大于0.8才是活体，否则是攻击
                    thePredict = 0
                else:
                    thePredict = 1
                    
                print('thePredict', thePredict, pred_class, prob)  
...
if __name__ == '__main__':
    ...
    onnx_file_faceDetect = "./FaceDetect/FaceDetection0114V4Main.onnx"
    onnx_facept = "./FaceLandmark/FacialLandmark220324V25Main.onnx"
    ...
    num_classes = 2
    use_onnx_model = True  # 使用onnx模型

    model_path = './model/Liveness0805V6MainRGBQATFuseBnFuse1Layer_sim_skipbn.onnx'
    net = onnxruntime.InferenceSession(model_path)
 
    #---------------run camera--------------------'
    # camera
    Danger.predict_camera(net, ort_sess_faceDetect, ort_sess_faceLandmark, use_onnx_model)
