...
const static DT_S8 g_kernel_tabel[5][5] =
{
    { 1,  4,  6,  4, 1},
    {-1, -2,  0,  2, 1},
    { 1,  0, -2,  0, 1},
...
AURA_ALWAYS_INLINE DT_VOID Sobel5x5VCore(HVX_Vector &vu8_src_p1, HVX_Vector &vu8_src_p0, HVX_Vector &vu8_src_c,
                                         HVX_Vector &vu8_src_n0, HVX_Vector &vu8_src_n1, HVX_VectorPair &ws16_sum, const DT_U8 *kernel_y)
{
    DT_U32 k0k0k0k0 = Q6_R_vsplatb_R(kernel_y[0]);
    DT_U32 k2k1k2k1 = (kernel_y[2] << 24) | (kernel_y[1] << 16) | (kernel_y[2] << 8) | kernel_y[1];
    DT_U32 k4k3k4k3 = (kernel_y[4] << 24) | (kernel_y[3] << 16) | (kernel_y[4] << 8) | kernel_y[3];
...
// using St = DT_U8, Dt = S16
template <typename St, typename Dt, typename std::enable_if<std::is_same<St, DT_U8>::value && std::is_same<Dt, DT_S16>::value>::type* = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID Sobel5x5HCore(HVX_VectorPair &ws16_sum_x0, HVX_VectorPair &ws16_sum_x1, HVX_VectorPair &ws16_sum_x2,
                                         HVX_Vector &vs16_result_lo, HVX_Vector &vs16_result_hi, const DT_S8 *kernel_x)
{
    ...
    DT_U32 k3k3k3k3            = Q6_R_vsplatb_R(kernel_x[3]);
    DT_U32 k4k4k4k4            = Q6_R_vsplatb_R(kernel_x[4]);
...
template <typename St, typename Dt, typename std::enable_if<std::is_same<St, DT_U8>::value && std::is_same<Dt, DT_S16>::value>::type* = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID Sobel5x5HCore(HVX_VectorPair &ws16_sum_x0, HVX_VectorPair &ws16_sum_x1, HVX_VectorPair &ws16_sum_x2, HVX_VectorPair &ws16_sum_x3,
...
static DT_VOID Sobel5x5Row(const St *src_p1, const St *src_p0, const St *src_c, const St *src_n0, const St *src_n1, Dt *dst,
                           const DT_S8 *kernel_x, const DT_S8 *kernel_y, DT_F32 scale, const std::vector<St> &border_value, DT_S32 width)
{
    ...
    // left border
    {
        ...
        for (DT_S32 ch = 0; ch < C; ch++)
        {
            ...
            Sobel5x5VCore<St, Dt>(v_p1_border, v_p0_border, v_c_border, v_n0_border, v_n1_border, mw_sum_x0.val[ch], (DT_U8 *)kernel_y);
            Sobel5x5VCore<St, Dt>(mv_src_p1.val[ch], mv_src_p0.val[ch], mv_src_c.val[ch], mv_src_n0.val[ch], mv_src_n1.val[ch], mw_sum_x1.val[ch], (DT_U8 *)kernel_y);
        }
    }
    ...
    // main(0 ~ n-2)
    {
        for (DT_S32 x = SRC_ELEM_COUNTS; x <= back_offset; x += SRC_ELEM_COUNTS)
        {
            ...
            for (DT_S32 ch = 0; ch < C; ch++)
            {
                Sobel5x5VCore<St, Dt>(mv_src_p1.val[ch], mv_src_p0.val[ch], mv_src_c.val[ch], mv_src_n0.val[ch], mv_src_n1.val[ch], mw_sum_x2.val[ch], (DT_U8 *)kernel_y);
    ...
    // remain
    {
        ...
        for (DT_S32 ch = 0; ch < C; ch++)
        {
            ...
            Sobel5x5VCore<St, Dt>(mv_src_p1.val[ch], mv_src_p0.val[ch], mv_src_c.val[ch], mv_src_n0.val[ch], mv_src_n1.val[ch], mw_sum_x2.val[ch], (DT_U8 *)kernel_y);
            Sobel5x5VCore<St, Dt>(v_p1_border, v_p0_border, v_c_border, v_n0_border, v_n1_border, mw_sum_x3.val[ch], (DT_U8 *)kernel_y);
...
template <typename St, typename Dt, BorderType BORDER_TYPE, DT_S32 C, DT_BOOL WITH_SCALE>
static Status Sobel5x5HvxImpl(const Mat &src, Mat &dst, DT_S8 *kernel_x, DT_S8 *kernel_y, DT_F32 scale, const std::vector<St> &border_value,
                              const St *border_buffer, DT_S32 start_row, DT_S32 end_row)
{
    ...
    for (DT_S32 y = start_row; y < end_row; y++)
    {
        if (y + 3 < height)
        {
            L2Fetch(reinterpret_cast<DT_U32>(src.Ptr<St>(y + 3)), L2fetch_param);
        }
...
static Status Sobel5x5HvxHelper(Context *ctx, const Mat &src, Mat &dst, DT_S32 dx, DT_S32 dy, DT_F32 scale,
                                const std::vector<St> &border_value, const St *border_buffer)
{
    ...
    {
        ret = wp->ParallelFor((DT_S32)0, height, Sobel5x5HvxImpl<St, Dt, BORDER_TYPE, C, DT_FALSE>,
                              std::cref(src), std::ref(dst), kernel_x, kernel_y, scale, std::cref(border_value), border_buffer);
        if (ret != Status::OK)
        {
            AURA_ADD_ERROR_STRING(ctx, "Sobel5x5HvxImpl<St, Dt, BORDER_TYPE, C, DT_FALSE> failed");
        }
    }
    else
    {
        ret = wp->ParallelFor((DT_S32)0, height, Sobel5x5HvxImpl<St, Dt, BORDER_TYPE, C, DT_TRUE>,
                              std::cref(src), std::ref(dst), kernel_x, kernel_y, scale, std::cref(border_value), border_buffer);
        if (ret != Status::OK)
        {
            AURA_ADD_ERROR_STRING(ctx, "Sobel5x5HvxImpl<St, Dt, BORDER_TYPE, C, DT_TRUE> failed");
        }
    }
...
Status Sobel5x5Hvx(Context *ctx, const Mat &src, Mat &dst, DT_S32 dx, DT_S32 dy, DT_F32 scale,
                   BorderType border_type, const Scalar &border_value)
{
    Status ret = Status::ERROR;
    ...
    switch (pattern)
    {
        ...
        {
            ret = Sobel5x5HvxHelper<DT_U8, DT_S16>(ctx, src, dst, dx, dy, scale, border_type, border_value);
            break;
        }
        default:
        {
