...
template <typename Tp0, typename Tp1, typename Tp2, DT_S32 C, typename = typename std::enable_if<C <= 3, Tp0>::type>
static DT_VOID SqSumBorderImpl(const Tp0 *src_row, Tp1 *row_result, DT_S32 cur_x, DT_S32 row_elem_count, Scalar &result)
{
...
struct SqSumNeonFunctor
{
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        ...
        constexpr DT_S32 block_size = 1 << 15;
        constexpr DT_S32 block_step = load_size * block_size;
        constexpr DT_S32 block_vec_size = 4 / sizeof(Tp);
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            for (; x < row_elem_count; x += block_step)
            {
                ...
                }
                {
                    ...
                    for (DT_S32 i = 0; i < block_vec_size * 2; i += 4)
                    {
            ...
            SqSumBorderImpl<Tp, DT_U64, PromType0, C>(src_row, row_result, bx, row_elem_count, result);
        }
...
struct SqSumNeonFunctor<Tp, 3>
{
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        ...
        constexpr DT_S32 block_size = (1 << 11);
        constexpr DT_S32 block_step = load_size * block_size;
        constexpr DT_S32 block_vec_size = 4 / sizeof(Tp);
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            for (; x < row_elem_count; x += block_step)
            {
                DT_S32 blk_len = Min(width - x, block_step);
                DT_S32 blk_len_align = blk_len & (-load_size);
                ...
                for (DT_S32 i = 0; i < 3; ++i)
                {
                    ...
                    for (DT_S32 j = 0; j < block_vec_size * 2; j++)
                    {
                        row_result[i] += block_result[j];
                    }
                }
            }
            SqSumBorderImpl<Tp, DT_U64, PromType1, 3>(src_row, row_result, bx, row_elem_count, result);
        }
...
template <DT_S32 C>
struct SqSumNeonFunctor<DT_F32, C>
{
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        AURA_UNUSED(ctx);
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            SqSumBorderImpl<DT_F32, DT_F32, DT_F32, C>(src_row, row_result, x, row_elem_count, result);
        }
...
template <>
struct SqSumNeonFunctor<DT_F32, 3>
{
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        AURA_UNUSED(ctx);
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            SqSumBorderImpl<DT_F32, DT_F32, DT_F32, 3>(src_row, row_result, x, row_elem_count, result);
        }
    ...
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            for (; x < elem_count_align8; x += 8)
            {
                ...
                vqf32_row_sum = neon::vmla(vqf32_row_sum, neon::vcvt<DT_F32>(neon::vgethigh(vqf16_src_data)), neon::vcvt<DT_F32>(neon::vgethigh(vqf16_src_data)));
                vqf32_row_sum = neon::vmla(vqf32_row_sum, neon::vcvt<DT_F32>(neon::vgetlow(vqf16_src_data)), neon::vcvt<DT_F32>(neon::vgetlow(vqf16_src_data)));
            }
            neon::vstore(row_result, vqf32_row_sum);
            SqSumBorderImpl<MI_F16, DT_F32, DT_F32, C>(src_row, row_result, x, row_elem_count, result);
        }
    ...
    Status operator()(Context *ctx, const Mat &mat, std::vector<Scalar> &task_result, DT_S32 start_blk, DT_S32 end_blk)
    {
        ...
        for (DT_S32 y = start_row; y < end_row; ++y)
        {
            ...
            for (; x < elem_count_align24; x += 24)
            {
                ...
                for (DT_S32 i = 0; i < 3; ++i)
                {
                    v3qf32_row_sum.val[i] = neon::vmla(v3qf32_row_sum.val[i], neon::vcvt<DT_F32>(neon::vgethigh(v3qf16_src_data.val[i])), neon::vcvt<DT_F32>(neon::vgethigh(v3qf16_src_data.val[i])));
                    v3qf32_row_sum.val[i] = neon::vmla(v3qf32_row_sum.val[i], neon::vcvt<DT_F32>(neon::vgetlow(v3qf16_src_data.val[i])), neon::vcvt<DT_F32>(neon::vgetlow(v3qf16_src_data.val[i])));
                }
            }
            for (DT_S32 i = 0; i < 3; ++i)
            {
            ...
            SqSumBorderImpl<MI_F16, DT_F32, DT_F32, 3>(src_row, row_result, x, row_elem_count, result);
        }
...
Status SqSumNeon(Context *ctx, const Mat &mat, Scalar &result, const OpTarget &target)
{
    ...
    switch (mat.GetElemType())
    {
        case ElemType::U8:
        {
            ret = SqSumNeonHelper<DT_U8>(ctx, mat, result, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "SqSumNeonHelper<DT_U8> failed.");
            }
            break;
        }
        case ElemType::S8:
        {
            ret = SqSumNeonHelper<DT_S8>(ctx, mat, result, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "SqSumNeonHelper<DT_S8> failed.");
            }
            break;
        }
        case ElemType::U16:
        {
            ret = SqSumNeonHelper<DT_U16>(ctx, mat, result, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "SqSumNeonHelper<DT_U16> failed.");
            }
            break;
        }
        case ElemType::S16:
        {
            ret = SqSumNeonHelper<DT_S16>(ctx, mat, result, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "SqSumNeonHelper<DT_S16> failed.");
            }
            break;
        }
#if defined(AURA_ENABLE_NEON_FP16)
        ...
        case ElemType::F32:
        {
            ret = SqSumNeonHelper<DT_F32>(ctx, mat, result, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "SqSumNeonHelper<DT_F32> failed.");
            }
            break;
        }
        default:
