...
/// The kind of QoS configuration.
/// @note This enum is used by NeuronRuntime_setQoS.
typedef enum {
    /// The estimated inference from users.
    NEURONRUNTIME_INFERENCE_MS = 0,
    /// The tolerance time for model inferences.
    NEURONRUNTIME_TOLERANCE_MS,
} RuntimeAPIQoSKind;
...
typedef struct {
    ...
    /// Setting any non-zero value would notify the scheduler that this inference is
    /// a real-time task.
...
typedef enum {
    ...
    NEURONRUNTIME_RESOURCE_BUSY   = 7,  ///< 7: Device or resource busy.
} RuntimeAPIErrorCode;
