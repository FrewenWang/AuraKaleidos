...
static Status CvtColorNoneImpl(Context *ctx, const std::vector<const Mat*> &src, std::vector<Mat*> &dst, CvtColorType type, const OpTarget &target)
{
    ...
    switch (type)
    {
        ...
        {
            ret = CvtBgr2BgraNone(ctx, *(src[0]), *(dst[0]), SwapBlue(type), target);
            break;
        }
        ...
        {
            ret = CvtBgr2GrayNone(ctx, *(src[0]), *(dst[0]), SwapBlue(type), target);
            break;
        }
        ...
        {
            ret = CvtGray2BgrNone(ctx, *(src[0]), *(dst[0]), target);
            break;
        }
        ...
        {
            ret = CvtNv2RgbNone(ctx, *(src[0]), *(src[1]), *(dst[0]), SwapUv(type), type, target);
            break;
        }
        ...
        {
            ret = CvtY4202RgbNone(ctx, *(src[0]), *(src[1]), *(src[2]), *(dst[0]), SwapUv(type), type, target);
            break;
        }
        ...
        {
            ret = CvtY4442RgbNone(ctx, *(src[0]), *(src[1]), *(src[2]), *(dst[0]), type, target);
            break;
        }
        ...
        {
            ret = CvtRgb2NvNone(ctx, *(src[0]), *(dst[0]), *(dst[1]), SwapUv(type), type, target);
            break;
        }
        ...
        {
            ret = CvtRgb2Y420None(ctx, *(src[0]), *(dst[0]), *(dst[1]), *(dst[2]), SwapUv(type), type, target);
            break;
        }
        case CvtColorType::RGB2YUV_Y444:
        case CvtColorType::RGB2YUV_Y444_601:
        {
            ret = CvtRgb2Y444None(ctx, *(src[0]), *(dst[0]), *(dst[1]), *(dst[2]), type, target);
            break;
        }
        case CvtColorType::RGB2YUV_NV12_P010:
        case CvtColorType::RGB2YUV_NV21_P010:
        {
            ret = CvtRgb2NvP010None(ctx, *(src[0]), *(dst[0]), *(dst[1]), SwapUv(type), target);
            break;
        }
        ...
        {
            ret = CvtBayer2BgrNone(ctx, *(src[0]), *(dst[0]), SwapBlue(type), SwapGreen(type), target);
            break;
        }
...
CvtColorNone::CvtColorNone(Context *ctx, const OpTarget &target) : CvtColorImpl(ctx, target)
{}

Status CvtColorNone::SetArgs(const std::vector<const Array*> &src, const std::vector<Array*> &dst, CvtColorType type)
{
...
Status CvtColorNone::Run()
{
    ...
    ret = CvtColorNoneImpl(m_ctx, src, dst, m_type, m_target);
