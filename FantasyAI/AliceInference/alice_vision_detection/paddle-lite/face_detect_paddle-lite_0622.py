...
def predict_image_onnx(ort_sess):
    """
    :param ort_sess: onnx runtime session
    :return:
    """
    image = cv2.imread("../../res/gui.jpg")
    img_w = image.shape[1]
    img_h = image.shape[0]
    ...
    boxes = copy.deepcopy(box_result)
    # print (boxes)
    box_dst = []
    for i, pred in enumerate(boxes):
        if pred[1] >= 0.35:
            ...
            cv2.rectangle(image_display, (l, t), (r, b), (0, 255, 255), 2, 8, 0)
            fscore = pred[1]
            fscore = round(fscore, 3)
            cv2.putText(image_display, str(fscore) + "|" + str(r - l) + "|" + str(b - t), (l, t), 1, 3, (0, 0, 255), 3,
                        8,
                        0)
    print("boxdst:", box_dst)
    cv2.imshow('preds', image_display)
    cv2.waitKey(0)


"""
人脸检测的paddle-lite的使用的python代码：
# https://paddle-lite.readthedocs.io/zh/develop/user_guides/python_demo.html
"""


def predict_image_paddle_lite(model_file, param_file):
    # 1. Set config information
    # 1. Set config information
    # config = MobileConfig()
    # # 2. Set the path to the model generated by opt tools
    # if model_file != '' and param_file != '':
    #     config.set_model_from_file(model_file)
    #     # config.set_param_file(param_file)
    # else:
    #     print("model file and param file empty")
    ...
    image[0][0] = image_origin
    # 转化成为onnx runtime的输入数据
    # input_tensor = {predictor.get_input(0).name: image}

    # 执行模型推理
    # predictor.run()

    # output_tensor = predictor.get_output(0)
    # head_output = output_tensor.numpy()
    # print(head_output)
    ...
    # 直接读取推理后处理结果
    head_output = []
    data0 = np.fromfile("../../res/face_rect0.raw", dtype=np.float32)
    data1 = np.fromfile("../../res/face_rect1.raw", dtype=np.float32)
    data2 = np.fromfile("../../res/face_rect2.raw", dtype=np.float32)
    head_output.append(data0)
    head_output.append(data1)
    head_output.append(data2)
...
if __name__ == "__main__":
    model_file = "../../models/onnx_models/FaceDetection0620V4Main_quant.onnx"
    ...
    # 使用paddle-lite进行推理
    param_file = "../../models/paddle-lite_models/x86/FaceDetection0620V4MainQuant.nb"
    predict_image_paddle_lite(model_file, param_file)
