...
const static MI_S8 g_kernel_tabel[4][3] =
{
    { 1,  2, 1},
    {-1,  0, 1},
    { 1, -2, 1},
...
AURA_ALWAYS_INLINE AURA_VOID Sobel3x3VCore(HVX_Vector &vu8_src_p0, HVX_Vector &vu8_src_c, HVX_Vector &vu8_src_n0,
                                         HVX_VectorPair &ws16_sum, const MI_U8 *kernel_y)
{
    MI_U32 k0k0k0k0 = Q6_R_vsplatb_R(kernel_y[0]);
    MI_S32 k2k1k2k1 = (kernel_y[2] << 24) | (kernel_y[1] << 16) | (kernel_y[2] << 8) | kernel_y[1];
...
// using St = MI_U8, Dt = S16
template <typename St, typename Dt, typename std::enable_if<std::is_same<St, MI_U8>::value && std::is_same<Dt, MI_S16>::value>::type* = MI_NULL>
AURA_ALWAYS_INLINE AURA_VOID Sobel3x3HCore(HVX_VectorPair &ws16_sum_x0, HVX_VectorPair &ws16_sum_x1, HVX_VectorPair &ws16_sum_x2,
...
template <typename St, typename Dt, typename std::enable_if<std::is_same<St, MI_U8>::value && std::is_same<Dt, MI_S16>::value>::type* = MI_NULL>
AURA_ALWAYS_INLINE AURA_VOID Sobel3x3HCore(HVX_VectorPair &ws16_sum_x0, HVX_VectorPair &ws16_sum_x1, HVX_VectorPair &ws16_sum_x2, HVX_VectorPair &ws16_sum_x3,
...
static AURA_VOID Sobel3x3Row(const St *src_p0, const St *src_c, const St *src_n0, Dt *dst, const MI_S8 *kernel_x,
                           const MI_S8 *kernel_y, MI_F32 scale, const std::vector<St> &border_value, MI_S32 width)
{
    ...
    // left border
    {
        ...
        for (MI_S32 ch = 0; ch < C; ch++)
        {
            ...
            Sobel3x3VCore<St, Dt>(v_p0_border, v_c_border, v_n0_border, mw_sum_x0.val[ch], (MI_U8 *)kernel_y);
            Sobel3x3VCore<St, Dt>(mv_src_p0.val[ch], mv_src_c.val[ch], mv_src_n0.val[ch], mw_sum_x1.val[ch], (MI_U8 *)kernel_y);
        }
    }
    ...
    // main(0 ~ n-2)
    {
        for (MI_S32 x = SRC_ELEM_COUNTS; x <= back_offset; x += SRC_ELEM_COUNTS)
        {
            ...
            for (MI_S32 ch = 0; ch < C; ch++)
            {
                Sobel3x3VCore<St, Dt>(mv_src_p0.val[ch], mv_src_c.val[ch],  mv_src_n0.val[ch], mw_sum_x2.val[ch],    (MI_U8 *)kernel_y);
    ...
    // remain
    {
        ...
        for (MI_S32 ch = 0; ch < C; ch++)
        {
            ...
            Sobel3x3VCore<St, Dt>(mv_src_p0.val[ch], mv_src_c.val[ch], mv_src_n0.val[ch], mw_sum_x2.val[ch], (MI_U8 *)kernel_y);
            Sobel3x3VCore<St, Dt>(v_p0_border, v_c_border, v_n0_border, mw_sum_x3.val[ch], (MI_U8 *)kernel_y);
...
template <typename St, typename Dt, BorderType BORDER_TYPE, MI_S32 C, MI_BOOL WITH_SCALE>
static Status Sobel3x3HvxImpl(const Mat &src, Mat &dst, MI_S8 *kernel_x, MI_S8 *kernel_y, MI_F32 scale, const std::vector<St> &border_value,
...
static Status Sobel3x3HvxHelper(Context *ctx, const Mat &src, Mat &dst, MI_S32 dx, MI_S32 dy, MI_F32 scale,
                                const std::vector<St> &border_value, const St *border_buffer)
{
    ...
    {
        ret = wp->ParallelFor((MI_S32)0, height, Sobel3x3HvxImpl<St, Dt, BORDER_TYPE, C, MI_FALSE>,
                              std::cref(src), std::ref(dst), kernel_x, kernel_y, scale, std::cref(border_value), border_buffer);
        if (ret != Status::OK)
        {
            AURA_ADD_ERROR_STRING(ctx, "Sobel3x3HvxImpl<St, Dt, BORDER_TYPE, C, MI_FALSE> failed");
        }
    }
    else
    {
        ret = wp->ParallelFor((MI_S32)0, height, Sobel3x3HvxImpl<St, Dt, BORDER_TYPE, C, MI_TRUE>,
                              std::cref(src), std::ref(dst), kernel_x, kernel_y, scale, std::cref(border_value), border_buffer);
        if (ret != Status::OK)
        {
            AURA_ADD_ERROR_STRING(ctx, "Sobel3x3HvxImpl<St, Dt, BORDER_TYPE, C, MI_TRUE> failed");
        }
    }
                   ...
                   BorderType border_type, const Scalar &border_value)
{
    ...
    switch (pattern)
    {
        ...
        {
            ret = Sobel3x3HvxHelper<MI_U8, MI_S16>(ctx, src, dst, dx, dy, scale, border_type, border_value);
            break;
        }
        default:
        {
