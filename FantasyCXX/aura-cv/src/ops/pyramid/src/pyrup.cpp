...
static std::shared_ptr<PyrUpImpl> CreatePyrUpImpl(Context *ctx, const OpTarget &target)
{
    std::shared_ptr<PyrUpImpl> impl;

    switch (target.m_type)
    {
        case TargetType::NONE:
        {
            impl.reset(new PyrUpNone(ctx, target));
            break;
        }
        ...
        case TargetType::NEON:
        {
            ...
            impl.reset(new PyrUpNeon(ctx, target));
        ...
        case TargetType::OPENCL:
        {
            ...
            impl.reset(new PyrUpCL(ctx, target));
        ...
        case TargetType::HVX:
        {
            ...
            impl.reset(new PyrUpHvx(ctx, target));
...
Status PyrUp::SetArgs(const Array *src, Array *dst, DT_S32 ksize, DT_F32 sigma, BorderType border_type)
{
    ...
    Status ret = pyrup_impl->SetArgs(src, dst, ksize, sigma, border_type);
...
Status PyrUp::CLPrecompile(Context *ctx, ElemType elem_type, DT_S32 channel, DT_S32 ksize, BorderType border_type)
{
    ...
    std::vector<CLKernel> cl_kernels = PyrUpCL::GetCLKernels(ctx, elem_type, channel, ksize, border_type);

    if (CheckCLKernels(ctx, cl_kernels) != Status::OK)
    {
        AURA_ADD_ERROR_STRING(ctx, "PyrUp CheckCLKernels failed");
        return Status::ERROR;
    }
#else
    AURA_UNUSED(ctx);
...
Status PyrUpImpl::SetArgs(const Array *src, Array *dst, DT_S32 ksize, DT_F32 sigma, BorderType border_type)
{
    ...
    {
        AURA_ADD_ERROR_STRING(m_ctx, "Unsupported elem format");
        return Status::ERROR;
    }
    ...
    if ((src_sizes.m_channel      != dst_sizes.m_channel) || (src_sizes.m_channel     != 1) ||
        (src_sizes.m_height << 1) != dst_sizes.m_height   || (src_sizes.m_width << 1) != dst_sizes.m_width)
    {
...
std::vector<const Array*> PyrUpImpl::GetInputArrays() const
{
    return {m_src};
}

std::vector<const Array*> PyrUpImpl::GetOutputArrays() const
{
    return {m_dst};
}

std::string PyrUpImpl::ToString() const
{
    std::string str;
