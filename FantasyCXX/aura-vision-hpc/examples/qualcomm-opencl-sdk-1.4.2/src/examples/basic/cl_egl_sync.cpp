...
// Desc: This example demonstrates the usage of sync between cl and gl/egl. Creates an
//       RGB format 2D texture as a source image on GL side. Creates image targets in CL
//       for RGB formats. In the kernel, applies the grayscale algorithm and copies to
//       destination image. Pass the destination image back to GL side for drawing on to
//       off screen buffer.
//
// Author: QUALCOMM
//
...
static const rgb_unit RGB[3] = {{255, 0 , 0}, {0, 0 , 255}, {0, 255 , 0}};
...
GLuint load_shader(GLenum shader_type, const char* p_source) {
...
int main(int argc, char* argv[])
{
    ...
    EGLBoolean return_value = eglInitialize(egl_display, &egl_version_major, &egl_version_minor);
    ...
    PFNEGLCREATESYNCPROC p_eglCreateSync = (PFNEGLCREATESYNCPROC)eglGetProcAddress("eglCreateSync");
    PFNEGLWAITSYNCPROC p_eglWaitSync = (PFNEGLWAITSYNCPROC)eglGetProcAddress("eglWaitSync");
    PFNEGLDESTROYSYNCPROC p_eglDestroySync = (PFNEGLDESTROYSYNCPROC)eglGetProcAddress("eglDestroySync");
    ...
    GLuint pixel_shader = load_shader(GL_FRAGMENT_SHADER, FRAGMENT_SHADER_SOURCE);
    ...
    if (shader_program)
    {
        ...
        if (linkStatus != GL_TRUE)
        {
            ...
            if (buf_length)
            {
                ...
                if (buf)
                {
                    glGetProgramInfoLog(shader_program, buf_length, nullptr, buf);
    ...
    glVertexAttribPointer(position_handle, 3, GL_FLOAT, GL_FALSE, TRIANGLE_VERTICES_DATA_STRIDE_BYTES, TRIANGLE_VERTICES_DATA);
    GLuint texture_handle = glGetAttribLocation(shader_program, "texCoords");
    glVertexAttribPointer(texture_handle, 2, GL_FLOAT, GL_FALSE, TRIANGLE_VERTICES_DATA_STRIDE_BYTES, &TRIANGLE_VERTICES_DATA[3]);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, IMAGE_WIDTH, IMAGE_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, input);
    ...
    cl_mem shared_mem[2];
    shared_mem[0] = clCreateFromGLTexture(context, CL_MEM_READ_WRITE, GL_TEXTURE_2D, 0, input_texture, &creturn);
    if (creturn != 0)
    {
        std::cerr << "clCreateFromGLTexture failed with error " << creturn << "\n";
    }
    shared_mem[1] = clCreateFromGLTexture(context, CL_MEM_READ_WRITE, GL_TEXTURE_2D, 0, output_texture, &creturn);
    if (creturn != 0)
    {
        std::cerr << "clCreateFromGLTexture failed with error " << creturn << "\n";
    }
    ...
    cl_event cl_event_cl_done = nullptr, cl_event_gl_done = nullptr;
    ...
    for (frame_num = 0; frame_num < FRAME_COUNT; frame_num++)
    {
        if(cl_event_cl_done)
        {
            ...
            {
                EGL_CL_EVENT_HANDLE_KHR,   static_cast<EGLAttrib>((EGLAttrib) cl_event_cl_done),
                EGL_NONE
            };

            EGLSync_cl_done = p_eglCreateSync(egl_display, EGL_SYNC_CL_EVENT_KHR, fence_attribs);
            eglWaitSync(egl_display, EGLSync_cl_done, 0);

            p_eglDestroySync(egl_display, EGLSync_cl_done);
            clReleaseEvent(cl_event_cl_done);

            cl_event_cl_done = nullptr;
        }

        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

        EGLSync_gl_done = p_eglCreateSync(egl_display, EGL_SYNC_FENCE, nullptr);
        cl_event_gl_done = clCreateEventFromEGLSyncKHR(context, EGLSync_gl_done, egl_display, &creturn);
        creturn |= clEnqueueAcquireGLObjects(queue, 2, shared_mem, 1, &cl_event_gl_done, nullptr);
        creturn |= clEnqueueNDRangeKernel(queue, kernel, 2, nullptr, wg_size, nullptr, 0, nullptr, nullptr);
        creturn |= clEnqueueReleaseGLObjects(queue, 2, shared_mem, 0, nullptr, &cl_event_cl_done);
        if (creturn != CL_SUCCESS)
        {
