...
def compute_boxIOU(box1, box_exist):
    if len(box_exist) == 0:
        return 0

    maxIOU = 0
    l1,t1,r1,b1 = box1
    for boxtmp in box_exist:
        l2, t2, r2, b2 = boxtmp
        ...
        if r<=l or b<=t:
            ...
            area1 = (r1 - l1) * (b1 - t1) * 1.0
            area2 = (r2 - l2) * (b2 - t2) * 1.0
            area_inter = (r - l) * (b - t) * 1.0
            iouTmp = area_inter/(area1 + area2 - area_inter)
        if iouTmp > maxIOU:
            maxIOU = iouTmp
    return maxIOU

#box1 是新图像的box位置，不是手的box， 如果已经贴上的手被新的方框覆盖，则不可取
def compute_imgwh_boxIOU(box1, box_exist):
    if len(box_exist) == 0:
        return 0
...
def getAllFiles(path):
    allFiles = []
    for root, foldert, files in os.walk(path):
        for filet in files:
