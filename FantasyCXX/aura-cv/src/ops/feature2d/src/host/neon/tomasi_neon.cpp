                                 ...
                                 DT_S32 gradient_size, DT_BOOL use_harris, DT_F64 harris_k, const OpTarget &target)
{
    ...
    {
        const DT_F32 *eigen_data = eigen.Ptr<DT_F32>(y);
        DT_S32 x = 0;
    ...
    for (DT_S32 y = 0; y < height; y++)
    {
        DT_F32 *eigen_data = eigen.Ptr<DT_F32>(y);
        DT_S32 x = 0;
        for (; x < width_align4; x += 4)
        {
            float32x4_t vqf32_src  = neon::vload1q(eigen_data);
        ...
        for (; x < width; x++)
        {
            DT_F32 val = *eigen_data > thresh ? *eigen_data : 0.f;
            *eigen_data++ = val;
        }
    }
    ...
    std::vector<const DT_F32*> tmp_corners;
    ...
    DT_S32 eigen_step       = eigen.GetStrides().m_width;
    const DT_U8 *eigen_data = eigen.Ptr<DT_U8>(0);
    ...
    if (min_distance >= 1)
    {
        ...
        {
            DT_S32 offset = reinterpret_cast<const DT_U8*>(tmp_corners[i]) - eigen_data;
            DT_S32 y      = offset / eigen_step;
            DT_S32 x      = (offset - y * eigen_step) / sizeof(DT_F32);
...
Status TomasiNeon::SetArgs(const Array *src, std::vector<KeyPoint> &key_points, DT_S32 max_num_corners,
