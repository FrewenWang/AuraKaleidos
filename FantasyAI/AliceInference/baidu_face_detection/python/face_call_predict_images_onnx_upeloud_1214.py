...
import face_call_facedetect_onnx_1107 as facedetect_onnx
import face_call_main_facept_onnx_1025 as facelandmark
...
#         count_img = 0
#         save_num = 0
#         with open('./0325_DataList_class3_xxshuffle_Test.txt', 'r') as fr:
#             lines = fr.readlines()
#             for line in lines:
#                 save_num += 1
#                 filepath, labels = line.split('\t')
#                 label_left, label_right = eval(labels)
#                 if label_left != 'hand' and label_right != 'hand':
#                     if save_num % 100 != 0:
#                         continue
...
#                 if 1:
        ...
        dir_path = '../../res/face_call'
        count_img = 0
        call_img = 0
        for root, dirs, files in os.walk(dir_path):
            for i, fie in enumerate(files):
                if fie.endswith('jpg') or fie.endswith('png') or fie.endswith('jpeg') or fie.endswith('bmp') or fie.endswith('avi') or fie.endswith('mp4'):
                    filepath = os.path.join(root, fie)
#                     print('filepath', filepath)
                    ...
                    # 打印出来所有的人脸框坐标
                    print(faceRectsFinal)
                    if faceRectsFinal == []:
                        continue

                    points, angle = facelandmark.main_facept_img(ort_sess_faceLandmark, frame, faceRectsFinal[0])
                    if points == []:
                        continue

                    count_img += 1

                    pointsX = [x for x, y in points]
                    pointsY = [y for x, y in points]
                    faceW = max(pointsX) - min(pointsX)
                    faceH = max(pointsY) - min(pointsY)
                    if faceW < 80 or faceH < 80:
                        continue
                    ...
                    for directionIsLeft in range(2):
                        ...
                        # duiqi
#                         dst = filepath.replace('.jpg', '_crop_direction'+str(directionIsLeft)+'_predict'+str(thePredict)+'_prob'+str(prob.tolist()))
#                         if dst is not None:
#                             cv2.imwrite(dst+'.jpg', img_crop)


                        # 500image
#                         if directionIsLeft == 1:
#                             if label_left == 'call':
#                                 dst = './image/500/call/' + str(save_num)
#                             elif label_left == 'hand':
#                                 dst = './image/500/hand/' + str(save_num)
#                             elif label_left == 'normal':
#                                 dst = './image/500/normal/' + str(save_num)
#                             else:
#                                 dst = None
#                                 pass
#                         else:
#                             if label_right == 'call':
#                                 dst = './image/500/call/' + str(save_num)
#                             elif label_right == 'hand':
#                                 dst = './image/500/hand/' + str(save_num)
#                             elif label_right == 'normal':
#                                 dst = './image/500/normal/' + str(save_num)
#                             else:
#                                 dst = None
#                                 pass
...
#             else:
#                 break
        print('call_img, count_img', call_img, count_img, float(call_img)/float(count_img))
...
if __name__ == '__main__':
    ...
    onnx_call = '../models/onnx_models/DMSCall221214V7MainQAT_sim_skipbn.onnx'
