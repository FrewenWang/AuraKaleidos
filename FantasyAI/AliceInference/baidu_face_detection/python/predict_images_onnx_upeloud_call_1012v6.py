...
import facedetect_onnx_call_1012v6 as facedetect_onnx
import main_facept_onnx_call_1012v6 as facelandmark
...
class DMSDanger(object): #GetValidDatasets
    ...
    def predict_camera(self, ort_sess_call, ort_sess_faceDetect, ort_sess_faceLandmark):
        ...
        :return:
        """
        print("=========================predict_camera===================================")
        landmarks_right, landmarks_left, dst_w, dst_h = self.landmarks_call_train_bigRect()

        # dir_path = '/media/baidu/17d80214-bfce-43ec-807d-d6ba4e44dfd6/DangerCall_paddle/0902/image/bug'
        ...
        video_capture = cv2.VideoCapture("../res/d4c41c8975c2d1c2910273820.png")
            ...
            if ret:
                    if 1:
                        if 1:
                            ...
                            for faceRect in faceRectsFinal:
                                points, angle = facelandmark.main_facept_img(ort_sess_faceLandmark, frame, faceRect)
                                if points == []:
                                    continue
    ...
    def predict_image(self, ort_sess_call, ort_sess_faceDetect, ort_sess_faceLandmark):
        """
        :param net:
        :param path:
        :return:
        ...
        while True:
            ...
            if count == 2:
                exit()
            frame = cv2.imread("../res/d4c41c8975c2d1c2910273820.png")
            if 1:
                if 1:
                    if 1:
                        if frame is None:
                        ...
                        for faceRect in faceRectsFinal:
                            ...
                            for directionIsLeft in range(2):
                                if directionIsLeft == 1:
                                    # 人的左耳，主驾靠窗，（图片右侧的耳朵
                                    cor_img, cor_pts = self.alignWithLandmark(frame, np.array(points),
                                                                              np.array(landmarks_left),
                                                                              dst_w, dst_h)
                                else:
                                    # 人的右耳，不靠窗的一侧，（图片左侧的耳朵
                                    cor_img, cor_pts = self.alignWithLandmark(frame, np.array(points),
                                                                              np.array(landmarks_right),
                                                                              dst_w, dst_h)
...
if __name__ == '__main__':
    ...
    onnx_file_faceDetect = "../../models/onnx_models/FaceDetection0816V4Main_noquant.onnx"
    onnx_facept = "../models/onnx_models/FacialLandmark220816V25Main_noquant.onnx"
    onnx_call = '../models/onnx_models/DMSCall221012V6MainQAT_sim_skipbn.onnx'
    ...
    Danger.predict_image(ort_sess_call, ort_sess_faceDetect, ort_sess_faceLandmark)
