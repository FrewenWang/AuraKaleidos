...
from src.utils.general import (LOGGER, ROOT, check_requirements, check_suffix, check_version, colorstr, increment_path,
...
from src.utils.torch_utils import copy_attr, smart_inference_mode, time_sync
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs, triton = self._model_type(w)
...
class Decouple_TaskAlign(nn.Module):
    def __init__(self, c1, nc=12, na=1):  # ch_in, num_classes, num_anchors
        super().__init__()
        c_ = min(c1, 256)  # min(c1, nc * na)
        self.na = na  # number of anchors
        self.nc = nc  # number of classes
        self.stem = Conv(c1, c_, 1)
        self.cls_conv = Conv(c_, c_, 3)
        self.reg_conv = Conv(c_, c_, 3)

        self.cls_pred = nn.Conv2d(in_channels=c_, out_channels=self.nc * self.na, kernel_size=1)
        self.reg_pred = nn.Conv2d(in_channels=c_, out_channels=4 * self.na, kernel_size=1)
...
class Decouple_OTA(nn.Module):
    def __init__(self, c1, nc=12, na=1):  # ch_in, num_classes, num_anchors
        ...
        self.cls_pred = nn.Conv2d(in_channels=c_, out_channels=self.nc * self.na, kernel_size=1)
        self.reg_pred = nn.Conv2d(in_channels=c_, out_channels=4 * self.na, kernel_size=1)
        self.obj_pred = nn.Conv2d(in_channels=c_, out_channels=1 * self.na, kernel_size=1)

    def forward(self, x):
        x = self.stem(x)
        cls_output = self.cls_pred(self.cls_conv(x))
        reg_feat = self.reg_conv(x)
        reg_output = self.reg_pred(reg_feat)
...
class GSConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        super().__init__()
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, g, act=act)
        self.cv2 = Conv(c_, c_, 5, 1, c_, act=act)
...
class GSBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):
        ...
        self.conv_lighting = nn.Sequential(GSConv(c1, c_, 1, 1, act=True),
...
class ConvBNAct(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ksize, stride=1, act='relu', groups=1, bias=False, norm='bn', reparam=False,):
        super().__init__()
        # same padding
        pad = (ksize - 1) // 2
        self.conv = nn.Conv2d(
...
class GPFN_Block(nn.Module):
    def __init__(self, ch_in, ch_out, n, ch_hidden_ratio=1.0, act='swish', spp=False):
        ...
        for i in range(n):
            ...
            if i == (n - 1) // 2 and spp:
                self.convs.add_module('spp', SPP(ch_mid * 4, ch_mid, [5, 9, 13]))
            next_ch_in = ch_mid
...
class CSPNeXtBlock(nn.Module):
    def __init__(self, in_channels, out_channels, expansion=0.5, add_identity=True):
        super().__init__()
        hidden_channels = int(out_channels * expansion)
        self.conv1 = Conv(in_channels, hidden_channels, 3)
        self.conv2 = DWConv(hidden_channels, out_channels, 5)
...
class CSPNeXtLayer(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, add_identity=True, expand_ratio=0.5):
        super().__init__()
        mid_channels = int(out_channels * expand_ratio)
        self.main_conv = Conv(in_channels, mid_channels, 1)
        self.short_conv = Conv(in_channels, mid_channels, 1)
        self.final_conv = Conv(2 * mid_channels, out_channels, 1)
        self.blocks = nn.Sequential(*[CSPNeXtBlock(mid_channels, mid_channels, 1.0, add_identity) for _ in range(n)])
