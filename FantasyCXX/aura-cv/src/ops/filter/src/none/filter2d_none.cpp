...
static Status Filter2dNoneImpl(Context *ctx, const Mat &src, Mat &dst, const std::vector<DT_F32> &kernel,
                               DT_S32 ksize, DT_S32 start_row, DT_S32 end_row)
{
    ...
    for (DT_S32 y = start_row; y < end_row; y++)
    {
        ...
        for (DT_S32 x = 0; x < width; x++)
        {
            for (DT_S32 c = 0; c < channel; c++)
            {
                ...
                volatile DT_F32 result = 0.f;
                DT_S32 idx = x * channel + c;
                for (DT_S32 j = 0; j < ksize; j++)
                {
    ...
    {                                                                                                                       \
        ...
        {                                                                                                                   \
            ...
            return Status::ERROR;                                                                                           \
                                                                                                                            ...
                                                                                                                            \
        ret = wp->ParallelFor(static_cast<DT_S32>(0), oheight, Filter2dNoneImpl<type>, m_ctx, std::cref(m_src_border),      \
