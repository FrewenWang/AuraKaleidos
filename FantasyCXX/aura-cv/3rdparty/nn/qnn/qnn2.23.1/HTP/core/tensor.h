...
template <typename T> class ScaleOffsetInterface final : public Interface {
  ...
  public:
    ...
    {
        return unsigned(qp.offset) * 0x10661 ^ (iaura_convert<unsigned, float>(qp.scale) << 1);
    }
...
API_EXPORT inline void **make_blocktab_for_overwrite(const size_t n_blocks, hnnx::Crate *const crate_p)
{
    return crate_p->alloc_array<void *>(n_blocks);
}
...
using TypicalTensors =
        std::tuple<PlainFloatTensor, PlainFloatTensor5D, PlainFloat16Tensor, QuantUint8Tensor, QuantUint8Tensor5D,
                   ...
                   PlainFloat16Tensor5D, Int64Tensor>;
