...
kernel void GaussianRemain3x3C1(global St *src, int istep,
                                ...
                                struct Scalar border_value)
{
    ...
    v3it_src_p = GAUSSIAN_CONVERT(v3st_src_p, V3InterType);
    v3it_src_c = GAUSSIAN_CONVERT(v3st_src_c, V3InterType);
    v3it_src_n = GAUSSIAN_CONVERT(v3st_src_n, V3InterType);

    v3it_sum  = (v3it_src_p + v3it_src_n) * (V3InterType)filter[0] + v3it_src_c * (V3InterType)filter[1];
    it_result = (v3it_sum.s0 + v3it_sum.s2) * (InterType)filter[0] + v3it_sum.s1 * (InterType)filter[1];
