...
{
    // Tp = DT_U8 MI_F16 DT_F32, DT_U8 DT_F32 DT_F32
    using InterType = typename std::conditional<sizeof(Tp) == 1, DT_U8, DT_F32>::type;
    static constexpr DT_U32 Q = sizeof(Tp) == 1 ? 8 : 0;
};

template <typename Tp,
          typename std::enable_if<std::is_same<Tp, DT_U8>::value, Tp>::type* = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID BilateralNoneCore(DT_F32 *color_weight_data, DT_F32 &cur_color_weight, DT_F32 alpha, DT_S32 max_idx)
{
    AURA_UNUSED(max_idx);

    DT_S32 idx       = Round(alpha);
...
AURA_ALWAYS_INLINE DT_VOID BilateralNoneCore(DT_F32 *color_weight_data, DT_F32 &cur_color_weight, DT_F32 alpha, DT_S32 max_idx)
{
    DT_S32 idx       = Floor(alpha);
    alpha           -= idx;
    idx              = Clamp(idx, static_cast<DT_S32>(0), max_idx);
    cur_color_weight = color_weight_data[idx] + alpha * (color_weight_data[idx + 1] - color_weight_data[idx]);
}

template <typename Tp, DT_S32 C,
          typename std::enable_if<(C == 1), Tp>::type* = DT_NULL>
static Status BilateralNoneImpl(const Mat &src, Mat &dst, const Mat &space_weight, const Mat &space_ofs, const Mat &color_weight,
                                DT_S32 ksize, DT_F32 scale_index, DT_S32 valid_num, DT_S32 start_row, DT_S32 end_row)
{
    using InterType = typename BilateralTraits<Tp>::InterType;

    DT_F32 *space_weight_data = (DT_F32*)space_weight.Ptr<DT_F32>(0);
    DT_S32 *space_ofs_data    = (DT_S32*)space_ofs.Ptr<DT_S32>(0);
    DT_F32 *color_weight_data = (DT_F32*)color_weight.Ptr<DT_F32>(0);

    const DT_S32 width        = dst.GetSizes().m_width;
    const DT_S32 ksh          = ksize >> 1;
    const DT_S32 max_idx      = color_weight.GetSizes().m_width - 2;
    ...
    for (DT_S32 y = start_row; y < end_row; y++)
    {
        ...
        for (DT_S32 x = 0; x < width; x++)
        {
            ...
            dst_row[x] = ShiftSatCast<DT_F32, Tp, BilateralTraits<Tp>::Q << 1>(sum / weight_sum);
        }
    }
...
template <typename Tp, DT_S32 C,
          typename std::enable_if<(C == 3), Tp>::type* = DT_NULL>
static Status BilateralNoneImpl(const Mat &src, Mat &dst, const Mat &space_weight, const Mat &space_ofs, const Mat &color_weight,
                                DT_S32 ksize, DT_F32 scale_index, DT_S32 valid_num, DT_S32 start_row, DT_S32 end_row)
{
    using InterType = typename BilateralTraits<Tp>::InterType;
    ...
    for (DT_S32 y = start_row; y < end_row; y++)
    {
        ...
        for (DT_S32 x = 0; x < width; x++)
        {
            ...
            for (DT_S32 k = 0; k < valid_num; k++)
            {
                ...
                DT_F32 cur_color_weight = 0.f;
                DT_F32 alpha = (Abs(r1 - r0) + Abs(g1 - g0) + Abs(b1 - b0)) * scale_index;
                BilateralNoneCore<Tp>(color_weight_data, cur_color_weight, alpha, max_idx);

                DT_F32 weight = space_weight_data[k] * cur_color_weight;
                weight_sum   += weight;
                sum_r        += r1 * weight;
                sum_g        += g1 * weight;
                sum_b        += b1 * weight;
            ...
            DT_F32 weight_inv  = 1.f / weight_sum;
...
Status BilateralNone::SetArgs(const Array *src, Array *dst, DT_F32 sigma_color, DT_F32 sigma_space,
...
Status BilateralNone::Run()
{
    ...
    if ((DT_NULL == src_mat) || (DT_NULL == dst_mat))
    {
    ...
    DT_S32 oheight = dst_mat->GetSizes().m_height;
    ...
    {                                                                                                                             \
        ...
        {                                                                                                                         \
            ...
            return Status::ERROR;                                                                                                 \
                                                                                                                                  ...
                                                                                                                                  \
        ret = wp->ParallelFor(static_cast<DT_S32>(0), oheight, BilateralNoneHelper<type>, m_ctx, std::cref(src_border),           \
