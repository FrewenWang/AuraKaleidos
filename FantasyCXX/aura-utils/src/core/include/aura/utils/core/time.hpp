...
struct AURA_EXPORTS Time
{
    ...
    */
    Time(AURA_S64 second = 0, AURA_S32 millisec = 0, AURA_S32 microsec = 0)
    {
        AURA_S32 ms_carry  = 0;
        AURA_S32 sec_carry = 0;
    ...
    AURA_VOID Update()
    {
        ...
        sec = static_cast<AURA_S64>(seconds.count());
        AURA_S32 microseconds = static_cast<AURA_S32>(std::chrono::duration_cast<std::chrono::microseconds>(now - seconds).count());
    ...
    AURA_S64 sec;  /*!< The second component of the time. */
    AURA_S32 ms;   /*!< The millisecond component of the time. */
    AURA_S32 us;   /*!< The microsecond component of the time. */
};
 ...
 */
AURA_INLINE AURA_BOOL operator==(const Time &t0, const Time &t1)
{
...
AURA_INLINE AURA_BOOL operator>(const Time &t0, const Time &t1)
{
    if (t0.sec != t1.sec)
    {
        return (t0.sec > t1.sec) ? AURA_TRUE : AURA_FALSE;
    }

    if (t0.ms != t1.ms)
    {
        return (t0.ms > t1.ms) ? AURA_TRUE : AURA_FALSE;
    }

    return (t0.us > t1.us) ? AURA_TRUE : AURA_FALSE;
}
...
AURA_INLINE Time operator+(const Time &t0, const Time &t1)
{
    ...
    // ms
    AURA_S64 sum_ms = t0.ms + t1.ms + ms_carry;
    if (sum_ms >= 1000)
    {
        sum.ms = sum_ms - 1000;
        sec_carry = 1;
...
public:
     ...
     */
    TimeStamp( AURA_S8 mon = 0, AURA_S8 day = 0, AURA_S8 hour = 0, AURA_S8 min = 0, AURA_S8 sec = 0, AURA_S16 ms = 0, AURA_S16 us = 0)
    ...
    ::AURA_VOID Update()
    {
        ...
        m_mon  = static_cast<AURA_S8>(tm->tm_mon + 1);
        m_day  = static_cast<AURA_S8>(tm->tm_mday);
        m_hour = static_cast<AURA_S8>(tm->tm_hour);
        m_min  = static_cast<AURA_S8>(tm->tm_min);
        m_sec  = static_cast<AURA_S8>(tm->tm_sec);
    ...
    {
        os        << std::setfill('0') << std::setw(2) << (AURA_S32)(time_stamp.m_mon);
        os << "-" << std::setfill('0') << std::setw(2) << (AURA_S32)(time_stamp.m_day);
        os << " " << std::setfill('0') << std::setw(2) << (AURA_S32)(time_stamp.m_hour);
        os << ":" << std::setfill('0') << std::setw(2) << (AURA_S32)(time_stamp.m_min);
        os << ":" << std::setfill('0') << std::setw(2) << (AURA_S32)(time_stamp.m_sec);
        os << "." << std::setfill('0') << std::setw(3) << (AURA_S32)(time_stamp.m_ms);
...
private:
    AURA_S8  m_mon;  /*!< Month (1 ~ 12). */
    AURA_S8  m_day;  /*!< Day (1 ~ 31). */
    AURA_S8  m_hour; /*!< Hour (0 ~ 23). */
    AURA_S8  m_min;  /*!< Minute (0 ~ 59). */
    AURA_S8  m_sec;  /*!< Second (0 ~ 59). */
    AURA_S16 m_ms;   /*!< Millisecond (0 ~ 999). */
    AURA_S16 m_us;   /*!< Microsecond (0 ~ 999). */
};
