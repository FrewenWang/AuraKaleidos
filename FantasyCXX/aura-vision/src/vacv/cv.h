 ...
 */
void resize(const vision::VTensor& src, vision::VTensor& dst,
			VSize dsize, double fx = 0, double fy = 0,
			int interpolation = INTER_LINEAR);

/**
 * @brief 图像缩放,并针对DTpye进行转换，确保符合模型数据输入要求
 * @param src 输入
 * @param dst 输出
 * @param dsize 目标尺寸
 */
void resizeNoNormalize(const vision::VTensor &src, vision::VTensor &dst,
                       VSize dsize, double fx = 0, double fy = 0,
                       int interpolation = INTER_LINEAR);
 ...
 * @param code 颜色转换模式
 */
void cvt_color(const vision::VTensor& src, vision::VTensor& dst, int code);
 ...
 * @param stddev 标准差
 */
void normalize(const vision::VTensor& src, vision::VTensor& dst,
 ...
 */
void warp_affine(const vision::VTensor& src, vision::VTensor& dst,
...
void minMaxIdx(const vision::VTensor& src, double* minVal, double* maxVal,
			   int* minIdx = nullptr, int* maxIdx = nullptr, const vision::VTensor& mask=vision::VTensor());
...
/**
 * @brief 计算 r-g, g-r, b-r方差的平均值
 * @param src 输入矩阵
 * @param variance 输出方差的平均值
 * @param 说明 只对channels=3有效
 */
