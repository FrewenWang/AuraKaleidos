...
class Templates(object):
    ...
    def createTemplate(self, frame, points):
        ...
        cp_Xmin = np.min(points[:, 0])
        cp_Xmax = np.max(points[:, 0])
        cp_Ymin = np.min(points[:, 1])
        cp_Ymax = np.max(points[:, 1])
        ...
        # 车内 * 1.2
        fratio = 1.2
        template_image = frame[centery - int(rect_size * fratio / 2.):centery + int(rect_size * fratio / 2.),
                         centerx - int(rect_size * fratio / 2.):centerx + int(rect_size * fratio / 2.)]
        # template_image = frame[centery - int(rect_size * 1.0 / 2.):centery + int(rect_size * 1.0 / 2.),
        #                  centerx - int(rect_size * 1.0 / 2.):centerx + int(rect_size * 1.0 / 2.)]
    ...
    def useTemplate(self, frame, template):
        ...
        """
        fratio = 2
        # cv2.imwrite("/home/baidu/Desktop/tempplate.jpg", template)
        # cv2.imwrite("/home/baidu/Desktop/temp_frame.jpg", frame)
        if template.shape[0] > 0 and template.shape[1] > 0:
            ...
            try:
                img_gray = cv2.resize(img_gray,
                                      (img_gray.shape[1] // fratio, img_gray.shape[0] // fratio))  # 1/4*1/4  (320, 180)
                template = cv2.resize(template,
                                      (template.shape[1] // fratio, template.shape[0] // fratio))
                ...
                print (e)
                return []
            return [top_left[0] * fratio, top_left[1] * fratio, bottom_right[0] * fratio, bottom_right[1] * fratio]
        else:
            return []
