...
def run(
    ...
    for k, m in model.named_modules():
        if m in [Detect, Decoupled_Detect_Tal, Decoupled_Detect, Decoupled_Detect_OTA, Detect_objectBox]:
            m.inplace = inplace
            m.onnx_dynamic = dynamic
            m.export = True

        # elif isinstance(m, Decoupled_Detect_Tal):
        #     m.inplace = inplace
        #     m.onnx_dynamic = dynamic
        #     m.export = True
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, 
                        default=ROOT / 'runs/train/YOLOv5s_TAL/weights/best.pt', help='model.pt path(s)')
