...
public:
    ...
    virtual Status SetArgs(const Array *src, Array *dst, DT_S32 dx, DT_S32 dy, DT_S32 ksize, DT_F32 scale = 1.f,
...
template <typename MVTypeInter, DT_BOOL WITH_SCALE,
          typename std::enable_if<!WITH_SCALE, DT_VOID>::type* = DT_NULL>
AURA_INLINE DT_VOID SobelPostProcess(MVTypeInter &mvqs16_result, DT_S16 *dst, DT_F32 scale)
{
    AURA_UNUSED(scale);
...
// WITH_SCALE  = DT_TRUE
// MVTypeInter = int16x8x1_t, int16x8x2_t, int16x8x3_t
template <typename MVTypeInter, DT_BOOL WITH_SCALE, typename std::enable_if<WITH_SCALE, DT_VOID>::type* = DT_NULL>
AURA_INLINE DT_VOID SobelPostProcess(MVTypeInter &mvqs16_result, DT_S16 *dst, DT_F32 scale)
{
    constexpr DT_S32 channel = static_cast<DT_S32>(sizeof(MVTypeInter) / sizeof(int16x8_t));

    using MVTypeF32 = typename neon::MQVector<DT_F32, channel>::MVType;
    ...
    for (DT_S32 c = 0; c < channel; c++)
    {
        ...
        mvqf32_result_l.val[c] = neon::vcvt<DT_F32>(vqs32_result_l);
        mvqf32_result_l.val[c] = neon::vmul(mvqf32_result_l.val[c], scale);
        vqs32_result_l         = neon::vcvt<DT_S32>(neon::vrndn(mvqf32_result_l.val[c]));

        vqs32_result_h         = neon::vmovl(neon::vgethigh(mvqs16_result.val[c]));
        mvqf32_result_h.val[c] = neon::vcvt<DT_F32>(vqs32_result_h);
        mvqf32_result_h.val[c] = neon::vmul(mvqf32_result_h.val[c], scale);
        vqs32_result_h         = neon::vcvt<DT_S32>(neon::vrndn(mvqf32_result_h.val[c]));
...
// WITH_SCALE  = DT_FALSE
// MVTypeInter = int16x8x1_t, int16x8x2_t, int16x8x3_t
template <typename MVTypeInter, DT_BOOL WITH_SCALE, typename SType = typename neon::Scalar<MVTypeInter>::SType,
          typename std::enable_if<(!WITH_SCALE) && std::is_same<SType, DT_S16>::value, DT_VOID>::type* = DT_NULL>
AURA_INLINE DT_VOID SobelPostProcess(MVTypeInter &mvqs16_result, DT_F32 *dst, DT_F32 scale)
{
    AURA_UNUSED(scale);

    constexpr DT_S32 channel = static_cast<DT_S32>(sizeof(MVTypeInter) / sizeof(int16x8_t));
    constexpr DT_S32 voffset = channel << 2;
...
template <typename MVTypeInter, DT_BOOL WITH_SCALE, typename SType = typename neon::Scalar<MVTypeInter>::SType,
          typename std::enable_if<(!WITH_SCALE) && std::is_same<SType, DT_F32>::value, DT_VOID>::type* = DT_NULL>
AURA_INLINE DT_VOID SobelPostProcess(MVTypeInter &mvqf32_result, DT_F32 *dst, DT_F32 scale)
{
    AURA_UNUSED(scale);
...
AURA_INLINE DT_VOID SobelPostProcess(MVTypeInter &mvqf32_result, DT_F32 *dst, DT_F32 scale)
{
    constexpr DT_S32 channel = static_cast<DT_S32>(sizeof(MVTypeInter) / sizeof(float32x4_t));
...
public:
    ...
    Status SetArgs(const Array *src, Array *dst, DT_S32 dx, DT_S32 dy, DT_S32 ksize,
                   DT_F32 scale, BorderType border_type = BorderType::REFLECT_101,
    ...
    static std::vector<CLKernel> GetCLKernels(Context *ctx, DT_S32 dx, DT_S32 dy, DT_S32 ksize, DT_F32 scale, BorderType border_type,
...
AURA_ALWAYS_INLINE DT_VOID SobelPostProcess(HVX_Vector &vs16_result_l, HVX_Vector &vs16_result_h, DT_F32 scale)
{
    HVX_Vector vf32_scale = vsplat<DT_F32>(scale);
...
using SobelInParam = HexagonRpcParamType<Mat, Mat, DT_S32, DT_S32, DT_S32, DT_F32, BorderType, Scalar>;
