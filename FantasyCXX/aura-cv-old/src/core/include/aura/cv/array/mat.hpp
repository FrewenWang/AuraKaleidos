#ifndef MAGE_RUNTIME_ARRAY_MAT_HPP__
#define MAGE_RUNTIME_ARRAY_MAT_HPP__

#include "mage/runtime/array/array.hpp"
 ...
 */
MAGE_INLINE std::ostream& operator<<(std::ostream &os, BorderType border_type)
{
    switch (border_type)
    {
 ...
 */
MAGE_INLINE std::string BorderTypeToString(BorderType type)
{
 ...
 *
 * The `Mat` Class represents a multi-dimensional dense numerical array, which can be used to store images, complex numbers
 ...
 *
 * The formula for accessing the k-th channel of the i-th row and j-th column in an RGB image matrix is as follows:
 * \f[M[i][j][k] = i \cdot \text{m_strides.m_width} + j \cdot \text{m_sizes.m_channel} + k\f]
 */
class MAGE_EXPORTS Mat : public Array
{
public:
    /**
     ...
     * @param mem_type Memory type for matrix allocation (default is MAGE_MEM_DEFAULT).
     * @param strides Strides for each dimension of the matrix (default is Sizes of 0, which means no padding).
     *
     * @note On the Android platform, MAGE_MEM_DEFAULT is equivalent to MAGE_MEM_DMA_BUF_HEAP, while on other platforms, it is equivalent to MAGE_MEM_HEAP.
     */
    Mat(Context *ctx, ElemType elem_type, const Sizes3 &sizes, MI_S32 mem_type = MAGE_MEM_DEFAULT, const Sizes &strides = Sizes());
...
/*!
 * @brief Enumerates different image formats.
 */
...
/**
 * @brief Overloaded stream operator for ImageFormat.
 *
 * @param os           Output stream.
 * @param image_format ImageFormat type to be streamed.
 * 
 * @return Output stream with image_format information.
 */
MAGE_INLINE std::ostream& operator<<(std::ostream &os, ImageFormat image_format)
{
    switch (image_format)
    {
        ...
        case ImageFormat::I422:
        {
            os << "I422";
            break;
        }
        case ImageFormat::I444:
        {
            os << "I444";
            break;
        }
...
/**
 * @brief Converts ImageFormat to a string.
 *
 * @param image_fmt ImageFormat type to be converted.
 * 
 * @return String representation of the image_fmt type.
 */
MAGE_INLINE std::string ImageFormatToString(ImageFormat image_format)
{
...
/**
 * @brief Get the sizes and strides of individual planes for a given image format.
 *
 * @param fmt         The image format.
 * @param img_sizes   The sizes of the original image.
 * @param img_strides The strides of the original image.
 ...
 * @return Status::OK if successful; otherwise, an appropriate error status.
 */
MAGE_EXPORTS Status GetImageFormatSize(ImageFormat fmt, Sizes img_sizes, const Sizes &img_strides,
                                       std::vector<Sizes3> &mat_sizes, std::vector<Sizes> &mat_strides);

/**
 * @brief Get the ROIs (regions of interest) for individual planes of a given image format.
 *
 * @param fmt      The image format.
 * @param img_roi  The region of interest in the original image.
 * @param mat_rois Output vector containing ROIs for individual planes.
 *
 * @return Status::OK if successful; otherwise, an appropriate error status.
 */
MAGE_EXPORTS Status GetImageFormatRoi(ImageFormat fmt, const Rect &img_roi, std::vector<Rect> &mat_rois);

/**
 * @brief Read a BMP image from a file.
 *
 * This function reads a BMP image from the specified file and returns it as a Mat object.
 *
 * @param ctx      The pointer to the Context object.
 * @param fname    The file name of the BMP image to read.
 * @param mem_type The type of memory to use for storing the image. Default is MAGE_MEM_DEFAULT.
 * @param strides  The strides for the image data. Default is an empty Sizes object.
 *
 * @return A Mat object containing the image data. An empty Mat will be returned if error occurs.
 */
MAGE_EXPORTS Mat ReadBmp(Context *ctx, const std::string &fname, MI_S32 mem_type = MAGE_MEM_DEFAULT, const Sizes &strides = Sizes());
 ...
 * @param fname The file name to write the BMP image to.
 *
 * @return Status::OK if successful; otherwise, an appropriate error status.
 */
MAGE_EXPORTS Status WriteBmp(Context *ctx, const Mat &mat, const std::string &fname);

/**
 * @brief Read a YUV image from a file.
 *
 * This function reads a YUV image from the specified file and returns a vector of Mat objects representing the individual planes.
 *
 * @param ctx      The pointer to the Context object.
 * @param fname    The file name of the YUV image to read.
 * @param fmt      The image format describing the YUV format.
 * @param sizes    The sizes of the image planes.
 * @param mem_type The type of memory to use for storing the image planes. Default is MAGE_MEM_DEFAULT.
 * @param strides  The strides for the image data. Default is an empty Sizes object.
 *
 * @return A vector of Mat objects containing the image data for each plane. An empty vector will be returned if error occurs.
 *
 * @note Supported ImageFormat: NV12/NV21/YU12/YV12/P010/P016/I422/I444.
 */
MAGE_EXPORTS std::vector<Mat> ReadYuv(Context *ctx, const std::string &fname, ImageFormat fmt, const Sizes &sizes,
                                      MI_S32 mem_type = MAGE_MEM_DEFAULT, const Sizes &strides = Sizes());

/**
 * @brief Write a YUV image to a file.
 *
 * This function writes the provided vector of Mat objects, representing the YUV planes, to a file.
 *
 * @param ctx   The pointer to the Context object.
 * @param mats  A vector of Mat objects containing the image data for each plane.
 * @param fmt   The image format describing the YUV format.
 * @param fname The file name to write the YUV image to.
 *
 * @return Status::OK if successful; otherwise, an appropriate error status.
 *
 * @note Supported ImageFormat: NV12/NV21/YU12/YV12/P010/P016/I422/I444.
 */
MAGE_EXPORTS Status WriteYuv(Context *ctx, const std::vector<Mat> &mats, ImageFormat fmt, const std::string &fname);

/**
 * @brief Get a multiple planes YUV image from a mat and imageformat
 *
 * This function get a YUV image from the imageformt mat and returns a vector of Mat objects representing the individual planes.
 *
 * @param ctx          The pointer to the Context object.
 * @param mat          A Mat objects containing the yuv image data, memory must continuous, channel must be 1
 * @param fmt          The image format describing the YUV format.
 * @param is_deep_copy identification of return mats deep copy, MI_TRUE means deep copy, MI_FALSE means shallow copy.
 *
 * @return vector of Mat objects which store yuv image different plane.
 *
 * @note Supported ImageFormat: NV12/NV21/YU12/YV12/P010/P016/I422/I444.
 */
MAGE_EXPORTS std::vector<Mat> GetYuvImage(Context *ctx, const Mat &mat, ImageFormat fmt, MI_BOOL is_deep_copy = MI_FALSE);
...
#endif // MAGE_RUNTIME_ARRAY_MAT_HPP__