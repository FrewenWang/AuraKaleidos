...
int main(int argc, char** argv)
{
    ...
    // Allocate matrices
    std::vector<cl_int> A_data(k*m);
    std::vector<cl_int> B_data(k*m);
    std::vector<cl_int> Bx2_data(k*m);
    ...
    std::memcpy(matrix_a_buf.dmabuf_hostptr, A_data.data(), matrix_bytes);
    ...
    std::memcpy(matrix_b_buf.dmabuf_hostptr, B_data.data(), matrix_bytes);
    ...
    /*
     * Matrix Bx2, which is defined as 2*B
     */

    cl_mem_dmabuf_host_ptr matrix_bx2_buf = wrapper.make_buffer(matrix_bytes);

    buf_sync.flags = DMA_BUF_SYNC_START | DMA_BUF_SYNC_WRITE;
    if ( ioctl(matrix_bx2_buf.dmabuf_filedesc, DMA_BUF_IOCTL_SYNC, &buf_sync) )
    {
        std::cerr << "Error preparing the cache for buffer writes. The DMA_BUF_IOCTL_SYNC operation returned " << strerror(errno) << "!\n";
        return errno;
    }

    std::memcpy(matrix_bx2_buf.dmabuf_hostptr, Bx2_data.data(), matrix_bytes);
    ...
    if (err != CL_SUCCESS)
    {
        std::cerr << "Error " << err << " with clCreateBuffer for matrix Bx2." << "\n";
        std::exit(err);
    }
    ...
    err = clSetKernelArg(gemm_kernel, 1, sizeof(cl_mem), &B);
    CL_ASSERT_ERRCODE(err);
    ...
    // and sub-optimal local size.  These will be set with the recordable queues API
    inc_by_amount = 0;
    ...
    cl_array_arg_qcom update_kernel_args[2] = {{0 /* dispatch_index */,
                                                0 /* arg_index */,
    ...
    cl_workgroup_qcom update_local_size[1] = {
        {
                1 /* dispatch_index */,
                inc_kernel_local_size /* workgroup_size */
        }
    };

    /*
     * Step 6:  Enqueue Recording with the first dispatch index performing C = AB
     */

    cl_event recording_dispatch_event = nullptr;

    err = clEnqueueRecordingQCOM(command_queue, recording,
            inc > 0 ? 2 : 1, update_kernel_args,
            0, nullptr,
    ...
    map_and_print_memobj_matrix("A =", command_queue, A, matrix_bytes, nullptr, m, matrix_a_buf.dmabuf_filedesc);
    map_and_print_memobj_matrix("B =", command_queue, B, matrix_bytes, nullptr, m, matrix_b_buf.dmabuf_filedesc);
    ...
    /*
     * Step 8: Prepare array to set matrix Bx2 on argument 1
     */

    update_kernel_args[0].dispatch_index    = 0;
    update_kernel_args[0].arg_size          = sizeof(cl_mem);
    update_kernel_args[0].arg_index         = 1;
    update_kernel_args[0].arg_value         = &Bx2;

    /*
     * Step 9: Enqueue Recording with the first dispatch index performing C = A*Bx2
     */

    err = clEnqueueRecordingQCOM(command_queue, recording,
                                 1, update_kernel_args,
                                 0, nullptr,
                                 0, nullptr,
                                 0, nullptr,
                                 0, nullptr, &recording_dispatch_event);
    ...
    /*
     * Step 10: Map and print matrix C again, expecting GEMM kernel to give C=A*Bx2
     */

    map_and_print_memobj_matrix("A =", command_queue, A, matrix_bytes, nullptr, m, matrix_a_buf.dmabuf_filedesc);
    map_and_print_memobj_matrix("B =", command_queue, Bx2, matrix_bytes, nullptr, m, matrix_bx2_buf.dmabuf_filedesc);
