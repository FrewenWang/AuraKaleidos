 ...
 */
public class VisionResult extends AbsVisionResult {

    private static final String TAG = VisionResult.class.getSimpleName();
     ...
     * @return 混合检测结果对象
     */
    public static VisionResult obtain() {
        VisionResult result = obtain(VisionResult.class);
        result.clearAllData(true);
        result.recyclable(true);
        return result;
    }

    public void copy(VisionResult src) {
        if (src != null) {
            mFaceResult.copy(src.mFaceResult);
            mGestureResult.copy(src.mGestureResult);
            mFrameInfo.copy(src.mFrameInfo);
        }
    }
...
//        return str + " | id = " + mFaceResult.mFaceInfos[0].id();
//    }
}
