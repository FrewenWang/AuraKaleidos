                                          ...
                                          MI_S32 left, MI_S32 right, const Scalar &border_value)
{
    ...
    for (MI_S32 y = 0; y < src_sz.m_height; ++y)
    {
        ...
        {
            for (MI_S32 ch = 0; ch < src_sz.m_channel; ++ch)
            {
                *dst_row++ = border_val[ch];
            }
        }
    ...
    for (MI_S32 y = dst_sz.m_height - bottom; y < dst_sz.m_height; ++y)
    {
        Tp *dst_row = dst.Ptr<Tp>(y);
        for (MI_S32 x = 0; x < dst_sz.m_width; ++x)
        {
...
template <typename Tp>
static AURA_VOID MakeBorderReplicateNoneImpl(const Mat &src, Mat &dst, MI_S32 top, MI_S32 bottom,
                                           MI_S32 left, MI_S32 right)
{
    const Sizes3 &src_sz = src.GetSizes();
    ...
    for (MI_S32 y = src_sz.m_height + top; y < src_sz.m_height + top + bottom; ++y)
    {
        Tp *dst_row = dst.Ptr<Tp>(y);
        memcpy(dst_row, src_row, dst.GetRowPitch());
    }
...
template <typename Tp>
static AURA_VOID MakeBorderReflect101NoneImpl(const Mat &src, Mat &dst, MI_S32 top, MI_S32 bottom,
                                            MI_S32 left, MI_S32 right)
{
    const Sizes3 &src_sz = src.GetSizes();

    // build idx table
    MI_S32 buffer_size = Max(top + bottom, left + right);
    std::vector<MI_S32> idx_tab(buffer_size, 0);
    for (MI_S32 i = 0; i < left; ++i)
    {
        idx_tab[i] = GetBorderIdx<BorderType::REFLECT_101>(i - left, src_sz.m_width) * src_sz.m_channel;
    }

    for (MI_S32 i = 0; i < right; i++)
    {
        idx_tab[i + left] = GetBorderIdx<BorderType::REFLECT_101>(i + src_sz.m_width, src_sz.m_width) * src_sz.m_channel;
    }

    // Process the middle rows
    for (MI_S32 y = top; y < src_sz.m_height + top; ++y)
    {
        const Tp *src_row = src.Ptr<Tp>(y - top);
        Tp *dst_row = dst.Ptr<Tp>(y);

        // Process left border
        for (MI_S32 x_dst = 0; x_dst < left; ++x_dst)
        {
            MI_S32 x_src = idx_tab[x_dst];
        ...
        for (MI_S32 x_dst = 0; x_dst < right; ++x_dst)
        {
            MI_S32 x_src = idx_tab[x_dst + left];
...
template <typename Tp>
static Status MakeBorderNoneHelper(Context *ctx, const Mat &src, Mat &dst, MI_S32 top, MI_S32 bottom, MI_S32 left, MI_S32 right,
...
Status MakeBorderNone::SetArgs(const Array *src, Array *dst, MI_S32 top, MI_S32 bottom, MI_S32 left, MI_S32 right,
                                  BorderType type, const Scalar &border_value)
{
...
Status MakeBorderNone::Run()
{
    ...
    switch (src->GetElemType())
    {
        case ElemType::U8:
        {
            ret = MakeBorderNoneHelper<MI_U8>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_U8> failed.");
            }
            break;
        }
        case ElemType::S8:
        {
            ret = MakeBorderNoneHelper<MI_S8>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_S8> failed.");
            }
            break;
        }
        case ElemType::U16:
        {
            ret = MakeBorderNoneHelper<MI_U16>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_U16> failed.");
            }
            break;
        }
        case ElemType::S16:
        {
            ret = MakeBorderNoneHelper<MI_S16>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_S16> failed.");
            }
            break;
        }
        case ElemType::U32:
        {
            ret = MakeBorderNoneHelper<MI_U32>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_U32> failed.");
            }
            break;
        }
        case ElemType::S32:
        {
            ret = MakeBorderNoneHelper<MI_S32>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_S32> failed.");
            }
            break;
        }
        ...
        case ElemType::F32:
        {
            ret = MakeBorderNoneHelper<MI_F32>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_F32> failed.");
            }
            break;
        }
        case ElemType::F64:
        {
            ret = MakeBorderNoneHelper<MI_F64>(m_ctx, *src, *dst, m_top, m_bottom, m_left, m_right, m_type, m_border_value);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(m_ctx, "MakeBorderNoneHelper<MI_F64> failed.");
            }
            break;
        }
#endif
