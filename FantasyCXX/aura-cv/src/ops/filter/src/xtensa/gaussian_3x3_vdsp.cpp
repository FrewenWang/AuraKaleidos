...
template <typename Kt, typename std::enable_if<(std::is_same<Kt, MI_S8>::value) || (std::is_same<Kt, MI_U8>::value)>::type* = MI_NULL>
AURA_ALWAYS_INLINE xb_int32pr GetKernel(Kt *kernel)
{
    return IVP_EXTRPRN_2X32((MI_U32)(0 << 24) + (MI_U32)(kernel[2] << 16) + (MI_U32)(kernel[1] << 8) + (MI_U32)(kernel[0] & 0xff), 0);
}

AURA_ALWAYS_INLINE AURA_VOID Gaussian3x3HCore(xb_vec2Nx8U &vqu8_src_x0, xb_vec2Nx8U &vqu8_src_x1, xb_int32pr vs32_kernel,
...
AURA_ALWAYS_INLINE xb_vec2Nx8U Gaussian3x3VCore(xb_vecNx16U &vqu16_sum_p_lo, xb_vecNx16U &vqu16_sum_p_hi,
                                                ...
                                                MI_U8 *kernel)
{
    xb_vecNx64w wqs64_sum_lo = IVP_MULUUNX16U((MI_U16)kernel[0], vqu16_sum_p_lo);
    xb_vecNx64w wqs64_sum_hi = IVP_MULUUNX16U((MI_U16)kernel[0], vqu16_sum_p_hi);

    IVP_MULUUANX16U(wqs64_sum_lo, (MI_U16)kernel[1], vqu16_sum_c_lo);
    IVP_MULUUANX16U(wqs64_sum_hi, (MI_U16)kernel[1], vqu16_sum_c_hi);

    IVP_MULUUANX16U(wqs64_sum_lo, (MI_U16)kernel[2], vqu16_sum_n_lo);
    IVP_MULUUANX16U(wqs64_sum_hi, (MI_U16)kernel[2], vqu16_sum_n_hi);
...
template <typename Tp, typename Kt, MI_S32 C>
MI_S32 Gaussian3x3VdspImpl(const xvTile *src, xvTile *dst, Kt *kernel)
{
    ...
    constexpr MI_S32 ELEM_COUNTS = static_cast<MI_S32>(sizeof(MVqType) / C / sizeof(Tp));
    constexpr MI_S32 ELEM_BYTES  = ELEM_COUNTS * C * sizeof(Tp);
    valign va_store = IVP_ZALIGN();

    MI_S32 src_x0_len = src->pitch * sizeof(Tp);
    MI_S32 src_x1_len = src_x0_len - ELEM_BYTES;
    MI_S32 dst_len = dst->pitch * sizeof(Tp);
    ...
    for (MI_S32 x = 0; x < dst->width; x += ELEM_COUNTS)
    {
        // row 0
        {
...
MI_S32 Gaussian3x3VdspHelper(const xvTile *src, xvTile *dst, Kt *kernel, MI_S32 channel)
{
...
MI_S32 Gaussian3x3Vdsp(const xvTile *src, xvTile *dst, AURA_VOID *kernel, ElemType elem_type, MI_S32 channel)
{
    ...
    switch (elem_type)
    {
        case ElemType::U8:
        {
            MI_U8 *kdata = reinterpret_cast<MI_U8*>(kernel);
            ret = Gaussian3x3VdspHelper<MI_U8>(src, dst, kdata, channel);
            break;
        }
