...
void CvtColor::cvt_color(const VTensor& src, VTensor& dst, int code) {
#if defined(BUILD_QNX) and defined (BUILD_FASTCV)
	cvt_color_fastcv(src, dst, code);
...
void CvtColor::cvt_color_naive(const VTensor& src, VTensor& dst, int code) {
...
#ifdef BUIILD_OPENCV
void CvtColor::cvt_color_opencv(const VTensor& src, VTensor& dst, int code) {
	...
	if (dst.empty()) {
		...
		const auto& mat_dst = vision::TensorConverter::convert_to<cv::Mat>(dst);
		cv::cvtColor(mat_src, mat_dst, code);
	}
}
#endif
...
#if defined (BUILD_FASTCV)
void CvtColor::cvt_color_fastcv(const VTensor& src, VTensor &dst, int code) {
	switch (code) {
		case FrameConvertFormat::COLOR_YUV2GRAY_UYVY: {
			fcvStatus ret = fcvColorUYVYtoYUV420SemiPlanaru8(src.asUChar(), src.w, src.h, src.w * 2,
														   dst.asUChar(), dst.uv,
														   dst.w, dst.w);
			if (ret != FASTCV_SUCCESS) {
				VLOGE("CvtColor", "Fail to fcvColorUYVYtoYUV420SemiPlanaru8, return code = %d", ret);
			}
			break;
		}
		case FrameConvertFormat::COLOR_YUV2RGB_UYVY: {
			fcvStatus ret = fcvColorUYVYtoRGB888u8(src.asUChar(), src.w * 2, src.w, src.h, dst.asUChar(), dst.w * 3);
			if (ret != FASTCV_SUCCESS) {
				VLOGE("cvt_color_fastcv", "Fail to fcvColorUYVYtoRGB888u8, return code = %d", ret);
			}
			break;
		}
		case FrameConvertFormat::COLOR_RGB2BGR: {
			fcvColorRGB888ToBGR888u8(src.asUChar(), src.w, src.h, 0, dst.asUChar(), 0);
			break;
		}
		case FrameConvertFormat::COLOR_YUV2RGB2BGR: {
			fcvStatus ret = fcvColorUYVYtoRGB888u8(src.asUChar(), src.w * 2, src.w, src.h, dst.asUChar(), dst.w * 3);
			if (ret != FASTCV_SUCCESS) {
				VLOGE("cvt_color_fastcv", "Fail to fcvColorUYVYtoRGB888u8, return code = %d", ret);
				break;
			}
			fcvColorRGB888ToBGR888u8(dst.asUChar(), src.w, src.h, 0, dst.asUChar(), 0);
			break;
		}
		case FrameConvertFormat::COLOR_YUV2BGR_UYVY: {
			throw std::runtime_error("Unsupported function : COLOR_YUV2BGR_UYVY");
		}
		default: {
#ifdef USE_OPENCV
			cvt_color_opencv(src, dst, code);
#else
			VLOGE("CvtColor", "Unknown convert type, code = %d", code);
#endif
		}
	}
}
...
void CvtColor::cvt_color_sse(const VTensor& src, VTensor& dst, int code) {
    // todo:
}

#if defined (USE_NEON) and __ARM_NEON
void CvtColor::cvt_color_neon(const VTensor& src, VTensor& dst, int code) {
