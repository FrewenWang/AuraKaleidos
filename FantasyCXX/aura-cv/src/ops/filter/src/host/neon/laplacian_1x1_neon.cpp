...
{
    float32x4_t vqf32_sum_pn     = neon::vadd(neon::vcvt<DT_F32>(vdf16_src_px1), neon::vcvt<DT_F32>(vdf16_src_nx1));
...
static DT_VOID Laplacian1x1Row(const St *src_p, const St *src_c, const St *src_n, Dt *dst,
                        ...
                        const Scalar &border_value, const OpTarget &target)
{
    ...
    switch (pattern)
    {
        ...
        {
            ret = Laplacian1x1NeonHelper<DT_U8, DT_S16>(ctx, src, dst, border_type, border_value, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Laplacian1x1NeonHelper<DT_U8> failed");
            }
            break;
        }
        ...
        {
            ret = Laplacian1x1NeonHelper<DT_U16, DT_U16>(ctx, src, dst, border_type, border_value, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Laplacian1x1NeonHelper<DT_U16> failed");
            }
            break;
        }
        ...
        {
            ret = Laplacian1x1NeonHelper<DT_S16, DT_S16>(ctx, src, dst, border_type, border_value, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Laplacian1x1NeonHelper<DT_S16> failed");
            }
            break;
        }
        ...
        {
            ret = Laplacian1x1NeonHelper<DT_F32, DT_F32>(ctx, src, dst, border_type, border_value, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Laplacian1x1NeonHelper<DT_F32> failed");
            }
            break;
        }
