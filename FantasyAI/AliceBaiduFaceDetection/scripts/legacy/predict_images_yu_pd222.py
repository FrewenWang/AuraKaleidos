...
import paddle
import os
from alice_face_detection.PPYoloMobileNetV3 import PPYoloMobileNetV3, PPYoloTiny
import cv2
import numpy as np
import random
...
from alice_face_detection.vehicle_pdc_hand import HandDataSet
...
if __name__ == '__main__':
    ...
    if usePruner:
        ...
        paddle.flops(net, (1, 3, 160, 288))
        # print("+1" * 100)
        plan = pruner.sensitive_prune(0.8)
        paddle.flops(net, (1, 3, 160, 288))
        print("+2" * 100)
    ...
    #mbv3原始网络
    pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/mbv3/全量数据/原始模型/原始模型训练/pd222_ppyolotiny_ori_epoch9.pdparams"
    ##mbv3量化网络
    # pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/全量数据/量化模型/v3_a100/epoch18_release.pdparams"

    #2d4f5904原始网络
    # pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/2d4f5904/原始网络/epoch19.pdparams"
    # 2d4f5904量化网络
    # pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/2d4f5904/原始网络直接量化/epoch9.pdparams"
    ...
    #
    # pathModel = "/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/2d4f5904/原始网络/epoch19.pdparams"
    # pathModel = "/home/baidu/Downloads/2d4f5904_oriModel_epoch19.pdparams"
    ...
    def load_pretrain_weight(model, weight):
        ...
        for name, weight in param_state_dict.items():
            if name in model_dict.keys():
                ...
                if ".bn" in name:
                    print(name, model_dict[name].shape, type(model_dict[name]))
    ...
    # save_path = '/media/baidu/3.6TB_SSD/facedetect/缩小尺寸/models_save/pd222/全量数据/量化模型/v3_a100/quant_save/model'
    # quanter.save_quantized_model(
    #     net,
    #     save_path,
    #     input_spec=[paddle.static.InputSpec(shape=[None, 3, 160, 288], dtype='float32')])

    # paddle.jit.to_static(net,  input_spec=[InputSpec(shape=[1,3,160,288], dtype=paddle.float32, name='inputs')])
    # paddle.jit.save(net,"/media/baidu/ssd1/someLIBS/tmp/1/model")
    # save_path = '/media/baidu/ssd1/someLIBS/tmp/2_/model'
    # quanter.save_quantized_model(net, save_path),
        # input_spec=[paddle.static.InputSpec(shape=[1, 3, 160, 288], dtype='float32')])
    ...
    folder_name1 = "image"
    folder_name2 = folder_name1 + "_2d4f5904_prune60"
