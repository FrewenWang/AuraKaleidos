//--------------------------------------------------------------------------------------
// File: p010_to_nv12.cpp
// Desc: This program converts P010 to NV12
//
// Author:      QUALCOMM
//
...
static const char *PROGRAM_SOURCE = R"(
    __kernel void
    p010_to_nv12(__read_only  image2d_t input_p010,
                              ...
                              sampler_t sampler)
    {
        ...
        in = read_imagef(input_p010, sampler, coord);
        write_imagef(out_nv12_y, coord, in);
        coord += (int2)(0, 1);
        in = read_imagef(input_p010, sampler, coord);
        write_imagef(out_nv12_y, coord, in);
        coord += (int2)(1, 0);
        ...
        write_imagef(out_nv12_y, coord, in);
        out.xy = in.yz;
        write_imagef(out_nv12_uv, coord / 2, out);
    }
)";
...
int main(int argc, char** argv)
{
    if (argc != 3)
    {
        std::cerr << "Usage: " << argv[0] << " <img data file> <out img data file> \n"
                     ...
                     "Demonstrates conversions from P010 to NV12\n"
                     "The conversion from 10-bit channels to 8-bit channels is lossy.\n";
        return EXIT_FAILURE;
    }
    ...
    cl_kernel           p010_to_nv12_kernel = wrapper.make_kernel("p010_to_nv12", program);
    ...
    {
        std::cerr << "Extension cl_qcom_other_image needed for P010 and NV12 image formats is not supported.\n";
        return EXIT_FAILURE;
    }
    ...
    cl_int err = 0;
    cl_mem_dmabuf_host_ptr src_p010_mem = wrapper.make_buffer_for_yuv_image(src_p010_format, src_p010_desc);
    ...
    out_nv12_desc.image_width            = src_p010_image_info.y_width;
    out_nv12_desc.image_height           = src_p010_image_info.y_height;
    ...
    src_y_plane_desc.image_width  = src_p010_desc.image_width;
    src_y_plane_desc.image_height = src_p010_desc.image_height;
    ...
    if (ioctl(src_p010_mem.dmabuf_filedesc, DMA_BUF_IOCTL_SYNC, &buf_sync))
    {
    ...
    err = clSetKernelArg(p010_to_nv12_kernel, 0, sizeof(src_p010_image), &src_p010_image);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 0 for p010_to_nv12_kernel." << "\n";
        return err;
    }

    err = clSetKernelArg(p010_to_nv12_kernel, 1, sizeof(out_y_plane), &out_y_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 1 for p010_to_nv12_kernel." << "\n";
        return err;
    }

    err = clSetKernelArg(p010_to_nv12_kernel, 2, sizeof(out_uv_plane), &out_uv_plane);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 2 for p010_to_nv12_kernel." << "\n";
        return err;
    }

    err = clSetKernelArg(p010_to_nv12_kernel, 3, sizeof(sampler), &sampler);
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clSetKernelArg for argument 3 for p010_to_nv12_kernel." << "\n";
        return err;
    }

    const size_t work_size[] = {out_nv12_desc.image_width / 2, out_nv12_desc.image_height / 2};
    ...
    if (err != CL_SUCCESS)
    {
        std::cerr << "\tError " << err << " with clEnqueueNDRangeKernel for p010_to_nv12_kernel kernel." << "\n";
        return err;
    }
