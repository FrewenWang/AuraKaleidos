                        ...
                        float32x4_t &vqf32_c_beta)
{
    ...
    using SType = typename std::conditional<std::is_same<Tp, MI_U16>::value, MI_U32, MI_S32>::type;
...
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_U8, SType>::value || std::is_same<MI_S8, SType>::value, int32x2_t>::type
...
template <typename SType, typename VType>
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_U16, SType>::value || std::is_same<MI_S16, SType>::value, float32x2_t>::type
ResizeCuCommNeonHorCore(VType &vd16_x0_src, VType &vd16_x1_src, float32x4_t &vqf32_x0_alpha, float32x4_t &vqf32_x1_alpha)
{
    ...
    float32x4_t vqf32_x0_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vq32_x0_src_mov0), vqf32_x0_alpha);
    float32x4_t vqf32_x1_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vq32_x1_src_mov0), vqf32_x1_alpha);
                        ...
                        float32x4_t &vqf32_x0_alpha, float32x4_t &vqf32_x1_alpha, float32x4_t &vqf32_x2_alpha, float32x4_t &vqf32_x3_alpha)
{
    ...
    float32x4_t vqf32_x0_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vdf16_x0_src), vqf32_x0_alpha);
    float32x4_t vqf32_x1_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vdf16_x1_src), vqf32_x1_alpha);
    float32x4_t vqf32_x2_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vdf16_x2_src), vqf32_x2_alpha);
    float32x4_t vqf32_x3_buffer_tmp0 = neon::vmul(neon::vcvt<MI_F32>(vdf16_x3_src), vqf32_x3_alpha);
...
// VType = float32x4_t
template <typename SType, typename VType>
AURA_ALWAYS_INLINE typename std::enable_if<std::is_same<MI_F32, SType>::value, float32x2_t>::type
...
ResizeCuCommNeonImpl(Context *ctx, const Mat &src, Mat &dst, MI_S32 *buffer, ThreadBuffer &thread_buffer, MI_S32 start_row, MI_S32 end_row)
{
    using MVTypeD8   = typename neon::MDVector<Tp, C>::MVType;
    using VTypeD8    = typename neon::DVector<Tp>::VType;
    using MVTypeS32  = typename neon::MDVector<MI_S32, C>::MVType;
    ...
    MI_S16 *alpha = reinterpret_cast<MI_S16*>(yofs + oheight);
    MI_S16 *beta  = reinterpret_cast<MI_S16*>(alpha + (owidth * 4));
    ...
    MI_S32 *rows0 = rows;
    MI_S32 *rows1 = rows0 + owidth * C;
    MI_S32 *rows2 = rows1 + owidth * C;
    MI_S32 *rows3 = rows2 + owidth * C;

    MI_S32 prev_sy1 = -5;
    MI_S32 loop_end_owidth = Ceil(((iwidth - 8) + 0.5) / scale_x - 0.5);
    MI_S32 width_align2 = loop_end_owidth & (-2);

    const MI_S16 *beta_ptr = beta + start_row * 4;
    ...
    for (MI_S32 dy = start_row; dy < end_row; dy++)
    {
        ...
        if (sy == prev_sy1)
        {
            ...
            const MI_S16 *alpha_ptr = reinterpret_cast<MI_S16*>(alpha);

            MI_S32 dx = 0;
            for (; dx < width_align2; dx += 2)
            {
                MI_S32 sx_c        = xofs[dx] * C;
                MI_S32 sx_n        = xofs[dx + 1] * C;
            ...
            for (; dx < owidth; dx++)
            {
                ...
                MI_S16 a0 = alpha_ptr[0];
                MI_S16 a1 = alpha_ptr[1];
                MI_S16 a2 = alpha_ptr[2];
                MI_S16 a3 = alpha_ptr[3];
        ...
        for (; dx < owidth; dx++)
        {
            ...
            for (MI_S32 ch = 0; ch < C; ch++)
            {
                MI_S32 result = (rows0[x_id + ch] * beta_ptr[0] + rows1[x_id + ch] * beta_ptr[1] + rows2[x_id + ch] * beta_ptr[2] + rows3[x_id + ch] * beta_ptr[3]);
...
ResizeCuCommNeonImpl(Context *ctx, const Mat &src, Mat &dst, MI_S32 *buffer, ThreadBuffer &thread_buffer, MI_S32 start_row, MI_S32 end_row)
{
    ...
    using MVTypeF32  = typename neon::MDVector<MI_F32, C>::MVType;
    ...
    MI_S32 *xofs = buffer;
    MI_S32 *yofs = xofs + owidth;
    MI_F32 *alpha = reinterpret_cast<MI_F32*>(yofs + oheight);
    MI_F32 *beta  = reinterpret_cast<MI_F32*>(alpha + (owidth * 4));
    ...
    MI_F32 *rows0 = rows;
    MI_F32 *rows1 = rows0 + owidth * C;
    MI_F32 *rows2 = rows1 + owidth * C;
    MI_F32 *rows3 = rows2 + owidth * C;

    MI_S32 prev_sy1 = -5;
    MI_S32 loop_end_owidth = Ceil(((iwidth - 8) + 0.5) / scale_x - 0.5);
    MI_S32 width_align2 = loop_end_owidth & (-2);

    const MI_F32 *beta_ptr = beta + start_row * 4;
    ...
    for (MI_S32 dy = start_row; dy < end_row; dy++)
    {
        ...
        if (sy == prev_sy1)
        {
            ...
            const Tp *src_n2 = src.Ptr<Tp>(sy + 3);
            const MI_F32 *alpha_ptr = reinterpret_cast<MI_F32*>(alpha);
            ...
            for (; dx < owidth; dx++)
            {
                ...
                MI_F32 a0 = alpha_ptr[0];
                MI_F32 a1 = alpha_ptr[1];
                MI_F32 a2 = alpha_ptr[2];
                MI_F32 a3 = alpha_ptr[3];
...
ResizeCuCommNeonImpl(Context *ctx, const Mat &src, Mat &dst, MI_S32 *buffer, ThreadBuffer &thread_buffer, MI_S32 start_row, MI_S32 end_row)
{
    ...
    MI_S32 prev_sy1 = -5;
    MI_S32 loop_end_owidth = Ceil(((iwidth - 8) + 0.5) / scale_x - 0.5);
    MI_S32 width_align4 = loop_end_owidth & (-4);
    ...
    for (MI_S32 dy = start_row; dy < end_row; dy++)
    {
        ...
        if (sy == prev_sy1)
        {
            ...
            {
                MI_S32 sx_c         = xofs[dx] * C;
                MI_S32 sx_r0        = xofs[dx + 1] * C;
                MI_S32 sx_r1        = xofs[dx + 2] * C;
                MI_S32 sx_r2        = xofs[dx + 3] * C;
                const Tp *src_n2_c  = src_n2 + sx_c;
        ...
        for (; dx < owidth_align4; dx += 4)
        {
            ...
            for (MI_S32 ch = 0; ch < C; ch++)
            {
                wmvqf32_result.val[ch] = ResizeCuCommNeonVerCore<MI_F32>(wmvqf32_cx0_rows.val[ch], wmvqf32_n0x0_rows.val[ch], wmvqf32_n1x0_rows.val[ch], wmvqf32_n2x0_rows.val[ch], vqf32_c_beta);
...
static Status ResizeCuCommNeonHelper(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    ThreadBuffer thread_buffer(ctx, owidth * 4 * channel * sizeof(MI_S32));
...
Status ResizeCuCommNeon(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            ret = ResizeCuCommNeonHelper<MI_U8>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeCuCommNeonHelper run failed, type: MI_U8");
            }
            break;
        }
        ...
        case ElemType::S8:
        {
            ret = ResizeCuCommNeonHelper<MI_S8>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeCuCommNeonHelper run failed, type: MI_S8");
            }
            break;
        }
        ...
        case ElemType::U16:
        {
            ret = ResizeCuCommNeonHelper<MI_U16>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeCuCommNeonHelper run failed, type: MI_U16");
            }
            break;
        }
        ...
        case ElemType::S16:
        {
            ret = ResizeCuCommNeonHelper<MI_S16>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeCuCommNeonHelper run failed, type: MI_S16");
            }
            break;
        }
        ...
        case ElemType::F32:
        {
            ret = ResizeCuCommNeonHelper<MI_F32>(ctx, src, dst, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "ResizeCuCommNeonHelper run failed, type: MI_F32");
            }
            break;
        }
