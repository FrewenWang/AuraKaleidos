...
DEFAULT_MODEL = Path(__file__).resolve().parents[2] / "models/yolo11s.pt"
...
class Detector:
    ...
    def __init__(self, model_path=DEFAULT_MODEL) -> None:
        ...
        try:
            ...
            logger.exception(
                f"Error loading model at path: {model_path}.\n {e}", exc_info=True
            )
            logger.info(f"Loading fallback model at path: '{DEFAULT_MODEL}'.\n {e}")
    ...
    def detect_object(self, frame):
        ...
        detections = list(
            map(
                lambda box: [
                    class_names[int(box.cls.cpu().numpy()[0])],
                    *box.xywh.cpu().numpy()[0],
                ],
                outputs.boxes,
            )
        )
