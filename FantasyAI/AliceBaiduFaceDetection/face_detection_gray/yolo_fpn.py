...
"""
YOLOv3 FPN是一种深度学习中的目标检测算法，结合了YOLOv3（You Only Look Once version 3）和FPN（Feature Pyramid Network）的特点。
YOLOv3是一种one-stage的目标检测算法，它通过特征提取网络对输入图像提取特征，得到一定大小的特征图，并将输入图像划分成多个网格。
每个网格都会预测固定数量的边界框，并根据与真实目标框的交并比（IOU）选择最合适的边界框进行目标预测。
而FPN则是从backbone获取三个有效特征层后，进一步提取特征并进行特征融合的网络。它的目的是结合不同尺度的特征信息，从而能够检测不同大小的目标。
因此，YOLOv3 FPN通过引入FPN结构，使得YOLOv3能够更有效地结合低层次的细节信息和高层次的语义信息，从而提高目标检测的准确性和性能。
这种算法在目标检测任务中取得了很好的效果，并在计算机视觉领域得到了广泛应用。
"""
    ...
    def forward(self, blocks):
        ...
        for i, block in enumerate(blocks):
            ...
            if i < self.num_blocks - 1:
                ...
                route = F.interpolate(
                    route, scale_factor=2., data_format=self.data_format, align_mode=1, align_corners=False)
