...
public:
    ...
    virtual Status SetArgs(const Array *src, Array *dst, MI_S32 top, MI_S32 bottom,
                           MI_S32 left, MI_S32 right, BorderType type,
...
template <typename Tp, BorderType BORDER_TYPE, MI_S32 C, typename std::enable_if<BorderType::REPLICATE == BORDER_TYPE>::type* = MI_NULL>
AURA_INLINE AURA_VOID MakeBorderOneRow(const Mat &src, const MI_S32 idx_row, const MI_S32 width, const MI_S32 ksize,
                                     Tp *src_row_border, const Scalar &border_value)
{
    ...
    MI_S32 idx_row_eff  = Min(Max(idx_row, (MI_S32)0), height - 1);

    const Tp *src_row = src.Ptr<Tp>(idx_row_eff);

    for (MI_S32 x = 0; x < ksh; x++)
    {
...
template <typename Tp, BorderType BORDER_TYPE, MI_S32 C, typename std::enable_if<BorderType::CONSTANT == BORDER_TYPE>::type* = MI_NULL>
AURA_INLINE AURA_VOID MakeBorderOneRow(const Mat &src, const MI_S32 idx_row, const MI_S32 width, const MI_S32 ksize,
                                     Tp *src_row_border, const Scalar &border_value)
{
    const Sizes3 sz     = src.GetSizes();
    ...
    {
        for (MI_S32 x = 0; x < width + ksize - 1; x++)
        {
            for (MI_S32 c = 0; c < C; c++)
            {
...
template <typename Tp, BorderType BORDER_TYPE, MI_S32 C, typename std::enable_if<BorderType::REFLECT_101 == BORDER_TYPE>::type* = MI_NULL>
AURA_INLINE AURA_VOID MakeBorderOneRow(const Mat &src, const MI_S32 idx_row, const MI_S32 width, const MI_S32 ksize,
                                     Tp *src_row_border, const Scalar &border_value)
{
    AURA_UNUSED(border_value);
    ...
    for (MI_S32 x = width; x > width - ksh; x--)
    {
        for (MI_S32 c = 0; c < C; c++)
        {
