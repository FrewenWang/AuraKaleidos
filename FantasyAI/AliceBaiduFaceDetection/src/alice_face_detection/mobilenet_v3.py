"""
mobile net v3 for paddle paddle
"""
    ...
    def forward(self, x):
        ...
        if self.act is not None:
            if self.act == "relu":
                ...
                # Preserve the historical model semantics while avoiding the
                # legacy ``swish`` op, which older Paddle2ONNX releases cannot
                # translate.  This is mathematically identical to F.swish(x).
                x = x * F.sigmoid(x)
            else:
