...
AURA_ALWAYS_INLINE MI_U64 RandomNext(MI_U64 &state)
{
    state = state * 4164903690U + (state >> 32);
    return state;
}

AURA_ALWAYS_INLINE AURA_VOID ElemRotate(MI_F64 &v0, MI_F64 &v1, MI_F64 s, MI_F64 c)
{
    MI_F64 a0 = v0;
    MI_F64 b0 = v1;
    v0 = a0 * c - b0 * s;
...
static MI_BOOL CollinearPoints(std::vector<Point2> &sub_points, MI_S32 count)
{
    MI_S32 i = count - 1;

    for (MI_S32 j = 0; j < i; j++)
    {
        MI_F32 dx1 = sub_points[j].m_x - sub_points[i].m_x;
        MI_F32 dy1 = sub_points[j].m_y - sub_points[i].m_y;

        for (MI_S32 k = 0; k < j; k++)
        {
            MI_F32 dx2 = sub_points[k].m_x - sub_points[i].m_x;
            MI_F32 dy2 = sub_points[k].m_y - sub_points[i].m_y;
...
static MI_BOOL CheckSubset(std::vector<Point2> &sub_src_points, std::vector<Point2> &sub_dst_points, MI_S32 count)
{
    ...
    if (4 == count)
    {
        const MI_S32 permute[4][3] = { { 0, 1, 2 },{ 1, 2, 3 },{ 0, 2, 3 },{ 0, 1, 3 } };
        MI_S32 negative = 0;

        for (MI_S32 i = 0; i < 4; i++)
        {
            const MI_S32 *t = permute[i];
            MI_F64 det_a = sub_src_points[t[0]].m_x * sub_src_points[t[1]].m_y + sub_src_points[t[0]].m_y * sub_src_points[t[2]].m_x +
            ...
            MI_F64 det_b = sub_dst_points[t[0]].m_x * sub_dst_points[t[1]].m_y + sub_dst_points[t[0]].m_y * sub_dst_points[t[2]].m_x +
...
static MI_BOOL GetSubPoints(const std::vector<Point2> &src_points, const std::vector<Point2> &dst_points, std::vector<Point2> &sub_src_points,
                            std::vector<Point2> &sub_dst_points, MI_S32 max_attempts, MI_U64 &state)
{
    MI_U32 idx[MODEL_POINTS_NUM] = { 0 };
    MI_U32 idx_i      = 0;
    MI_S32 i          = 0;
    MI_S32 j          = 0;
    MI_S32 iters      = 0;
    ...
    for (iters = 0; iters < max_attempts; iters++)
	{
        for (i = 0; i < MODEL_POINTS_NUM && iters < max_attempts; i++)
        {
            ...
            for(;;)
            {
                idx_i  = (MI_U32)RandomNext(state) % points_num;
                idx[i] = idx_i;
    ...
    return (i == MODEL_POINTS_NUM && iters < max_attempts) ? MI_FALSE : MI_TRUE;
}

static MI_S32 RansacUpdateNumIters(MI_F32 p, MI_F32 ep, MI_S32 model_points_num, MI_S32 max_iters)
{
    p  = p  > 0. ? p  : 0.;
    p  = p  < 1. ? p  : 1.;
    ep = ep > 0. ? ep : 0.;
    ...
    MI_F32 num   = (1.0 - p) > FLT_MIN ? (1.0 - p) : FLT_MIN;
    MI_F32 denom = 1.0 - pow(1.0 - ep, model_points_num);
    if (denom < FLT_MIN)
    {
        return 0;
    }
...
static AURA_VOID SvdJacobiN8(MI_F64 a[][8], MI_F64 w[], MI_F64 v[][8])
{
    const MI_F64 eps = DBL_EPSILON;
    const MI_S32 n   = 8;
    MI_S32 i         = 0;
    MI_S32 j         = 0;
    ...
    for (iters = 0; iters < max_iters; iters++)
    {
        ...
        MI_F64 y = (w[l] - w[k]) * 0.5;
        MI_F64 t = Abs(y) + hypot(p, y);
        MI_F64 s = hypot(p, t);
        MI_F64 c = t / s;
        s        = p / s;
        t        = (p / t) * p;
...
static AURA_VOID SvdJacobiN9(MI_F64 a[][9], MI_F64 w[], MI_F64 v[][9])
{
    const MI_F64 eps = DBL_EPSILON;
    const MI_S32 n   = 9;
    MI_S32 i         = 0;
...
static MI_S32 FindInliers(const std::vector<Point2> &src_points, const std::vector<Point2> &dst_points, Mat &dst_h, MI_S32 *mask,
                          MI_F64 reproj_threshold)
{
    MI_S32 inliers_num = 0;
    MI_S32 count       = (MI_S32)src_points.size();
    MI_F64 per_error   = 0.0f;
    reproj_threshold  *= reproj_threshold;

    MI_F64 *h = (MI_F64*)dst_h.Ptr<MI_F64>(0);
    for (MI_S32 i = 0; i < count; i++)
    {
        MI_F64 ww = 1.0f / (h[6] * src_points[i].m_x + h[7] * src_points[i].m_y + 1.0);
        MI_F64 dx = ((h[0] * src_points[i].m_x + h[1] * src_points[i].m_y + h[2]) * ww - dst_points[i].m_x);
        MI_F64 dy = ((h[3] * src_points[i].m_x + h[4] * src_points[i].m_y + h[5]) * ww - dst_points[i].m_y);
...
static AURA_VOID ComputeErrorAndJacobi(std::vector<Point2> &src_points, std::vector<Point2> &dst_points, MI_F64 h[8], MI_F64 err[], MI_F64 *jac)
{
    MI_S32 in_num = (MI_S32)src_points.size();
    for (MI_S32 i = 0; i < in_num; i++)
    {
        MI_F64 mx = src_points[i].m_x;
        MI_F64 my = src_points[i].m_y;
        MI_F64 ww = h[6] * mx + h[7] * my + 1.0;

        ww        = Abs(ww) > DBL_EPSILON ? 1.0 / ww : 0.0;
        MI_F64 xi = (h[0] * mx + h[1] * my + h[2])*ww;
        MI_F64 yi = (h[3] * mx + h[4] * my + h[5])*ww;
...
static MI_F64 NormL2sqr(MI_F64 src[], MI_S32 num)
{
    MI_F64 res = 0.0;
    for (MI_S32 i = 0; i < num; i++)
    {
        res += (src[i] * src[i]);
    }
...
static MI_F64 NormInf(MI_F64 src[], MI_S32 num)
{
    MI_F64 res = 0.0;
    for (MI_S32 i = 0; i < num; i++)
    {
...
static MI_F64 VectorDot(MI_F64 src1[], MI_F64 src2[])
{
    MI_F64 res = 0.0;
...
static Status MulTransposed(Context *ctx, std::vector<Point2> &inliers, MI_F64 *src, MI_F64 dst[8][8])
{
    MI_S32 cols = (MI_S32)inliers.size() * 2;
    MI_F64 *src_trans = reinterpret_cast<MI_F64*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, 8 * cols * sizeof(MI_F64), 0));
    if (MI_NULL == src_trans)
    {
...
static Status Gemm1Trans(Context *ctx, std::vector<Point2> &inliers, MI_F64 *j, MI_F64 b[], MI_F64 d[8])
{
    MI_S32 cols = (MI_S32)inliers.size() * 2;
    MI_F64 *j_trans = reinterpret_cast<MI_F64*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, 8 * cols * sizeof(MI_F64), 0));
    if (MI_NULL == j_trans)
    {
        AURA_ADD_ERROR_STRING(ctx, "AURA_ALLOC_PARAM failed");
        return Status::ERROR;
...
static AURA_VOID SVBkSbnb1(MI_S32 n, MI_F64 w[], MI_F64 u[][8], MI_F64 v[][8], MI_F64 b[], MI_F64 x[])
{
    MI_F64 eps       = DBL_EPSILON * 2;
    MI_F64 threshold = 0.0;
    MI_S32 i         = 0;
    MI_S32 j         = 0;
...
static AURA_VOID SolveDelta(MI_F64 src[8][8], MI_F64 src2[8], MI_F64 dst[8])
{
    const MI_S32 n = 8;
    MI_F64 w[n]    = { 0.0 };
    MI_F64 v[n][n] = {{ 0.0 }};
...
static AURA_VOID Gemm1Trans3Trans(MI_F64 src1[][8], MI_F64 src2[], MI_F64 alpha, MI_F64 src3[], MI_F64 beta, MI_F64 dst[])
{
    const MI_S32 rows          = 8;
    MI_F64 src1_trans[8][rows] = {{ 0 }};
    MI_S32 i                   = 0;
    MI_S32 j                   = 0;
...
static AURA_VOID SVBkSbnb8(MI_S32 n, MI_F64 w[], MI_F64 u[][8], MI_F64 v[][8], MI_F64 x[8][8])
{
    MI_F64 buffer[8] = { 0 };
    MI_F64 eps       = DBL_EPSILON * 2;
    MI_F64 threshold = 0.0;
...
static AURA_VOID InvertEig(MI_F64 src[][8], MI_F64 dst[][8])
{
    const MI_S32 n  = 8;
    MI_F64 u[n][n]  = {{ 0.0 }};
    MI_F64 w[n]     = { 0.0 };
...
static Status LMSolver(Context *ctx, std::vector<Point2> &src_points, std::vector<Point2> &dst_points, MI_S32 max_iters, MI_F64 h[8])
{
    MI_S32 in_num  = (MI_S32)src_points.size() * 2;
    MI_F64 *buffer = reinterpret_cast<MI_F64*>(AURA_ALLOC_PARAM(ctx, AURA_MEM_HEAP, 10 * sizeof(MI_F64) * in_num, 0));
    if (MI_NULL == buffer)
    {
        AURA_ADD_ERROR_STRING(ctx, "AURA_ALLOC_PARAM failed");
        return Status::ERROR;
    ...
    MI_F64 *pr  = buffer;
    MI_F64 *prd = buffer + in_num;
    MI_F64 *pj  = buffer + in_num * 2;

    const MI_S32 m   = 8;
    MI_F64 x[m]      = { h[0], h[1], h[2], h[3], h[4], h[5], h[6], h[7] };
    MI_F64 a[m][m]   = {{ 0.0 }};
    MI_F64 ap[m][m]  = {{ 0.0 }};
    MI_F64 v[m]      = { 0.0 };
    MI_F64 d[m]      = { 0.0 };
    ...
    const MI_F64 rlo = 0.25;
    const MI_F64 rhi = 0.75;
    MI_F64 lambda    = 1;
    MI_F64 lc        = 0.75;
    MI_F64 ds        = 0.0;
    MI_F64 r         = 0.0;
    MI_F64 maxval    = DBL_EPSILON;
    MI_S32 iter      = 0;
    MI_S32 proceed   = 1;
    MI_S32 i         = 0;
    MI_S32 j         = 0;
    ...
    for (;;)
    {
        ...
        else if (r < rlo)
        {
            ...
            MI_F64 t  = VectorDot(d, v);
            MI_F64 nu = (sd - s) / (Abs(t) > DBL_EPSILON ? t : 1) + 2;
            nu = Min(Max(nu, (MI_F64)2.0), (MI_F64)10.0);
...
static Status CalHomography(const std::vector<Point2> &src_points, const std::vector<Point2> &dst_points, Mat &h_mat)
{
    ...
    {
        src_cm[0] += (MI_F64)src_points[i].m_x;
        src_cm[1] += (MI_F64)src_points[i].m_y;
        dst_cm[0] += (MI_F64)dst_points[i].m_x;
        dst_cm[1] += (MI_F64)dst_points[i].m_y;
    }
    ...
    for (i = 0; i < points_num; i++)
    {
        MI_F64 X = (src_points[i].m_x - src_cm[0]) * src_sm[0];
        MI_F64 Y = (src_points[i].m_y - src_cm[1]) * src_sm[1];
        MI_F64 x = (dst_points[i].m_x - dst_cm[0]) * dst_sm[0];
        MI_F64 y = (dst_points[i].m_y - dst_cm[1]) * dst_sm[1];

        MI_F64 lx[n] = { X, Y, 1.f, 0.f, 0.f, 0.f, -x * X, -x * Y, -x };
        MI_F64 ly[n] = { 0.f, 0.f, 0.f, X, Y, 1.f, -y * X, -y * Y, -y };
    ...
    MI_F64 norm_h0[3][3] = {{ 1. / dst_sm[0], 0, dst_cm[0] }, { 0, 1. / dst_sm[1], dst_cm[1] }, { 0, 0, 1 }};
    MI_F64 norm_h1[3][3] = {{ src_sm[0], 0, -src_cm[0] * src_sm[0] }, { 0, src_sm[1], -src_cm[1] * src_sm[1] }, { 0, 0, 1 }};

    MI_F64 result_h[3][3] = {{ 0.0 }};
    for (i = 0; i < 3; i++)
    {
...
static Status FindHomographyRansacImpl(Context *ctx, const std::vector<Point2> &src_points, const std::vector<Point2> &dst_points, Mat &h_mat,
                                       MI_F64 reproj_threshold, MI_S32 max_iters, MI_F64 confidence)
{
    Status ret = Status::ERROR;

    MI_S32 max_inliers = MODEL_POINTS_NUM;
    MI_S32 inliers_num = 0;
    MI_U64 state       = 0xffffffffffffffff;
    MI_S32 niters      = max_iters;
    MI_S32 points_num  = (MI_S32)src_points.size();
    ...
    MI_S32 *mask_iter   = (MI_S32 *)mask_mat.Ptr<MI_S32>(0);
    MI_S32 *mask_result = (MI_S32 *)mask_mat.Ptr<MI_S32>(1);

    if (MODEL_POINTS_NUM == points_num)
    {
        for (MI_S32 i = 0; i < MODEL_POINTS_NUM; i++)
        {
    ...
    for (MI_S32 iter = 0; iter < niters; iter++)
    {
        if (points_num > MODEL_POINTS_NUM)
        {
            MI_BOOL found = GetSubPoints(src_points, dst_points, src_model_points, dst_model_points, MAX_ATTEMPTS, state);
            if (found)
            {
                if (0 == iter)
                {
        ...
        if (inliers_num >= max_inliers)
        {
            ...
            niters = RansacUpdateNumIters(confidence, (MI_F32)(points_num - inliers_num) / points_num, MODEL_POINTS_NUM, niters);
        }
    }
    ...
    MI_F64 h8[8]  = {0.0};
    MI_F64 *dst_h = (MI_F64*)h_mat.Ptr<MI_F64>(0);
