...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Prepare(MVqType *mvq_src_p, MVqType *mvq_src_c, MVqType *mvq_src_n)
{
...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Core(uint8x16_t &vqu8_src_x0, uint8x16_t &vqu8_src_x1, uint8x16_t &vqu8_src_x2,
...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Core(d16x8_t &vq16_src_x0, d16x8_t &vq16_src_x1, d16x8_t &vq16_src_x2,
...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Core(float32x4_t &vqf32_src_x0, float32x4_t &vqf32_src_x1, float32x4_t &vqf32_src_x2,
...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Core(float16x8_t &vqf16_src_x0, float16x8_t &vqf16_src_x1, float16x8_t &vqf16_src_x2,
                                           float32x4_t &vqf32_result_lo, float32x4_t &vqf32_result_hi,
                                           const std::vector<MI_F32> &kernel, MI_S32 line)
{
    const MI_S32 idx = 3 * line;
...
AURA_ALWAYS_INLINE AURA_VOID Filter2d3x3Row(const Tp *src_p, const Tp *src_c, const Tp *src_n, Tp *dst, MI_S32 width,
...
template <typename Tp, BorderType BORDER_TYPE, MI_S32 C>
static Status Filter2d3x3NeonImpl(const Mat &src, Mat &dst, const std::vector<MI_F32> &kdata,
                                  const std::vector<Tp> &border_value, const Tp *border_buffer, MI_S32 start_row, MI_S32 end_row)
{
...
static Status Filter2d3x3NeonHelper(Context *ctx, const Mat &src, Mat &dst, const std::vector<MI_F32> &kdata,
                       ...
                       BorderType border_type, const Scalar &border_value, const OpTarget &target)
{
    ...
    switch (pattern)
    {
        ...
        {
            ret = Filter2d3x3NeonHelper<MI_U8>(ctx, src, dst, kdata, border_type, border_value, target);
            break;
        }

        case AURA_MAKE_PATTERN(ElemType::U16, ElemType::U16):
        {
            ret = Filter2d3x3NeonHelper<MI_U16>(ctx, src, dst, kdata, border_type, border_value, target);
            break;
        }

        case AURA_MAKE_PATTERN(ElemType::S16, ElemType::S16):
        {
            ret = Filter2d3x3NeonHelper<MI_S16>(ctx, src, dst, kdata, border_type, border_value, target);
            break;
        }
        ...
        {
            ret = Filter2d3x3NeonHelper<MI_F32>(ctx, src, dst, kdata, border_type, border_value, target);
            break;
        }
