...
int OpProfiler::_k_test_times = 10;
static int k_log_batch_size = 5;
static const char *TAG = "CvProfiler";

void OpProfiler::profile(TestFuncList &func_list,
...
void OpProfiler::print_results(std::vector <SpeedResult> &speed_profile, std::vector <OutputResult> &output_result) {
...
void OpProfiler::save_results(std::string tag, std::vector<long> &records) {
    // todo
}
