...
{
    // Tp = MI_U8, MI_U16 MI_S16 MI_F16 MI_F32
    // 表达式拆分
    // 表达式1 = std::is_same<Tp, MI_U8>::value,   这是类型特性工具 std::is_same 的调用，用于判断模板参数 TP 是否与 U8 是同一类型
    ...
    using KernelType = typename std::conditional<std::is_same<Tp, MI_U8>::value, typename Promote<Tp>::Type, MI_F32>::type;
    // Tp = MI_U8, MI_U16 MI_S16 MI_F16 MI_F32
    static constexpr MI_U32 Q = std::is_same<Tp, MI_U8>::value ? 8 : 0;
};
...
static AURA_VOID GetCLGlobalSize(GpuType gpu_type, MI_S32 ksize, MI_S32 channel, MI_S32 height,
                               MI_S32 width, ElemType elem_type, cl::NDRange range[2], MI_S32 &main_width)
{
 ...
 * @return
 */
Status GaussianCL::SetArgs(const Array *src, Array *dst, MI_S32 ksize, MI_F32 sigma,
...
Status GaussianCL::PrepareKmat()
{
    ...
    switch (m_src->GetElemType())
    {
        ...
        {
            /// 或是高斯核的类型
            using KernelType   = GaussianTraits<MI_U8>::KernelType;
            constexpr MI_U32 Q = GaussianTraits<MI_U8>::Q;
...
Status GaussianCL::Run()
{
    ...
    cl_ret |= m_cl_kernels[1].Run<cl::Buffer, MI_S32, cl::Buffer, MI_S32, MI_S32, MI_S32, MI_S32, MI_S32, MI_S32, cl::Buffer, CLScalar>(
                                  m_cl_src.GetCLMemRef<cl::Buffer>(), istep,
                                  m_cl_dst.GetCLMemRef<cl::Buffer>(), ostep,
...
std::vector<CLKernel> GaussianCL::GetCLKernels(Context *ctx, ElemType elem_type, MI_S32 channel, MI_S32 ksize, BorderType border_type)
{
