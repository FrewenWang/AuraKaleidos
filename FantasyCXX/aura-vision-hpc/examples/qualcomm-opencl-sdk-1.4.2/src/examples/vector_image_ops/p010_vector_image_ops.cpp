...
// Desc: Demonstrates use of vector image ops for p010 using Qualcomm extensions to OpenCL. These
//       functions read and write several pixels at once. This example has several kernels
//       which simply copy an image, and verifies each result at runtime, indicating on
//       std err if there is an error. There is no output image.
//
// Author:      QUALCOMM
//
...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; i < kernel_execution_params.size(); ++i)
    {
        ...
        {
            std::cerr << "Error preparing the DMA buffer for reads and writes. The DMA_BUF_IOCTL_SYNC operation returned "
                      << strerror(errno) << "!\n";
            return errno;
        }
        ...
        err = clEnqueueUnmapMemObject(command_queue, comparison_plane, src_image_ptr, 0, nullptr, nullptr);
        if (err != CL_SUCCESS)
        {
            std::cerr << "On iteration " << i << ":\n";
        ...
        err = clEnqueueUnmapMemObject(command_queue, dst_plane, dst_image_ptr, 0, nullptr, &unmap_event);
        if (err != CL_SUCCESS)
        {
            std::cerr << "On iteration " << i << ":\n";
        ...
        if (ioctl(out_p010_mem.dmabuf_filedesc, DMA_BUF_IOCTL_SYNC, &buf_sync))
        {
            std::cerr << "Error finalizing the DMA buffer for reads and writes. The DMA_BUF_IOCTL_SYNC operation returned "
                      << strerror(errno) << "!\n";
            return errno;
        }
    }
