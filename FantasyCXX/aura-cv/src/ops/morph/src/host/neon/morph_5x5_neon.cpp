...
AURA_ALWAYS_INLINE DT_VOID Morph5x5Vector(VqTpye &vq_src_p1x0, VqTpye &vq_src_p1x1, VqTpye &vq_src_p1x2,
...
template <typename Tp, MorphShape MORPH_SHAPE, MorphType MORPH_TYPE, DT_S32 ELEM_COUNTS,
          typename VqTpye = typename neon::QVector<Tp>::VType,
          typename std::enable_if<MorphShape::RECT == MORPH_SHAPE, Tp>::type* = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID Morph5x5Vector(VqTpye &vq_src_p1x2, VqTpye &vq_src_p0x2, VqTpye &vq_src_cx0,
...
template <typename Tp, MorphShape MORPH_SHAPE, MorphType MORPH_TYPE, DT_S32 ELEM_COUNTS,
          typename VqTpye = typename neon::QVector<Tp>::VType,
          typename std::enable_if<MorphShape::CROSS == MORPH_SHAPE, Tp>::type* = DT_NULL>
AURA_ALWAYS_INLINE DT_VOID Morph5x5Vector(VqTpye &vq_src_p1x2, VqTpye &vq_src_p0x2, VqTpye &vq_src_cx0,  VqTpye &vq_src_cx1,
...
static DT_VOID Morph5x5Row(const Tp *src_p1, const Tp *src_p0, const Tp *src_c, const Tp *src_n0,
                           const Tp *src_n1, Tp *dst, DT_S32 width)
{
...
template <typename Tp, MorphShape MORPH_SHAPE, MorphType MORPH_TYPE, DT_S32 C>
static Status Morph5x5NeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_S32 start_row, DT_S32 end_row)
{
    AURA_UNUSED(ctx);
...
Status Morph5x5Neon(Context *ctx, const Mat &src, Mat &dst, MorphType type, MorphShape shape, const OpTarget &target)
{
    ...
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            ret = Morph5x5NeonHelper<DT_U8>(ctx, src, dst, type, shape, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Morph5x5NeonHelper<DT_U8> run failed!");
            }
            break;
        }
        ...
        case ElemType::U16:
        {
            ret = Morph5x5NeonHelper<DT_U16>(ctx, src, dst, type, shape, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Morph5x5NeonHelper<DT_U16> run failed!");
            }
            break;
        }
        ...
        case ElemType::S16:
        {
            ret = Morph5x5NeonHelper<DT_S16>(ctx, src, dst, type, shape, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Morph5x5NeonHelper<DT_S16> run failed!");
            }
            break;
        }
        ...
        case ElemType::F32:
        {
            ret = Morph5x5NeonHelper<DT_F32>(ctx, src, dst, type, shape, target);
            if (ret != Status::OK)
            {
                AURA_ADD_ERROR_STRING(ctx, "Morph5x5NeonHelper<DT_F32> run failed!");
            }
            break;
        }
