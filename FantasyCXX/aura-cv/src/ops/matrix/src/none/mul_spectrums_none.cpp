...
static Status MulSpectrumsNoneImpl(const Mat &src0, const Mat &src1, Mat &dst, DT_BOOL conj_src1, DT_S32 start_row, DT_S32 end_row)
{
    Sizes3 sz              = src0.GetSizes();
    DT_S32 width           = sz.m_width;
    DT_S32 row_elem_counts = width * sz.m_channel;

    for (DT_S32 y = start_row; y < end_row; ++y)
    {
        const DT_F32 *src0_row = src0.Ptr<DT_F32>(y);
        const DT_F32 *src1_row = src1.Ptr<DT_F32>(y);
        DT_F32 *dst_row        = dst.Ptr<DT_F32>(y);

        for (DT_S32 x = 0; x < row_elem_counts; x += 2)
        {
            DT_F32 real0 = src0_row[x];
            DT_F32 imag0 = src0_row[x + 1];

            DT_F32 real1 = src1_row[x];
            DT_F32 imag1 = conj_src1 ? -src1_row[x + 1] : src1_row[x + 1];
...
Status MulSpectrumsNone::SetArgs(const Array *src0, const Array *src1, Array *dst, DT_BOOL conj_src1)
{
...
Status MulSpectrumsNone::Run()
{
    ...
    if (m_target.m_data.none.enable_mt)
    {
        ...
        ret = wp->ParallelFor(static_cast<DT_S32>(0), height, MulSpectrumsNoneImpl, std::cref(*src0), std::cref(*src1),
                              std::ref(*dst), m_conj_src1);
    }
    else
    {
        ret = MulSpectrumsNoneImpl(*src0, *src1, *dst, m_conj_src1, static_cast<DT_S32>(0), height);
    }
