...
# ``paddle.fluid`` was removed after Paddle 2.4.  The same low-level helpers
# live under ``paddle.base`` in Paddle 2.6, which is the last 2.x release with
# an Intel macOS wheel.  Keep a fallback so the historical cloud environment
...
def yolo_box(
    ...
    if in_dygraph_mode():
        # ``core.ops`` was a private API and no longer exists in Paddle 2.6.
        # The public vision operator has equivalent semantics and remains
        # available in both legacy 2.x and current Paddle releases.
...
def multiclass_nms(bboxes,
    ...
    if in_dygraph_mode():
        if len(bboxes.shape) != 3 or len(scores.shape) != 3:
            raise ValueError(
                "The Paddle 2.6 compatibility NMS expects bboxes [N,M,4] "
                "and scores [N,C,M].")

        batch_outputs = []
        batch_indices = []
        batch_counts = []
        box_count = int(bboxes.shape[1])
        for batch_id in range(int(bboxes.shape[0])):
            image_outputs = []
            image_indices = []
            for class_id in range(int(scores.shape[1])):
                if class_id == background_label:
                    continue
                class_scores = scores[batch_id, class_id]
                selected = paddle.nonzero(
                    class_scores > score_threshold).flatten()
                if int(selected.shape[0]) == 0:
                    continue
                selected_scores = paddle.gather(class_scores, selected)
                if nms_top_k > 0 and int(selected.shape[0]) > nms_top_k:
                    selected_scores, order = paddle.topk(
                        selected_scores, k=nms_top_k)
                    selected = paddle.gather(selected, order)
                selected_boxes = paddle.gather(bboxes[batch_id], selected)
                keep = paddle.vision.ops.nms(
                    selected_boxes,
                    iou_threshold=nms_threshold,
                    scores=selected_scores,
                )
                kept_boxes = paddle.gather(selected_boxes, keep)
                kept_scores = paddle.gather(selected_scores, keep)
                labels = paddle.full(
                    [int(keep.shape[0]), 1],
                    float(class_id),
                    dtype=bboxes.dtype,
                ...
                image_outputs.append(
                    paddle.concat(
                        [labels, kept_scores.unsqueeze(1), kept_boxes], axis=1))
                absolute = paddle.gather(selected, keep) + batch_id * box_count
                image_indices.append(absolute.unsqueeze(1))

            if image_outputs:
                image_output = paddle.concat(image_outputs, axis=0)
                image_index = paddle.concat(image_indices, axis=0)
                order = paddle.argsort(image_output[:, 1], descending=True)
                if keep_top_k > 0:
                    order = order[:keep_top_k]
                image_output = paddle.gather(image_output, order)
                image_index = paddle.gather(image_index, order)
            else:
                image_output = paddle.zeros([0, 6], dtype=bboxes.dtype)
                image_index = paddle.zeros([0, 1], dtype='int64')
            batch_outputs.append(image_output)
            batch_indices.append(image_index)
            batch_counts.append(int(image_output.shape[0]))

        output = (
            paddle.concat(
                [item for item in batch_outputs if int(item.shape[0]) > 0],
                axis=0)
            if any(batch_counts)
            else paddle.zeros([0, 6], dtype=bboxes.dtype)
        )
        nms_rois_num = paddle.to_tensor(batch_counts, dtype='int32')
        index = None
        if return_index:
            index = (
                paddle.concat(
                    [item for item in batch_indices if int(item.shape[0]) > 0],
                    axis=0)
                if any(batch_counts)
                else paddle.zeros([0, 1], dtype='int64')
            )
