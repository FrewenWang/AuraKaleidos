...
static Status GridDft4x4NeonImpl(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    {
        DT_F32 buffer_roi[16] = {0};
        for (DT_S32 y = start * 4; y < end * 4; y += 4)
        {
            const std::complex<DT_F32> *dst_row0 = dst.Ptr<std::complex<DT_F32>>(y);
            const std::complex<DT_F32> *dst_row1 = dst.Ptr<std::complex<DT_F32>>(y + 1);
            const std::complex<DT_F32> *dst_row2 = dst.Ptr<std::complex<DT_F32>>(y + 2);
            const std::complex<DT_F32> *dst_row3 = dst.Ptr<std::complex<DT_F32>>(y + 3);
            for (DT_S32 x = 0; x < width; x += 4)
            {
                DT_F32 *dst_shift0 = (DT_F32 *)(dst_row0 + x);
                DT_F32 *dst_shift1 = (DT_F32 *)(dst_row1 + x);
                DT_F32 *dst_shift2 = (DT_F32 *)(dst_row2 + x);
                DT_F32 *dst_shift3 = (DT_F32 *)(dst_row3 + x);

                for (DT_S32 y_shift = 0; y_shift < 4; y_shift++)
                {
                    const Tp *src_row = src.Ptr<Tp>(y + y_shift);
                    for (DT_S32 x_shift = 0; x_shift < 4; x_shift++)
                    {
                        buffer_roi[y_shift * 4 + x_shift] = SaturateCast<DT_F32>(src_row[x + x_shift]);
                    }
                }
...
template <typename T>
static DT_VOID RowDftNeon1x8(T *src, std::complex<DT_F32> *dst, std::complex<DT_F32> *row_real_exp_table)
{
    DT_F32 conj[2] = {1.0f, -1.0f};
    float32x2_t vdf32_conj = neon::vload1(conj);
    ...
    float32x2_t vdf32_src0 = neon::vload1((DT_F32 *)(&src0));
    float32x2_t vdf32_src1 = neon::vload1((DT_F32 *)(&src1));
    float32x2_t vdf32_src2 = neon::vload1((DT_F32 *)(&src2));
    float32x2_t vdf32_src3 = neon::vload1((DT_F32 *)(&src3));

    std::complex<DT_F32>fk_scale(0.5f, 0.0f);
    std::complex<DT_F32>gk_scale(0.0f, 0.5f);

    float32x2_t vdf32_fk_scale = neon::vload1((DT_F32 *)(&fk_scale));
    float32x2_t vdf32_gk_scale = neon::vload1((DT_F32 *)(&gk_scale));
    float32x2_t vdf32_exp1     = neon::vload1((DT_F32 *)(row_real_exp_table + 1));
    float32x2_t vdf32_exp3     = neon::vload1((DT_F32 *)(row_real_exp_table + 3));
...
static DT_VOID ColButterflyCalc(std::complex<DT_F32> src[][8], DT_BOOL is_inverse)
{
    DT_F32 conj0[4] = { 1.0f, -1.0f,  1.0f, -1.0f};
    DT_F32 conj1[4] = {-1.0f,  1.0f, -1.0f,  1.0f};

    float32x4_t vqf32_conj;
    vqf32_conj = is_inverse ? neon::vload1q(conj1) : neon::vload1q(conj0);

    DT_F32 *line0 = (DT_F32 *)(&src[0][0]);
    DT_F32 *line1 = (DT_F32 *)(&src[1][0]);
    DT_F32 *line2 = (DT_F32 *)(&src[2][0]);
    DT_F32 *line3 = (DT_F32 *)(&src[3][0]);
    DT_F32 *line4 = (DT_F32 *)(&src[4][0]);
    DT_F32 *line5 = (DT_F32 *)(&src[5][0]);
    DT_F32 *line6 = (DT_F32 *)(&src[6][0]);
    DT_F32 *line7 = (DT_F32 *)(&src[7][0]);
...
static DT_VOID ColGridDft8x8(std::complex<DT_F32> src[][8], std::complex<DT_F32> *dst, DT_S32 ostep)
{
    // butter fly size is 2 and 4, and only update src
    ColButterflyCalc(src, DT_FALSE);

    // butterfly size is 8, and store result to dst
    DT_F32 exp1[8] = { 0.707107f, -0.707107f,  0.707107f, -0.707107f,  0.707107f, -0.707107f,  0.707107f, -0.707107f};
    DT_F32 exp3[8] = {-0.707107f, -0.707107f, -0.707107f, -0.707107f, -0.707107f, -0.707107f, -0.707107f, -0.707107f};
    ...
    DT_F32 *dst0 = (DT_F32 *)(&dst[0]);
    DT_F32 *dst1 = (DT_F32 *)(&dst[1 * ostep]);
    DT_F32 *dst2 = (DT_F32 *)(&dst[2 * ostep]);
    DT_F32 *dst3 = (DT_F32 *)(&dst[3 * ostep]);
    DT_F32 *dst4 = (DT_F32 *)(&dst[4 * ostep]);
    DT_F32 *dst5 = (DT_F32 *)(&dst[5 * ostep]);
    DT_F32 *dst6 = (DT_F32 *)(&dst[6 * ostep]);
    DT_F32 *dst7 = (DT_F32 *)(&dst[7 * ostep]);
...
static Status GridDft8x8NeonImpl(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        std::complex<DT_F32> tmp_conv[8][8];
        for (DT_S32 y = start * 8; y < end * 8; y += 8)
        {
...
static Status GridDftNeonNxN(const Mat &src, Mat &dst, DT_U16 *idx_x_table, DT_U16 *idx_y_table,
                      ...
                      DT_S32 width_offset, DT_S32 height_offset)
{
    ...
    for (DT_S32 y_grid = 0; y_grid < GRID_LEN; y_grid++)
    {
        ...
        ButterflyTransformNeon(buffer, 2, HALF_GRID, DT_FALSE, dft_row_exp_table);
    ...
    for (DT_S32 size = 2; size < GRID_LEN; size *= 2)
    {
        ...
        for (DT_S32 i = 0; i < GRID_LEN; i += size)
        {
            ...
            DT_S32 k = 0;
            {
                ...
                for (DT_S32 row_index = 0; row_index < GRID_LEN; row_index += 4)
                {
                    DT_F32 *src_complex0 = (DT_F32 *)(&dst_complex[j][row_index]);
                    DT_F32 *src_complex1 = (DT_F32 *)(&dst_complex[j + half_size][row_index]);
    ...
    for (DT_S32 j = 0; j < HALF_GRID; j++)
    {
        ...
        for (DT_S32 row_index = 0; row_index < GRID_LEN; row_index += 4)
        {
            DT_F32 *src_complex0 = (DT_F32 *)(row_result_shift      + row_index);
            DT_F32 *src_complex1 = (DT_F32 *)(row_result_half_shift + row_index);
            DT_F32 *dst_data0    = (DT_F32 *)(dst_shift      + width_offset + row_index);
            DT_F32 *dst_data1    = (DT_F32 *)(dst_half_shift + width_offset + row_index);
...
static Status GridDftNeonImpl(Context *ctx, const Mat &src, Mat &dst, const OpTarget &target)
{
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        for (DT_S32 y = start * GRID_LEN; y < end * GRID_LEN; y += GRID_LEN)
        {
...
template <typename Tp>
static Status GridDftNeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_S32 grid_len, const OpTarget &target)
{
...
Status GridDftNeon::SetArgs(const Array *src, Array *dst, DT_S32 grid_len)
{
...
Status GridDftNeon::Run()
{
    ...
    switch (src->GetElemType())
    {
        case ElemType::U8:
        {
            ret = GridDftNeonImpl<DT_U8>(m_ctx, *src, *dst, m_grid_len, m_target);
            break;
        }
        case ElemType::S8:
        {
            ret = GridDftNeonImpl<DT_S8>(m_ctx, *src, *dst, m_grid_len, m_target);
            break;
        }
        case ElemType::U16:
        {
            ret = GridDftNeonImpl<DT_U16>(m_ctx, *src, *dst, m_grid_len, m_target);
            break;
        }
        case ElemType::S16:
        {
            ret = GridDftNeonImpl<DT_S16>(m_ctx, *src, *dst, m_grid_len, m_target);
            break;
        }
        ...
        case ElemType::F32:
        {
            ret = GridDftNeonImpl<DT_F32>(m_ctx, *src, *dst, m_grid_len, m_target);
            break;
        }
        default:
        {
...
AURA_ALWAYS_INLINE DT_VOID IDft4x4NeonImpl(const std::complex<DT_F32> *src0, const std::complex<DT_F32> *src1,
                                           const std::complex<DT_F32> *src2, const std::complex<DT_F32> *src3,
                                           DT_BOOL with_scale, Tp *dst, DT_S32 ostep)
{
    ...
    vqf32_src00 = neon::vload1q((DT_F32 *)src0); vqf32_src01 = neon::vload1q((DT_F32 *)(src0 + 2));
    vqf32_src10 = neon::vload1q((DT_F32 *)src1); vqf32_src11 = neon::vload1q((DT_F32 *)(src1 + 2));
    vqf32_src20 = neon::vload1q((DT_F32 *)src2); vqf32_src21 = neon::vload1q((DT_F32 *)(src2 + 2));
    vqf32_src30 = neon::vload1q((DT_F32 *)src3); vqf32_src31 = neon::vload1q((DT_F32 *)(src3 + 2));
...
template <typename Tp, DT_S32 C>
static Status GridIDft4x4NeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        for (DT_S32 h = start * 4; h < end * 4; h += 4)
        {
...
static DT_VOID RowGridIDft1x8(const std::complex<DT_F32> *src, std::complex<DT_F32> *dst, DT_BOOL with_scale)
{
    ...
    DT_F32 conj[2] = {-1.0f, 1.0f};
    DT_F32 exp0[2] = {0.707107f, 0.707107f};
    DT_F32 exp1[2] = {-0.707107f, 0.707107f};
    ...
    vqf32_src0 = neon::vload1q((DT_F32 *)src);
    vqf32_src1 = neon::vload1q((DT_F32 *)(src + 2));
    vqf32_src2 = neon::vload1q((DT_F32 *)(src + 4));
    vqf32_src3 = neon::vload1q((DT_F32 *)(src + 6));
    ...
    neon::vstore((DT_F32 *)(dst),     vqf32_dst0);
    neon::vstore((DT_F32 *)(dst + 2), vqf32_dst1);
    neon::vstore((DT_F32 *)(dst + 4), vqf32_dst2);
    neon::vstore((DT_F32 *)(dst + 6), vqf32_dst3);
}

template <typename Tp, DT_S32 C>
static DT_VOID ColGridIDft8x8(std::complex<DT_F32> src[][8], DT_BOOL with_scale, Tp *dst, DT_S32 ostep)
{
    float32x4_t vqf32_n;
    if (with_scale)
    {
    ...
    {
        DT_F32 *dst0 = (DT_F32 *)dst;
        DT_F32 *dst2 = dst0 + 2 * ostep;
        DT_F32 *dst4 = dst0 + 4 * ostep;
        DT_F32 *dst6 = dst0 + 6 * ostep;
    ...
    {
        DT_F32 *dst0 = (DT_F32 *)dst;
        DT_F32 *dst1 = dst0 + 1 * ostep;
        DT_F32 *dst3 = dst0 + 3 * ostep;
        DT_F32 *dst5 = dst0 + 5 * ostep;
        DT_F32 *dst7 = dst0 + 7 * ostep;
...
template <typename Tp, DT_S32 C>
static Status GridIDft8x8NeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
    Sizes3 size    = dst.GetSizes();
    DT_S32 width   = size.m_width;
    ...
    auto row_process_func = [&](DT_S32 start, DT_S32 end)->Status
    {
        for (DT_S32 h = start * 8; h < end * 8; h += 8)
        {
...
static Status GridIDftNeonNxN(const Mat &src, Mat &dst, DT_U16 *idx_table,
                              std::complex<DT_F32> *exp_table, DT_BOOL with_scale,
                              DT_S32 width_offset, DT_S32 height_offset)
{
    ...
    {
        float32x4_t vqf32_scale;
        neon::vdup(vqf32_scale, (DT_F32)GRID_LEN);
        vqf32_n = neon::vreciprocal_newton(vqf32_scale);
    }
    else
    {
    ...
    for (DT_S32 size = 2; size < GRID_LEN; size *= 2)
    {
        ...
        for (DT_S32 i = 0; i < GRID_LEN; i += size)
        {
            ...
            DT_S32 k = 0;
            {
                ...
                for (DT_S32 row_index = 0; row_index < GRID_LEN; row_index += 4)
                {
                    DT_F32 *src_complex0 = (DT_F32 *)(&row_dft_result[j][row_index]);
                    DT_F32 *src_complex1 = (DT_F32 *)(&row_dft_result[j + half_size][row_index]);
    ...
    for (DT_S32 j = 0; j < HALF_GRID; j++)
    {
        ...
        for (DT_S32 row_index = 0; row_index < GRID_LEN; row_index += 8)
        {
            DT_F32 *src_complex0 = (DT_F32 *)(row_result_shift      + row_index);
            DT_F32 *src_complex1 = (DT_F32 *)(row_result_half_shift + row_index);
            ...
            {
                DT_F32 *dst_data0 = (DT_F32 *)(dst_shift      + (width_offset + row_index) * 2);
                DT_F32 *dst_data1 = (DT_F32 *)(dst_half_shift + (width_offset + row_index) * 2);
...
template <typename Tp, DT_S32 GRID_LEN, DT_S32 C>
static Status GridIDftNeonNxNImpl(Context *ctx, const Mat &src, Mat &dst, DT_BOOL with_scale, const OpTarget &target)
{
    AURA_UNUSED(target);
...
template <typename Tp, DT_S32 C>
static Status GridIDftNeonImpl(Context *ctx, const Mat &src, Mat &dst, DT_S32 grid_len, DT_BOOL with_scale, const OpTarget &target)
{
    Status ret = Status::OK;
    switch (grid_len)
    {
...
template <typename Tp>
static Status GridIDftNeonHelper(Context *ctx, const Mat &src, Mat &dst, DT_S32 grid_len, DT_BOOL with_scale, const OpTarget &target)
{
...
Status GridIDftNeon::SetArgs(const Array *src, Array *dst, DT_S32 grid_len, DT_BOOL with_scale)
{
    ...
    if (1 == dst_channel)
    {
        ...
        {
            AURA_ADD_ERROR_STRING(m_ctx, "when dst channel is 1, current dst does not support DT_S32/DT_U32/DT_F64 type.");
            return Status::ERROR;
        }
    }
...
Status GridIDftNeon::Run()
{
    ...
    switch (dst->GetElemType())
    {
        case ElemType::U8:
        {
            ret = GridIDftNeonHelper<DT_U8>(m_ctx, *src, *dst, m_grid_len, m_with_scale, m_target);
            break;
        }
        case ElemType::S8:
        {
            ret = GridIDftNeonHelper<DT_S8>(m_ctx, *src, *dst, m_grid_len, m_with_scale, m_target);
            break;
        }
        case ElemType::U16:
        {
            ret = GridIDftNeonHelper<DT_U16>(m_ctx, *src, *dst, m_grid_len, m_with_scale, m_target);
            break;
        }
        case ElemType::S16:
        {
            ret = GridIDftNeonHelper<DT_S16>(m_ctx, *src, *dst, m_grid_len, m_with_scale, m_target);
            break;
        }
# if defined(AURA_ENABLE_NEON_FP16)
        case ElemType::F16:
...
# endif
        case ElemType::F32:
        {
            ret = GridIDftNeonHelper<DT_F32>(m_ctx, *src, *dst, m_grid_len, m_with_scale, m_target);
            break;
        }
        default:
        {
