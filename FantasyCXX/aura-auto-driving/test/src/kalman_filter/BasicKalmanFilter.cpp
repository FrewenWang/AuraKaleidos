...
void BasicKalmanFilter::init(const Eigen::VectorXd &x0) {
	x_hat = x0;
	P = P0;
	initialized = true;
}
...
void BasicKalmanFilter::predict(const Eigen::VectorXd &u) {
	if (!initialized) {
...
void BasicKalmanFilter::update_dynamics(const Eigen::MatrixXd A) {
	this->A = A;
}

void BasicKalmanFilter::update_output(const Eigen::MatrixXd C) {
	this->C = C;
}