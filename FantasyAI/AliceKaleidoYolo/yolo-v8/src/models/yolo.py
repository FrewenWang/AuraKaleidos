...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 ...
                 GSConv, VoVGSCSP, GPFN_Block, CSPNeXtLayer, C2f):
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, C3x, GSConv, VoVGSCSP, GPFN_Block, CSPNeXtLayer, C2f]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./hub_plus/yolov5s_pai_tal_head.yaml', help='model.yaml')
