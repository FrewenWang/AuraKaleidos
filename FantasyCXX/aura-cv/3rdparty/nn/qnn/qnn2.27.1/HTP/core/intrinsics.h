...
inline HVX_Vector ALWAYSINLINE q6op_Vw_vasl_VwVw_sat(const HVX_Vector vin, const HVX_Vector vshift)
{
    ...
    const HVX_Vector vnormbits = Q6_Vw_vnormamt_Vw(vin); //count lead bits - 1, handles 0 special case
    const HVX_VectorPred qlt_lb = Q6_Q_vcmp_gt_VwVw(vshift, vnormbits);

    return Q6_V_vmux_QVV(qlt_lb, vsat, Q6_Vw_vasl_VwVw(vin, vshift));
}
