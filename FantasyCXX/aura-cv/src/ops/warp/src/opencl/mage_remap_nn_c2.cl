...
kernel void RemapNnC2(read_only iaura2d_t src, int iheight, int iwidth,
...
#pragma unroll
    for (int i = 0; i < ELEM_HEIGHT; i++, y_idx++)
    {
        ...
        global MapType *map_row = map + mad24(y_idx, mstep, x_idx);
        VS32 vs32_offset        = REMAP_CONVERT_MAP(VLOAD(map_row, ELEM_LENGTH), VS32, rte);
