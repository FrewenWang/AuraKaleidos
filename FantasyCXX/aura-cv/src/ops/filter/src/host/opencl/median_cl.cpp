...
static Status GetCLName(Context *ctx, MI_S32 channel, MI_S32 ksize, std::string &program_name, std::string &kernel_name)
{
    if (MI_NULL == ctx)
    {
        return Status::ERROR;
...
Status MedianCL::SetArgs(const Array *src, Array *dst, MI_S32 ksize)
{
...
std::vector<CLKernel> MedianCL::GetCLKernels(Context *ctx, ElemType elem_type, MI_S32 channel, MI_S32 ksize)
{
