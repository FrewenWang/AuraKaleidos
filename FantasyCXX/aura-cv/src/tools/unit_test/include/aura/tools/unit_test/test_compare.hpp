...
struct AURA_EXPORTS RelativeDiff
{
    ...
    MI_F64 operator()(const MI_F64 val0, const MI_F64 val1) const
    {
        ...
        {
            MI_F64 abs_diff = Abs(val0 - val1);
            MI_F64 max_abs_val = Max(Abs(val0), Abs(val1));
...
struct AURA_EXPORTS MatCmpPos
{
     ...
     */
    MatCmpPos(MI_S32 h, MI_S32 w, MI_S32 c, MI_F64 src, MI_F64 ref, MI_F64 diff)
 ...
 */
AURA_INLINE MI_BOOL operator>(const MatCmpPos &p0, const MatCmpPos &p1)
{
...
struct ArrayCmpPos
{
     ...
     * @param diff Absolute difference between source and reference values.
     */
    ArrayCmpPos(MI_S32 pos = -1, MI_F64 src = 0, MI_F64 ref = 0, MI_F64 diff = 0) : pos{pos}, val{src, ref, diff}
    {}
 ...
 * @return True if diff value of p0 is greater than p1, false otherwise.
 */
AURA_INLINE MI_BOOL operator>(const ArrayCmpPos &p0, const ArrayCmpPos &p1)
{
    return p0.val[2] > p1.val[2];
}
...
struct ScalarCmpPos
{
    MI_S32 pos[2]; /*!< Array representing the position (vector_idx, scalar_idx). */
    MI_F64 val[3]; /*!< Array representing values (src, ref, diff). */
     ...
     */
    ScalarCmpPos(MI_S32 pos0, MI_S32 pos1, MI_F64 src = 0, MI_F64 ref = 0, MI_F64 diff = 0) : pos{pos0, pos1}, val{src, ref, diff}
    {}
 ...
 */
AURA_INLINE MI_BOOL operator>(const ScalarCmpPos &p0, const ScalarCmpPos &p1)
{
    return p0.val[2] > p1.val[2];
}
...
{
    MI_S32 status;                                  /*!< Status of the comparison. */
    MI_S32 total;                                   /*!< Total number of elements. */
    std::string cmp_method;                         /*!< Comparison method. */
    std::vector<std::pair<MI_F64, MI_S32>> hist;    /*!< Vector of tolerances and corresponding pixel counts. */

    MI_BOOL is_detail;                              /*!< Indicates whether detailed information is available. */
    std::vector<Tp> detail;                         /*!< Vector of detailed differences. */

    /**
     * @brief Default constructor for ArithmCmpResult.
     */
    ArithmCmpResult() : status(0), total(0), is_detail(MI_FALSE)
    {
        hist.clear();
        detail.clear();
    }
...
Status MatCompare_(Context *ctx, const Mat &src, const Mat &ref, MatCmpResult &result,
                   MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
    result.Clear();
    ...
    // compare
    MI_F64 max_diff = 0;
    MI_F64 max_diff_src_val = 0;
    MI_F64 max_diff_ref_val = 0;
    MI_S64 interval = (0 == step) ? 0 : Ceil(tolerate / step);

    std::vector<MI_S32> diff_count(interval + 2, 0);
    ...
    for (auto y = 0; y < size.m_height; ++y)
    {
        ...
        for (auto x = 0; x < size.m_width; ++x)
        {
            ...
            {
                MI_S32 idx = x * size.m_channel + c;
                MI_F64 v_src = SaturateCast<MI_F64>(src_c[idx]);
                MI_F64 v_ref = SaturateCast<MI_F64>(ref_c[idx]);

                // calculate compare error using compare function which can be defined by user
                MI_F64 diff = compare_func(v_src, v_ref);
                ...
                if (diff > cmp_eps)
                {
                    ...
                    MI_S32 diff_idx = (0 == step) ? (diff > tolerate) : Min(Ceil(diff / step), interval + 1);
                    diff_count[diff_idx]++;
                }
                else
                {
                        ...
                        MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
    switch (ref.GetElemType())
    {
        case ElemType::U8:
        {
            MatCompare_<Tp, MI_U8, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S8:
        {
            MatCompare_<Tp, MI_S8, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::U16:
        {
            MatCompare_<Tp, MI_U16, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S16:
        {
            MatCompare_<Tp, MI_S16, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        ...
        case ElemType::U32:
        {
            MatCompare_<Tp, MI_U32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S32:
        {
            MatCompare_<Tp, MI_S32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::F32:
        {
            MatCompare_<Tp, MI_F32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::F64:
        {
            MatCompare_<Tp, MI_F64, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        default:
        {
                  ...
                  MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
    switch (src.GetElemType())
    {
        case ElemType::U8:
        {
            MatCompareHelper<MI_U8, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S8:
        {
            MatCompareHelper<MI_S8, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::U16:
        {
            MatCompareHelper<MI_U16, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S16:
        {
            MatCompareHelper<MI_S16, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
#if defined(AURA_BUILD_HOST)
        case ElemType::F16:
        ...
        case ElemType::U32:
        {
            MatCompareHelper<MI_U32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::S32:
        {
            MatCompareHelper<MI_S32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::F32:
        {
            MatCompareHelper<MI_F32, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        case ElemType::F64:
        {
            MatCompareHelper<MI_F64, CmpFunc>(ctx, src, ref, result, tolerate, step, cmp_eps);
            break;
        }
        default:
        {
...
Status MatCompare(Context *ctx, const std::vector<Mat> &src, const std::vector<Mat> &ref,
                  MatCmpResult &result, MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
...
template<typename Iter, typename CmpFunc = AbsDiff,
         typename std::enable_if<is_arithmetic<typename ArrayCompareTraits<Iter>::type>::value>::type* = MI_NULL>
Status ArrayCompare(Context *ctx, Iter src, Iter ref, MI_S32 num, ArrayCmpResult &result,
                    MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
    result.Clear();
    ...
    std::vector<MI_S32> diff_count(interval + 2, 0);
    MI_F64 max_diff = 0;
    MI_S32 max_diff_idx = 0;

    Iter it_src = src;
    Iter it_ref = ref;
    for (MI_S32 cnt = 0; cnt < num; it_src++, it_ref++, cnt++)
    {
        MI_F64 v_src = SaturateCast<MI_F64>(*it_src);
        MI_F64 v_ref = SaturateCast<MI_F64>(*it_ref);
        MI_F64 diff = compare_func(v_src, v_ref);
        if (diff > cmp_eps)
        {
            if (result.is_detail)
...
Status ScalarCompare(Context *ctx, const std::vector<Scalar> &scalar0, const std::vector<Scalar> &scalar1,
                     ScalarCmpResult &result, MI_F64 tolerate = 1, MI_F64 step = 1, MI_F64 cmp_eps = 1e-6)
{
    ...
    // compare
    MI_S32 num_scalar = static_cast<MI_S32>(scalar0.size());
    MI_S64 interval = (0 == step) ? 0 : Ceil(tolerate / step);

    std::vector<MI_S32> diff_count(interval + 2, 0);
    MI_F64 max_diff = 0;
    MI_S32 max_diff_idx[2] = {0};

    for (MI_S32 idx_scalar = 0; idx_scalar < num_scalar; idx_scalar++)
    {
        for (MI_S32 i = 0; i < 4; i++)
        {
            MI_F64 v0 = scalar0[idx_scalar].m_val[i];
            MI_F64 v1 = scalar1[idx_scalar].m_val[i];
            MI_F64 diff = compare_func(v0, v1);
            if (diff > cmp_eps)
            {
                if (result.is_detail)
                {
